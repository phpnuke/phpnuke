...
$result = $db->query(
    "SELECT c.cid, (SELECT COUNT(c4.cid) FROM " .
        COMMENTS_TABLE .
        " AS c4 WHERE " .
        str_replace("c.", "c4.", $where) .
        " AND c4.cid " .
        ($nuke_comments_configs['order_by'] == 1 ? ">" : "<") .
        "= IF(c.main_parent =0, c.cid,c.main_parent) AND c4.pid ='0') as position FROM " .
        COMMENTS_TABLE .
        " AS c WHERE $where 
...
if ($result->count() > 0) {
        ...
        if (!empty($all_userids)) {
            ...
            if (!empty($result)) {
                ...
                foreach ($users_data as $users_val) {
                    $users_data[$users_val['user_id']]['group_id'] = intval(
                        $users_val['group_id']
                    );
                }
            }
    ...
    foreach ($all_comments as $row) {
        ...
        $shorted =
            mb_strlen(strip_tags(stripslashes($row['comment']))) !==
            mb_strlen(
                mb_word_wrap(
                    strip_tags(stripslashes($row['comment'])),
                    200,
                    null,
                    false
                )
                ...
                ? true
                : false;
        $comment = smilies_parse(
            mb_word_wrap(strip_tags(stripslashes($row['comment'])), 200)
        );
        $ip_info = "http://whatismyipaddress.com/ip/$ip";
        $username_link =
            $username == $ip
                ? $ip_info
                : LinkToGT(sprintf($users_system->profile_url, '', $username));
        ...
        $current_comment_page =
            $nuke_comments_configs['item_per_page'] > 0
                ? ceil($position / $nuke_comments_configs['item_per_page'])
                : 0;
        ...
        if (
            ...
            isset($users_data[$user_id]['group_id']) &&
            $users_data[$user_id]['group_id'] != 0
        ) {
            $forum_groups_cacheData = get_cache_file_contents(
                'nuke_forum_groups'
            );
            if (
                isset(
                    $forum_groups_cacheData[$users_data[$user_id]['group_id']]
                )
            ) {
                $user_colour =
                    "#" .
                    str_replace(
                        "#",
                        "",
                        $forum_groups_cacheData[
                            $users_data[$user_id]['group_id']
                        ]['group_colour']
                    );
                $group_name =
                    $forum_groups_cacheData[$users_data[$user_id]['group_id']][
        ...
        $content .=
            "
				...
				<span class=\"user\" style=\"margin-bottom:8px;\"><i class=\"glyphicon glyphicon-user\"></i> <a href=\"" .
            $url .
            "\" target=\"_blank\">$user_level<span style=\"font-weight:bold;color:" .
            $user_colour .
            "\">" .
            $name .
            "</span></a></span>
				<p class=\"clear\" align=\"justify\"><i class=\"fa fa-comment\" aria-hidden=\"true\"></i> " .
            $comment .
            " ";
        if ($shorted) {
            $content .=
                "<a href=\"" .
                LinkToGT($comment_link . "#comment-$cid") .
                "\" target=\"_blank\">" .
                _MORE .
                "</a>";
        }
        $content .=
            "</p>
				در : <a href=\"" .
            LinkToGT($post_link . "#comment-$cid") .
            "\" target=\"_blank\">$post_title </a>
				</li>
				";
    }
}
