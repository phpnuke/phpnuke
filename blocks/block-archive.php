	...
	foreach($result as $row)
	{
		...
		if($nuke_configs['datetype'] == 1)
		{
			...
			if ($jalalidate[1] != $thisjmonth)
			{
				...
				$content .= "<li><a href=\"".LinkToGT("index.php?modname=Articles&op=article_archive&year=$jalalidate[0]&month=$jalalidate[1]&month_l=$month2")."\">$month, $jalalidate[0]</a>";
		...
		elseif($nuke_configs['datetype'] == 2)
		{
			...
			if ($dateTimes[0] != $thishmonth)
			{
				...
				$month2 = str_replace(" ","-",$month);
				$content .= "<li><a href=\"".LinkToGT("index.php?modname=Articles&op=article_archive&year=$dateTimes[2]&month=$dateTimes[0]&month_l=$month2")."\">$month, $dateTimes[2]</a>";
		...
		else
		{
			...
			if ($dateTimes_month != $thismonth)
			{
				...
				$month2 = str_replace(" ","-",$month);
				$content .= "<li><a href=\"".LinkToGT("index.php?modname=Articles&op=article_archive&&year=$dateTimes_year&month=$dateTimes_month&month_l=$month2")."\">$month, $dateTimes_year</a>";
