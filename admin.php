...
if (isset($aid) && isset($pwd) && isset($op) && $op == "login") {
    if (!empty($aid) and !empty($pwd)) {
        ...
        if (
            extension_loaded("gd") &&
            in_array("admin_login", $nuke_configs['mtsn_gfx_chk'])
        ) {
        ...
        if (
            $code_accepted &&
            isset($nuke_authors_cacheData[$aid]) &&
            !empty($nuke_authors_cacheData[$aid])
        ) {
            $rpwd = $nuke_authors_cacheData[$aid]['pwd'];
            $admlanguage = addslashes(
                $nuke_authors_cacheData[$aid]['admlanguage']
            );

            if (phpnuke_check_password($pwd, $rpwd)) {
                $expiration = 30 * 24 * 3600;

                $admin = phpnuke_generate_user_cookie(
                    "admin",
                    0,
                    $aid,
                    $rpwd,
...
if (isset($admin) && !empty($admin)) {
    ...
    if (empty($aid) or empty($hmac)) {
        ...
        $alert .=
            "<div class=\"text-center\"><img src=\"images/eyes.gif\" border=\"0\" title=\"get out\" alt=\"get out\"><br><br>\n";
        $alert .=
            "<font face=\"Verdana\" size=\"+4\"><b>Get Out!</b></font></div>\n";
        $alert .= "</body>\n";
        $alert .= "</html>\n";
        die($alert);
    }
...
function add_remove_nuke_admins_menu()
{
    ...
    foreach ($main_menus as $admin_module_key => $admin_modules_data) {
        ...
        if ($new_module) {
            $insert_query_values[] = [$admin_module_key];
        }
    }

    if (isset($insert_query_values) && !empty($insert_query_values)) {
        $db->table(ADMINS_MENU_TABLE)->multiinsert(
            ["atitle"],
            $insert_query_values
        );
    ...
    if ($change) {
        ...
        $nuke_admins_menu_cacheData = get_cache_file_contents(
            'nuke_admins_menu'
        );
        phpnuke_db_error();
    }
...
function check_admin_permission($filename, $only_god = false, $modules = false)
{
    ...
    if ($modules) {
        ...
        $module_admins =
            $nuke_modules_cacheData_by_title[$filename]['admins'] != ''
                ? explode(
                    ",",
                    $nuke_modules_cacheData_by_title[$filename]['admins']
                )
...
function login()
{
	...
	<div class=\"module form-module\">
        ...
        _USERNAME .
        "\" name=\"aid\" maxlength=\"25\" />
				<input type=\"password\" name=\"pwd\" maxlength=\"40\" placeholder=\"" .
        _PASSWORD .
        "\" />";
    ...
    $contents .=
        ...
        _USERNAME .
        "\" />
				<input type=\"email\" name=\"admin_email\" placeholder=\"" .
        _EMAIL .
        "\" />";
...
function forget_password(
    ...
    if (
        $mode == 'reset_confirm' &&
        $id_admin != '' &&
        isset($nuke_authors_cacheData[$id_admin]) &&
        isset($new_admin_password) &&
        ...
        if ($code_accepted) {
            if ($new_admin_password[0] != $new_admin_password[1]) {
                ...
                $hashed_admin_password = phpnuke_hash_password(
                    $new_admin_password
                );
    ...
    if (
        ...
        $password_reset =
            $nuke_authors_cacheData[$id_admin]['password_reset'] != ''
                ? phpnuke_unserialize(
                    stripslashes(
                        $nuke_authors_cacheData[$id_admin]['password_reset']
                    )
        ...
        if (in_array($code, $password_reset)) {
			...
			<div class=\"module form-module\">
                ...
                $admin_file .
                ".php\" method=\"post\">
						<input type=\"password\" name=\"new_admin_password[]\" placeholder=\"" .
                _NEWPASSWORD .
                "\" />
						<input type=\"password\" name=\"new_admin_password[]\" placeholder=\"" .
                _NEWPASSWORD_REPEAT .
    ...
    if (
        isset($nuke_authors_cacheData[$id_admin]['email']) &&
        $nuke_authors_cacheData[$id_admin]['email'] != $admin_email
    ) {
    ...
    if (!empty($errors)) {
        ...
        $password_reset =
            $row['password_reset'] != ''
                ? phpnuke_unserialize(stripslashes($row['password_reset']))
                : [];
        $new_hashed_code = sha1($pn_salt . _NOWTIME . $id_admin . $admin_email);
        $password_reset[_NOWTIME] = $new_hashed_code;
        $result['password_reset'] = addslashes(
            phpnuke_serialize($password_reset)
        );
        ...
        $recovery_link =
            ...
            $admin_file .
            ".php?op=forget_password&id_admin=$id_admin&mode=reset&code=$new_hashed_code";

        $message =
            "<table class=\"email-body_inner\" align=\"center\" width=\"570\" cellpadding=\"0\" cellspacing=\"0\" style=\"box-sizing: border-box; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; margin: 0 auto; padding: 0; width: 570px;direction:" .
            _DIRECTION .
            "\" bgcolor=\"#FFFFFF\">
			...
			<tr>
				<td class=\"content-cell\" style=\"box-sizing: border-box; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; padding: 35px;\">
					<h1 style=\"box-sizing: border-box; color: #2F3133; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 19px; font-weight: bold; margin-top: 0;\" align=\"left\">سلام " .
            $nuke_authors_cacheData[$id_admin]['name'] .
            ",</h1>
					<p style=\"box-sizing: border-box; color: #74787E; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 16px; line-height: 1.5em; margin-top: 0;\" align=\"left\">" .
            _REQUEST_RESET_PASSWORD .
            " <strong style=\"box-sizing: border-box; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif;\">" .
            _REQUEST_RESET_PASSWORD_EXPIRE .
            "</strong></p>
					...
					<p style=\"box-sizing: border-box; color: #74787E; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 16px; line-height: 1.5em; margin-top: 0;\" align=\"left\">" .
            sprintf(
                _NO_RESET_PASS_REQUEST,
                "<a href=\"" .
                    $nuke_configs['adminmail'] .
                    "\" style=\"box-sizing: border-box; color: #3869D4; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif;\">contact support</a>"
            ) .
            "</p>
					<p style=\"box-sizing: border-box; color: #74787E; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 16px; line-height: 1.5em; margin-top: 0;\" align=\"left\">" .
            _THANKYOU .
            "</p>
					...
					<table class=\"body-sub\" style=\"border-top-color: #EDEFF2; border-top-style: solid; border-top-width: 1px; box-sizing: border-box; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; margin-top: 25px; padding-top: 25px;\">
						<tr>
							<td style=\"box-sizing: border-box; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif;\">
								<p class=\"sub\" style=\"box-sizing: border-box; color: #74787E; font-family: Tahoma, Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 12px; line-height: 1.5em; margin-top: 0;\" align=\"left\">" .
            _RESET_PASS_LINK_PROBLEM .
            "</p>
        ...
        phpnuke_mail(
            $nuke_authors_cacheData[$id_admin]['email'],
            _RESET_PASSWORD,
            $message
        );
...
function adminmenu($url, $title, $image)
{
    ...
    if (file_exists($image)) {
        ...
        file_exists(
            "themes/" . $nuke_configs['ThemeSel'] . "/images/admin/$image"
        )
    ) {
        $admin_icon =
            "themes/" . $nuke_configs['ThemeSel'] . "/images/admin/$image";
...
function ipcount()
{
    ...
    $numattacks =
        is_array($nuke_mtsn_ipban_cacheData) &&
        !empty($nuke_mtsn_ipban_cacheData)
            ? sizeof($nuke_mtsn_ipban_cacheData)
            : 0;
    return $numattacks;
}
...
{
    global $nuke_configs;
    $file_info = phpnuke_get_url_contents(
        "https://www.phpnuke.ir/version.html",
        true
    );
...
function admin_tables_sortable(
    $fields = [],
    $sort = 'DESC',
    $link_to_more = '',
    $order_by = ''
    ...
    if (is_array($fields) && !empty($fields)) {
        foreach ($fields as $field) {
            $contents .=
                "<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:" .
                $field['width'] .
                ";\"><a href=\"" .
                $admin_file .
                ".php?op=" .
...
function GraphicAdmin()
{
    ...
    $nuke_admins_menu_cacheData_by_atitle = phpnuke_array_change_key(
        $nuke_admins_menu_cacheData,
        'amid',
        'atitle'
    );
    ...
    if ($op != 'adminMain') {
        $contents .=
            "
		<div id=\"admin_menus\">
		<div class=\"admin_menu_toogle\">" .
            _SHOW_ADMINMENUS .
            "</div><div class=\"admin_menu_icons\" style=\"display:none;\">";
    }
    ...
    if ($radminsuper == 1 && $op == 'adminMain') {
        ...
        $total_attackes = 0;
        $articles = $db->query(
            "SELECT COUNT(sid) AS total_articles,
        ...
        $infoicon =
            "<img border='0' src='images/checkbox_checked_todo_icon.png'>";
        $contents .=
            "<table border='0' cellspacing='0' style='border: 0 solid #808080; ' cellpadding='0' width='100%'>
		<tr>
			<td  width='254' colspan='4'></td>
		</tr>
		<tr>
			<td  width='354'>$infoicon " .
            _SEQ_STAT .
            " : $mtsnactive</td>
			<td  width='354'>$infoicon " .
            _CLOSED_IP .
    ...
    foreach ($main_menus as $menu_item_key => $menu_item_data) {
        if (!empty($menu_item_key) && !empty($menu_item_data)) {
            $admins = explode(
                ",",
                $nuke_admins_menu_cacheData_by_atitle[
                    strtolower($menu_item_key)
                ]['admins']
            );
            if (
                $radminsuper != 1 &&
                ($aadminsuper != 1 || !in_array($aid, $admins))
            ) {
    ...
    if (isset($nuke_modules_cacheData) && !empty($nuke_modules_cacheData)) {
        foreach ($nuke_modules_cacheData as $mid => $module_info) {
            ...
            if ($radminsuper == 1 or $auth_user == 1) {
                if (
                    file_exists(
                        "modules/" . $module_info['title'] . "/admin/index.php"
                    ) and
                    file_exists(
                        "modules/" . $module_info['title'] . "/admin/links.php"
                    ) and
                    file_exists(
                        "modules/" . $module_info['title'] . "/admin/case.php"
                    )
                ) {
                    include "modules/" .
                        $module_info['title'] .
    ...
    if (sizeof($main_menus) > 1 || !empty($module_menus)) {
        ...
        $admin_menus['logout'] = adminmenu(
            "" .
                $admin_file .
                ".php?op=logout&csrf_token=" .
                _PN_CSRF_TOKEN .
                "",
            "" . _ADMIN_LOGOUT . "",
            "logout.png"
        ...
        $contents .= OpenAdminTable();
        $contents .=
            "<table border=\"0\" width=\"100%\" cellspacing=\"1\" class=\"product-table td-hover\">\n";
...
function adminMain()
{
    ...
    if ($users_table_exists) {
        $result1 = $db->query(
            "SELECT " .
                $users_system->user_fields['user_regdate'] .
                " as user_regdate FROM " .
        ...
        if ($db->count() > 0) {
            foreach ($result1 as $row1) {
                ...
                if (
                    isset(
                        $registered_users[$user_regdate_year][
                            $user_regdate_month
                        ][$user_regdate_day]
                    )
                ) {
                    $registered_users[$user_regdate_year][$user_regdate_month][
                        $user_regdate_day
                    ]++;
                } else {
                    $registered_users[$user_regdate_year][$user_regdate_month][
    ...
    $result = $db->query(
        "SELECT year, month, day, visitors, hits FROM " .
            STATISTICS_TABLE .
            " WHERE id >= COALESCE((SELECT id FROM " .
            STATISTICS_TABLE .
            " WHERE year = '$last_month_year' AND month = '$last_month_month' AND day >= '$last_month_day' ORDER BY id ASC LIMIT 1), 1) AND id <= COALESCE((SELECT id FROM " .
            STATISTICS_TABLE .
            " WHERE year = '$today_year' AND month = '$today_month' AND day <= '$today_day' ORDER BY id DESC LIMIT 1), (SELECT id FROM " .
            STATISTICS_TABLE .
    ...
    if ($db->count() > 0) {
        foreach ($result as $row) {
            ...
            $contents .=
                ...
                ',
									"column-3": ' .
                (isset($registered_users[$this_year][$this_month][$this_day])
                    ? $registered_users[$this_year][$this_month][$this_day]
                    : 0) .
                ',
									"date": "' .
                $datetime .
    ...
    $contents .=
		...
		<div id=\"chartdiv\" style=\"width: 100%; height: 500px; background-color: #FFFFFF;\" dir=\"ltr\" ></div>
			<table width=\"100%\" class=\"product-table min_table\">
				<tr>
					<td colspan=\"4\" style=\"background:#eee;\" align=\"center\"><b>" .
        _ADMIN_ACTIVITIES .
        "</b><span style=\"float:left;\"><a href=\"" .
        $admin_file .
        ".php?op=show_log_list&log_type=1\">" .
        _ADMIN_ACTIVITIES_ARCHIVE .
        "</a> | <a href=\"" .
        $admin_file .
        ".php?op=show_log_list&log_type=2\">" .
        _USERS_ACTIVITIES_ARCHIVE .
        "</a></span></td>
				</tr>
				<tr>
        ...
        _IP .
        "</th>
					<th class=\"table-header-repeat line-left\" style=\"width:200px\">" .
        _DATE .
        "</th>
        ...
        foreach ($result as $row) {
            ...
            $contents .=
                "<tr style=\"border-bottom:1px solid #eee;\">
								<td style=\"padding-right:10px;\"><span title=\"" .
                sprintf(_COMMENTS_RELATEDTO, $post_title, $module) .
                "\">$comment_short</span></td>
								<td align=\"center\">
									<a href=\"" .
                $admin_file .
                ".php?op=comments_delete&cid=$cid&csrf_token=" .
                _PN_CSRF_TOKEN .
                "\" title=\"" .
                _DELETE .
                "\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('" .
                _SURETODELCOMMENTS .
                "');\"></a>
									<a href=\"" .
                $admin_file .
                ".php?op=comments_status&cid=$cid&new_status=1&csrf_token=" .
                _PN_CSRF_TOKEN .
                "\" title=\"" .
                _APPROVE .
                "\" class=\"table-icon icon-5 info-tooltip\"></a>
									<a href=\"#TB_inline?height=300&amp;width=600&amp;inlineId=comments_" .
                $cid .
                "\" title=\"" .
                _SHOW .
                "\" class=\"table-icon icon-7 info-tooltip thickbox\"></a>
									<div id=\"comments_" .
                $cid .
                "\" style=\"display:none;\">
										<p>$comment</p>
    ...
    $result = $db->query(
        "SELECT * FROM " .
            FEEDBACKS_TABLE .
            " WHERE replys IS NULL OR replys = '' ORDER BY fid DESC LIMIT 0,5"
    );
    ...
    if ($db->count() > 0) {
        foreach ($result as $row) {
            ...
            $custom_fields = phpnuke_unserialize(
                stripslashes($row['custom_fields'])
            );
            ...
            $contents .=
                "<tr style=\"border-bottom:1px solid #eee;\">
								...
								<td align=\"center\">
									<a href=\"" .
                $admin_file .
                ".php?op=feedbacks&mode=delete&fids=$fid&csrf_token=" .
                _PN_CSRF_TOKEN .
                "\" title=\"" .
                _DELETE .
                "\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('" .
                _FEEDBACK_DELETE_CONFIRM .
                "');\"></a>
									<a href=\"" .
                $admin_file .
                ".php?op=reply_feedback_pm&fid=$fid&inline=1\" title=\"" .
                _SHOW .
                "\" class=\"table-icon icon-7 info-tooltip thickbox\"></a>
								</td>
							</tr>";
            ...
            $contents .=
                "<tr style=\"border-bottom:1px solid #eee;\">
								...
								<td align=\"center\">
									<a href=\"" .
                $admin_file .
                ".php?op=reports&mode=delete&rids=$rid&csrf_token=" .
                _PN_CSRF_TOKEN .
                "\" title=\"" .
                _DELETE .
                "\" class=\"table-icon icon-2 info-tooltip\" onclick=\"return confirm('" .
                _REPORT_DELETE_CONFIRM .
                "');\"></a>
									<a href=\"#TB_inline?height=300&amp;width=600&amp;inlineId=reports_" .
                $rid .
                "\" title=\"" .
                _SHOW .
                "\" class=\"table-icon icon-7 info-tooltip thickbox\"></a>
    ...
    $contents .=
		...
		<table width=\"98%\" class=\"product-table min_table no-border\">
			<tr>
				<th class=\"table-header-repeat line-left\" style=\"width:50px;\"><a>" .
        _LATEST_NUKE_NEWS .
        "</a></th>
			</tr>
			<tr><td><ul style=\"list-style:none;margin-" .
        _TEXTALIGN1 .
        ":5px;\">" .
...
function get_latest_news_from_nuke()
{
    ...
    if (
        $cache->isCached("latest_news_from_nuke") &&
        _NOWTIME - $cache->retrieve("latest_news_from_nuke", true) < 3600
    ) {
        return $cache->retrieve("latest_news_from_nuke");
    } else {
        $latest_news_from_nuke = [];
        ...
        $file_info = phpnuke_get_url_contents(
            "https://www.phpnuke.ir/notice.html",
            true
        );
        ...
        if ($file_info) {
            ...
            if (is_array($phpnuke_news) && !empty($phpnuke_news)) {
                foreach ($phpnuke_news as $article) {
                    $PnValidator->validation_rules([
                        'title' => 'required',
                        'link' => 'required',
                    ]);

                    // Get or set the filtering rules
                    $PnValidator->filter_rules([
                        'title' => 'sanitize_string',
                        'link' => 'urldecode|strip_tags',
                    ]);
                    $article = $PnValidator->sanitize(
                        $article,
                        ['title'],
                        true,
                        true
...
function show_log_list($log_type = 1)
{
    ...
    $hooks->add_filter("set_page_title", function () use ($log_name) {
        return ["admin_page" => "فعالیتهای $log_name"];
    });
    ...
    $contents .=
        "<table width=\"100%\" class=\"product-table min_table\">
			<tr>
				<td colspan=\"4\" style=\"background:#eee;\" align=\"center\"><b>" .
        sprintf(_ADMIN_USERS_ACTIONS, $log_name) .
        "</b></td>
			</tr>
				<tr>
					<th class=\"table-header-repeat line-left\" style=\"width:80px\">" .
    ...
    $result_log = $db->query(
        "SELECT *, (SELECT COUNT(lid) FROM " .
            LOG_TABLE .
            " WHERE log_type = ?) as total_rows FROM " .
            LOG_TABLE .
            " WHERE log_type = ? ORDER BY log_time DESC LIMIT $start_at, $entries_per_page",
        [$log_type, $log_type]
    );
...
if ($admintest) {
    ...
    switch ($op) {
        ...
        default:
            ...
            $nuke_modules_cacheData = get_cache_file_contents('nuke_modules');
            if (
                isset($nuke_modules_cacheData) &&
                is_array($nuke_modules_cacheData)
            ) {
                foreach ($nuke_modules_cacheData as $mid => $module_info) {
                    $mod_title = $module_info['title'];
                    if (
                        file_exists("modules/$mod_title/admin/index.php") and
                        file_exists("modules/$mod_title/admin/links.php") and
                        file_exists("modules/$mod_title/admin/case.php")
                    ) {
    ...
    $security_code = isset($security_code) ? $security_code : "";
    $security_code_id = isset($security_code_id) ? $security_code_id : "";
    $new_admin_password = isset($new_admin_password) ? $new_admin_password : [];
    switch ($op) {
