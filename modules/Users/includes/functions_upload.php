...
class filespec
{
	...
	*/
	...
	{
		if (!isset($upload_ary) || $upload_ary == '')
		{
		...
		$name = stripslashes($upload_ary['name']);
		...
		if ($this->is_image() && !$skip_image_check)
		{
			...
			if (($this->image_info = @getimagesize($this->destination_file)) !== false)
			{
				...
				if (!isset($types[$this->image_info[2]]) || !in_array($this->extension, $types[$this->image_info[2]]))
				{
					...
					{
						$this->error[] = 'IMAGE_FILETYPE_INVALID';
					}
					else
					{
						$this->error[] = 'IMAGE_FILETYPE_MISMATCH';
					}
				}
	...
	*/
	function additional_checks()
	{
		...
		{
			$max_filesize = formatBytes($this->upload->max_filesize);

			$this->error[] = 'WRONG_FILESIZE';
	...
	*/
	function form_upload($form_name)
	{
		...
		if ($file->get('filename') == 'none')
		{
			...
			$file->error[] = (empty($max_filesize)) ? 'PHP_SIZE_NA' : 'PHP_SIZE_OVERRUN';
			return $file;
		}
	...
	function remote_upload($upload_url)
	{
		...
		while (!@feof($fsock))
		{
			if ($get_info)
			{
				...
				{
					$max_filesize = formatBytes($this->max_filesize);

					$file = new fileerror('WRONG_FILESIZE');
					return $file;
				}
		...
		$tmp_path = (!@ini_get('safe_mode') || strtolower(@ini_get('safe_mode')) == 'off') ? false : 'cache';

		$filename = tempnam($tmp_path, uniqid(microtime(), true) . '-');
	...
	function assign_internal_error($errorcode)
	{
		...
		switch ($errorcode)
		{
			case 1:
				...
				$error = (empty($max_filesize)) ? 'PHP_SIZE_NA' : 'PHP_SIZE_OVERRUN';
			break;

			case 2:
				$max_filesize = formatBytes($this->max_filesize);

				$error = 'WRONG_FILESIZE';
			break;
