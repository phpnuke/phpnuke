...
$_REQUEST['modname'] = isset($_REQUEST['modname'])
    ? filter($_REQUEST['modname'], "nohtml")
    : "";
$_REQUEST['file'] = isset($_REQUEST['file'])
    ? filter($_REQUEST['file'], "nohtml")
    : "index";
...
if (
    isset($redirect) && filter($redirect, "nohtml") != '' and is_admin($admin)
) {
    $redirect = adv_filter($redirect, [''], ['valid_url']);
...
if (
    ...
    $main_module != '' &&
    (trim($REQUESTURL, "/") == '' ||
        in_array($REQUESTURL, ["index.php", "index.html", "index.htm"]))
) {
    // is in home
    $modname = $main_module;
...
if (
    stripos_clone($modname, "..") ||
    (isset($file) && stripos_clone($file, "..")) ||
    stripos_clone($op, "..")
) {
    die("You are so cool...");
} elseif (
    defined("HOME_FILE") &&
    isset($nuke_configs['website_index_theme']) &&
    $nuke_configs['website_index_theme'] == 1 &&
    (file_exists(
        "themes/" . $nuke_configs['ThemeSel'] . "/website_index.php"
    ) ||
        function_exists("website_index"))
) {
    ...
    if (
        stripos_clone($modname, "..") or
        stripos_clone($modstring, "&file=nickpage") ||
            stripos_clone($modstring, "&user=")
    ) {
        header("Location: " . LinkTOGT("index.php") . "");
    }
    ...
    if (isset($nuke_modules_cacheData_by_title[$modname])) {
        $mod_active = intval(
            $nuke_modules_cacheData_by_title[$modname]['active']
        );
        $mod_permissions =
            $nuke_modules_cacheData_by_title[$modname]['mod_permissions'] != ""
                ? explode(
                    ",",
                    $nuke_modules_cacheData_by_title[$modname][
                        'mod_permissions'
                    ]
                )
                : [0];
        ...
        if (
            ...
            if (
                $allow_to_view ||
                $nuke_modules_cacheData_by_title[$modname]['main_module'] ==
                    1 ||
                defined("INDEX_FILE")
            ) {
                ...
                if (file_exists($modpath)) {
                    ...
                    simple_output(
                        "<br><div class=\"text-center\">" .
                            _SORRY_PAGE_NOTFOUND .
                            "</div><br>"
                    );
                }
            } else {
                simple_output(
                    "<div class=\"text-center\">$disallow_message<br><br>" .
                        _GOBACK .
                        "</div>"
                );
            }
