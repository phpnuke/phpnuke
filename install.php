...
if (version_compare(PHP_VERSION, '5.4.0', "<")) {
    die(
        "<p align=\"center\">sorry. PHPNuke 8.4 MT Edition needs php 5.4.0 or above. your Server PHP Version is : " .
            PHP_VERSION .
            "</p>"
    );
}
...
if ($cache->isCached('install_options')) {
    ...
    if (
        isset($install_options['admininfo']) &&
        isset($install_options['db_info'])
    ) {
        if ($install_options['mode'] == 'upgrade') {
            define("OLD_DB", $install_options['admininfo']['old_dbname']);
            define(
                "OLD_DB_PREFIX",
                $install_options['admininfo']['old_dbprefix']
            );
        }
...
function upgrade_header($step = 1, $progress = 0)
{
    ...
    $step_title =
        isset($install_options['mode']) && $install_options['mode'] == 'install'
            ? "اطلاعات مدیر جدید"
            : "اطلاعات نسخه قدیم";
...
<body class=\"modal-open\">
    ...
    if (
        isset($install_options['mode']) &&
        $install_options['mode'] == 'upgrade'
    ) {
...
function step_db()
{
    ...
    $mode =
        isset($mode) && in_array($mode, ["install", "upgrade"])
            ? $mode
            : "install";
			...
			<div class=\"wizard-card\" data-cardname=\"group\" style=\"height: 300px;\">
				...
				<div class=\"wizard-input-section\">
					...
					<div class=\"form-group\">
						...
						<div class=\"col-sm-8\">
							<input type=\"text\" class=\"form-control\" id=\"pn_server_prefix\" name=\"db_fields[db_prefix]\" value=\"nuke\" placeholder=\"پیشفرض nuke\" />
						</div>
					</div>
				</div>
			</div>
...
function step_server_check()
{
    ...
    $import_db = Database::connect(
        $db_fields['db_server_name'],
        $db_fields['db_username'],
        $db_fields['db_password'],
        $db_fields['db_name'],
        $pn_dbtype,
        $pn_dbfetch,
        $pn_dbcharset,
    ...
    $install_options['db_info']['db_prefix'] =
        $install_options['db_info']['db_prefix'] == ''
            ? "nuke"
            : $install_options['db_info']['db_prefix'];
    ...
    if (!file_exists("config.php")) {
        if (!@rename("config.default.php", "config.php")) {
            ...
            $configstatus =
                "<span class=\"fail\"><strong>عدم دسترسی</strong></span>";
            $showerror = 1;
        }
    }
    ...
    if (version_compare(PHP_VERSION, '5.4.0', "<")) {
        $errors[] =
            "برای نصب نسخه 8.4 نیوک باید نسخه PHP سرور شما 5.4.0 به بالا باشد. نسخه فعلی : " .
            PHP_VERSION .
            "";
        $phpversion =
            "<span class=\"fail\"><strong>" . PHP_VERSION . "</strong></span>";
        $showerror = 1;
    } else {
    ...
    if (count($mboptions) < 1) {
        ...
        $mbstatus =
            "<span class=\"pass\">" . implode(', ', $mboptions) . "</span>";
    }
    ...
    if (count($supported_dbs) < 1) {
        $errors[] =
            'نیوک برای نصب نیاز به حداقل یکی از انواع دیتابیس ها دارد. سرور شما هیچ دیتابیسی را پشتیبانی نمی کند';
        $dbsupportlist = "<span class=\"fail\"><strong>None</strong></span>";
        $showerror = 1;
    } else {
        $dbsupportlist =
            "<span class=\"pass\">" . implode(', ', $supported_dbs) . "</span>";
    }

    // Check config file is writable
    if (!is_writable('config.php')) {
        $errors[] =
            "فایل تنظیمات (config.php) قابل بازنشانی نیست. لطفاً حق دسترسی به این فایل را 777 نمایید";
        $configstatus =
            "<span class=\"fail\"><strong>عدم دسترسی</strong></span>";
        $showerror = 1;
    } else {
        $configstatus =
            "<span class=\"pass\"><strong>دسترسی مجاز</strong></span>";
    }
    ...
    if (!is_writable(dirname('cache/'))) {
        $errors[] =
            "دسترسی ایجاد فایل در پوشه (cache/) وجود ندارد. لطفاً حق دسترسی به این پوشه را 777 نمایید.";
        $cachestatus =
            "<span class=\"fail\"><strong>عدم دسترسی</strong></span>";
        $showerror = 1;
    } else {
    ...
    if (!is_writable(dirname('files/uploads/'))) {
        $errors[] =
            "دسترسی ایجاد فایل در پوشه (files/uploads/) وجود ندارد. لطفاً حق دسترسی به این پوشه را 777 نمایید.";
        $uploadsstatus =
            "<span class=\"fail\"><strong>عدم دسترسی</strong></span>";
        $showerror = 1;
    } else {
    ...
    if (!is_writable(dirname('files/Articles/'))) {
        $errors[] =
            "دسترسی ایجاد فایل در پوشه (files/Articles/) وجود ندارد. لطفاً حق دسترسی به این پوشه را 777 نمایید.";
        $Articlesstatus =
            "<span class=\"fail\"><strong>عدم دسترسی</strong></span>";
        $showerror = 1;
    } else {
    ...
    if ($showerror == 1) {
        ...
        $message =
            "سرور شما همه پیش نیازهای نصب نیوک 8.4 را داراست. میتوانید به مرحله بعد بروید";
    }
...
{
    global $db, $nuke_configs, $mode, $Req_URL, $cache;
...
function step_install()
{
    ...
    $install_options['admininfo']['old_site_link'] =
        $install_options['mode'] == 'install'
            ? ""
            : (isset($install_fields['old_site_link']) &&
            !empty($install_fields['old_site_link'])
                ? $install_fields['old_site_link']
                : "");

    if (
        $install_options['admininfo']['admin_filename'] != '' &&
        $install_options['admininfo']['admin_filename'] != 'admin' &&
        file_exists("admin.php") &&
        !file_exists($install_options['admininfo']['admin_filename'] . ".php")
    ) {
        if (
            !rename(
                "admin.php",
                $install_options['admininfo']['admin_filename'] . ".php"
            )
        ) {
            upgrade_header(7, 95);
            echo "<div class=\"wizard-card-container\" style=\"height: 326px;\">
    ...
    if (
        $install_options['db_info']['db_server_name'] == '' ||
        $install_options['db_info']['db_username'] == '' ||
        $install_options['db_info']['db_name'] == ''
    ) {
    ...
    $import_db = Database::connect(
        $install_options['db_info']['db_server_name'],
        $install_options['db_info']['db_username'],
        $install_options['db_info']['db_password'],
        $install_options['db_info']['db_name'],
        $pn_dbtype,
        $pn_dbfetch,
        $pn_dbcharset,
    ...
    $default_tables = [
        ...
        "`nuke_users`",
        "`nuke_users_fields`",
        "`nuke_users_fields_values`",
        "`nuke_users_invites`",
    ];

    $import_db->query(
        "DROP TABLE IF EXISTS " . implode(", ", $default_tables) . ";"
    );
    ...
    if ($install_options['db_info']['db_prefix'] != 'nuke') {
        foreach ($default_tables as $key => $default_table) {
            $new_table_name = str_replace(
                "nuke_",
                $install_options['db_info']['db_prefix'] . "_",
                $default_table
            );
...
function upgrade_progress_output(
    $pagetitle = '',
    $total_rows = 0,
    $fetched_rows = 0,
    $start = 0,
    ...
    $percent =
        $fetched_rows == 0 ||
        ($fetched_rows > 0 && $fetched_rows < $total_proccess && $start != 0) ||
        ($fetched_rows > 0 && $fetched_rows == $total_rows)
            ? 100
            : (int) ((($fetched_rows + $start) / $total_rows) * 100);
    ...
    if ($finish_page != '') {
        ...
        echo "<div class=\"wizard-success\">
				<div class=\"alert alert-success\">
					<span class=\"create-server-name\"></span>نیوک شما <strong>با موفقیت " .
            ($install_options['mode'] == 'install' ? "نصب" : "بروزرسانی") .
            " شد.</strong>
				</div> 
				<a class=\"btn btn-default\" href=\"" .
            $install_options['admininfo']['admin_filename'] .
            ".php\">ورود به مدیریت</a>  
				<a class=\"btn btn-default\" href=\"" .
            $install_options['siteinfo']['nukeurl'] .
            "\">نمایش سایت</a>";
        if (
            isset($install_options['db_info']['db_have_forum']) &&
            $install_options['db_info']['db_have_forum'] == 1
        ) {
            echo "<a class=\"btn btn-default\" target=\"_blank\" href=\"" .
                $install_options['siteinfo']['nukeurl'] .
                $install_options['db_info']['db_forumpath'] .
                "/install/\">" .
                ($install_options['mode'] == 'install' ? "نصب" : "بروزرسانی") .
                " تالار گفتمان</a>";
        }
...
function upgrade_first()
{
    ...
    $insert_query = [];
    $result = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_authors`"
    );

    if (
        isset($install_fields) &&
        isset($install_fields['pwd']) &&
        !empty($install_fields['pwd'])
    ) {
        if (!$cache->isCached('install_options')) {
            upgrade_header(7, 95);
            echo "<div class=\"wizard-card-container\" style=\"height: 326px;\">
    ...
    if (intval($result->count()) > 0) {
        ...
        if (
            $install_options['mode'] == 'upgrade' &&
            !empty($default_admin_pwds) &&
            !in_array(
                md5($install_options['admininfo']['pwd']),
                $default_admin_pwds
            )
        ) {
            upgrade_header(7, 95);
        ...
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(AUTHORS_TABLE)->multiinsert(
                [
                    "aid",
                    "name",
                    "url",
    ...
    $result = $db->query(
        "SELECT * FROM `" .
            ...
            "`.`" .
            OLD_DB_PREFIX .
            "_blocks` ORDER BY bposition ASC, weight ASC"
    );
    if (intval($result->count()) > 0) {
        $rows = $result->results();
        foreach ($rows as $row) {
            if (
                $row['blockfile'] != '' &&
                !file_exists("blocks/" . $row['blockfile'] . "")
            ) {
                continue;
            }
            ...
            $box_blocks_data[$box_id][$row['bid']] = [
                "title" => $row['title'],
                "lang_titles" => !empty($lang_titles)
                    ? phpnuke_serialize($lang_titles)
                    : "",
        ...
        $result = $db->query(
            "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_message`"
        );
        if (intval($result->count()) > 0) {
            $rows = $result->results();
            foreach ($rows as $row) {
                ...
                $box_id = "topcenter";
                $top_center_weight++;
                $box_blocks_data[$box_id][$last_bid] = [
                    "title" => $row['title'],
                    "lang_titles" => !empty($lang_titles)
                        ? phpnuke_serialize($lang_titles)
                        : "",
        ...
        if (isset($insert_query1) && !empty($insert_query1)) {
            $blocks_insert = $db
                ->table(BLOCKS_TABLE)
                ->multiinsert(
                    ["bid", "title", "content", "url", "refresh", "blockfile"],
                    $insert_query1
                );
        }
        ...
        if (
            $blocks_insert &&
            isset($box_blocks_data) &&
            !empty($box_blocks_data)
        ) {
    ...
    $query_IN = [];
    $feedback_configs =
        isset($nuke_configs['feedbacks']) &&
        !empty($nuke_configs['feedbacks']) &&
        !is_array($nuke_configs['feedbacks'])
            ? phpnuke_unserialize($nuke_configs['feedbacks'])
            : $nuke_configs['feedbacks'];
    $feedback_configs['depts'] = [];

    $db->query("set names 'latin1'");
    $result = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_config`"
    );
    if (intval($result->count()) > 0) {
        ...
        if (intval($result2->count()) > 0) {
            ...
            if (!empty($rows2)) {
                foreach ($rows2 as $row2) {
                    $feedback_configs['depts'][$row2['did']] = [
                        $row2['dname'],
                        $row2['demail'],
                        $row2['dresponsibility'],
                    ];
                }
            }
            $feedback_configs = phpnuke_serialize($feedback_configs);
            $when_query['feedbacks'] =
                "WHEN config_name = 'feedbacks' THEN :" .
                $feedback_configs .
                "";
        ...
        if (isset($old_rows) && !empty($old_rows)) {
            ...
            $nuke_configs = [];
            $config_result = $db->table(CONFIG_TABLE)->select();
            ...
            foreach ($old_rows as $config_key => $config_val) {
                if (!isset($nuke_configs[$config_key])) {
                    $insert_query[] = [$config_key, $config_val];
                }
            }
    ...
    $insert_query = [];
    $result = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_headlines`"
    );
    if (intval($result->count()) > 0) {
        $rows = $result->results();
        $hid = 0;
        ...
        $insert_query[] = [$hid + 1, 'phpnuke', 'http://www.phpnuke.ir/feed/'];

        $db->query("set names '$pn_dbcharset'");
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(HEADLINES_TABLE)->multiinsert(
                ["hid", "sitename", "headlinesurl"],
                $insert_query
            );
        }
    }
    ...
    $when_query = [];
    $result = $db->query(
        "SELECT g.*, p.points FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_groups_info` as g LEFT JOIN `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_groups_points` as p ON g.id = p.id ORDER BY g.id ASC"
    );
    if (intval($result->count()) > 0) {
        $rows = $result->results();
        ...
        foreach ($rows as $row) {
            $when_query[$row['id']] =
                "WHEN id = '" . $row['id'] . "' THEN " . $row['points'] . "";
        }
    ...
    $result1 = $db->query(
        "SELECT SUM(g_hits) as total_hits FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_stats_hour` WHERE g_year != '0'"
    );
    ...
    $result2 = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_counter"
    );
    if (intval($result2->count()) > 0) {
        $rows2 = $result2->results();
        if (!empty($rows2)) {
            ...
            $p_br_others =
                $p_MSIE + $p_FireFox + $p_Opera < 100
                    ? 100 - ($p_MSIE + $p_FireFox + $p_Opera)
                    : 0;
            ...
            $p_os_others =
                $p_Windows + $p_Linux + $p_Mac < 100
                    ? 100 - ($p_Windows + $p_Linux + $p_Mac)
                    : 0;
            ...
            $br_diff =
                $total_hits -
                ($Msie_hits + $Firefox_hits + $Opera_hits + $Others_br_hits);
            if ($br_diff != 0) {
            ...
            $os_diff =
                $total_hits -
                ($win7_hits + $Linux_hits + $MacOSX_hits + $Others_os_hits);
            if ($os_diff != 0) {
    ...
    $when_query = [];
    $result = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_mostonline"
    );
    if (intval($result->count()) > 0) {
        $rows = $result->results();
        $row = $rows[0];
        ...
        $when_query[] =
            "WHEN `type` = 'browser' AND var = 'Msie' THEN " . $Msie_hits . "";
        $when_query[] =
            "WHEN `type` = 'browser' AND var = 'Firefox' THEN " .
            $Firefox_hits .
            "";
        $when_query[] =
            "WHEN `type` = 'browser' AND var = 'Opera' THEN " .
            $Opera_hits .
            "";
        $when_query[] =
            "WHEN `type` = 'browser' AND var = 'Others' THEN " .
            $Others_br_hits .
            "";
        $when_query[] =
            "WHEN `type` = 'os' AND var = 'win 7' THEN " . $win7_hits . "";
        $when_query[] =
            "WHEN `type` = 'os' AND var = 'Linux' THEN " . $Linux_hits . "";
        $when_query[] =
            "WHEN `type` = 'os' AND var = 'Mac OS X' THEN " . $MacOSX_hits . "";
        $when_query[] =
            "WHEN `type` = 'os' AND var = 'Others' THEN " .
            $Others_os_hits .
            "";
        $when_query[] = "WHEN `type` = 'total' THEN " . $total_hits . "";
        $when_query[] =
            "WHEN `type` = 'mosts' AND var = 'total' THEN " .
            $row['total'] .
            "";
        $when_query[] =
            "WHEN `type` = 'mosts' AND var = 'members' THEN " .
            $row['members'] .
            "";
        $when_query[] =
            "WHEN `type` = 'mosts' AND var = 'guests' THEN " .
            $row['nonmembers'] .
            "";
	...
	INNER JOIN `" .
            ...
            "`.`" .
            OLD_DB_PREFIX .
            "_poll_data` as pd ON p.pollID = pd.pollID AND optionText != ''
	GROUP BY p.pollID
    ...
    if (intval($result->count()) > 0) {
            ...
            if (isset($optionTextlist) && !empty($optionTextlist)) {
                foreach ($optionTextlist as $op_key => $optionText) {
                    if ($optionText != '' && isset($optionCountlist[$op_key])) {
                        $options[] = [$optionText, $optionCountlist[$op_key]];
                    }
                }
            }
        ...
        if (isset($insert_query) && !empty($insert_query)) {
            $db->query("set names '$pn_dbcharset'");
            $db->table(SURVEYS_TABLE)->multiinsert(
                [
                    "pollID",
                    "status",
                    "aid",
            ...
            $db->query(
                "ALTER TABLE `" .
                    ...
                    "`.`" .
                    OLD_DB_PREFIX .
                    "_poll_check` ADD `id` INT NOT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`id`); "
            );
            $db->query(
                "INSERT INTO `" .
                    SURVEYS_CHECK_TABLE .
...
function upgrade_cateogories()
{
    ...
    $insert_query = [];
    $result = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_topics`"
    );
    if (intval($result->count()) > 0) {
        $rows = $result->results();
        foreach ($rows as $row) {
    ...
    $result = $db->query(
        "SELECT * FROM `" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_faqcategories`"
    );
    if (intval($result->count()) > 0) {
        $insert_query[] = [
            'Null',
    ...
    $db->query("set names '$pn_dbcharset'");
    if (isset($insert_query) && !empty($insert_query)) {
        $db->table(CATEGORIES_TABLE)->multiinsert(
            [
                "catid",
                "type",
                "module",
...
function upgrade_feedbacks($start)
{
    ...
    $run_per_step = 500;
    $transfer_counter =
        isset($transfer_counter) && $transfer_counter != 0
            ? $transfer_counter
            : $run_per_step;
    ...
    $insert_query = [];
    $result = $db->query(
        "SELECT *, (SELECT COUNT(fid) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_feedbacks`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_feedbacks` ORDER BY fid ASC LIMIT $start, $transfer_counter"
    );
    ...
    if ($fetched_rows > 0) {
        ...
        $db->query("set names '$pn_dbcharset'");
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(FEEDBACKS_TABLE)->multiinsert(
                [
                    "fid",
                    "sender_name",
                    "sender_email",
    ...
    if (
        ...
        ($errors['message'] == "MySQL server has gone away" ||
            stristr($errors['message'], "max_allowed_packet"))
    ) {
        upgrade_header(7, 95);
        echo "
		<form role=\"form\" class=\"form-horizontal\" id=\"nukeform\" action=\"install.php?op=feedbacks" .
            (isset($start) && $start != 0 ? "&start=$start" : "") .
            "\" method=\"post\">
			<div class=\"wizard-card-container\" style=\"height: 326px;\">
				<div class=\"wizard-card\" data-cardname=\"group\">
					<h3>بروزرسانی سیستم</h3>
    ...
    upgrade_progress_output(
        "انتقال پیامهای ارتباط با ما",
        $total_rows,
        $fetched_rows,
        $start,
        "install.php?op=ipbans",
        "install.php?op=feedbacks&start=$new_start",
        12,
        $transfer_counter
    );
}
...
function upgrade_ipbans($start)
{
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(id) FROM `" .
            ...
            "`.`" .
            OLD_DB_PREFIX .
            "_mtsn_ipban`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_mtsn_ipban` ORDER BY id ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    if ($fetched_rows > 0) {
        $rows = $result->results();
        ...
        $db->query("set names '$pn_dbcharset'");
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(MTSN_IPBAN_TABLE)->multiinsert(
                ["id", "blocker", "ipaddress", "system", "time"],
                $insert_query
            );
        }
    }
    ...
    upgrade_progress_output(
        "انتقال آی پی های مسدود شده",
        $total_rows,
        $fetched_rows,
        $start,
        "install.php?op=reports",
        "install.php?op=ipbans&start=$new_start",
        18,
        $run_per_step
    );
}
...
function upgrade_reports($start)
{
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(rid) FROM `" .
            ...
            "`.`" .
            OLD_DB_PREFIX .
            "_reports` WHERE rcode = '1') as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_reports` WHERE rcode = '1' ORDER BY rid ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    if ($fetched_rows > 0) {
        $rows = $result->results();
        ...
        $db->query("set names '$pn_dbcharset'");
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(REPORTS_TABLE)->multiinsert(
                ["rid", "post_id", "user_id", "subject", "message", "time"],
                $insert_query
            );
        }
    }
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=scores",
        "install.php?op=reports&start=$new_start",
        24,
        $run_per_step
    );
}
...
function upgrade_scores($start)
{
    ...
    $insert_query = [];
    $result = $db->query(
        "SELECT s.*, u.user_id, (SELECT COUNT(id) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_stories_score`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_stories_score` as s LEFT JOIN `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_users` as u ON u.username = s.username ORDER BY s.id ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    if ($fetched_rows > 0) {
        $rows = $result->results();
        ...
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(SCORES_TABLE)->multiinsert(
                [
                    "id",
                    "post_id",
                    "db_table",
    ...
    upgrade_progress_output(
        "انتقال امتیازات اخبار",
        $total_rows,
        $fetched_rows,
        $start,
        "install.php?op=statistics",
        "install.php?op=scores&start=$new_start",
        30,
        $run_per_step
    );
}
...
function upgrade_statistics($start = 0)
{
    ...
    if ($start == 0) {
        ...
        $db->query(
            "INSERT INTO `nuke_stats_hour` SELECT * FROM `" .
                OLD_DB .
                "`.`" .
                OLD_DB_PREFIX .
                "_stats_hour`;"
        ...
        $db->query(
            "ALTER TABLE `nuke_stats_hour` MODIFY shid INT AUTO_INCREMENT PRIMARY KEY"
        );
        $result1 = $db->query(
            "SELECT shid FROM `nuke_stats_hour` ORDER BY shid DESC LIMIT 0,1"
        );
    ...
    $run_per_step = 5000;
    $result = $db->query(
        "SELECT shid, g_year, g_month, g_date, g_hour, g_hits FROM `nuke_stats_hour` WHERE shid > '$start' ORDER BY shid ASC LIMIT 0,$run_per_step"
    );
    ...
    if ($fetched_rows > 0) {
        ...
        foreach ($rows as $row) {
            if (
                ...
                $statistics[$row['g_year']][$row['g_month']][$row['g_date']][
                    $row['g_hour']
                ] = $row['g_hits'];
        ...
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(STATISTICS_TABLE)->multiinsert(
                ["year", "month", "day", "hourly_info", "hits"],
                $insert_query
            );
        }
    }
    ...
    if (
        $fetched_rows == 0 ||
        ($fetched_rows > 0 && $fetched_rows < $run_per_step && $start != 0)
    ) {
        $db->query("DROP TABLE IF EXISTS `nuke_stats_hour`;");
    }
    ...
    upgrade_progress_output(
        ...
        $start,
        "install.php?op=tags",
        "install.php?op=statistics&start=$new_start",
        36,
        $run_per_step
    );
}
...
function upgrade_tags($start)
{
    ...
    $db->query("set names 'latin1'");
    $result = $db->query(
        "SELECT *, (SELECT COUNT(tag_id) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_tags`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_tags` ORDER BY tag_id ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    if ($fetched_rows > 0) {
        $rows = $result->results();
        ...
        foreach ($rows as $row) {
            ...
            $insert_query[] = [$row['tag_id'], $row['tag'], $row['counter']];
        }

        $db->query("set names '$pn_dbcharset'");
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(TAGS_TABLE)->multiinsert(
                ["tag_id", "tag", "counter"],
                $insert_query
            );
        }
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=articles",
        "install.php?op=tags&start=$new_start",
        42,
        $run_per_step
    );
}
...
function upgrade_articles($start)
{
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(sid) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_stories`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_stories` ORDER BY sid ASC LIMIT $start, $transfer_counter"
    );
    $fetched_rows = intval($result->count());
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                foreach ($row as $col_name => $col_val) {
                    if (
                        !in_array($col_name, $default_cols) &&
                        $col_name != 'total_rows'
                    ) {
                        $meta_keys[$row['sid']] = [$col_name, $col_val];
                    }
                }
                ...
                $newsurl =
                    $row['newsurl'] != '' ? $row['newsurl'] : $row['title'];

                $post_url = trim(sanitize(str2url($newsurl)), "-");
                $post_url = get_unique_post_slug(
                    POSTS_TABLE,
                    "sid",
                    $row['sid'],
                    "post_url",
                ...
                $post_image = '';
                $old_post_image =
                    "$old_site_path/files/News/" . $row['sid'] . ".jpg";
            ...
            if (isset($meta_keys) && !empty($meta_keys)) {
                foreach ($meta_keys as $sid => $meta_data) {
                    $insert_query[] = [$sid, $meta_data[0], $meta_data[1]];
                }
                $db->table(POSTS_META_TABLE)->multiinsert(
    ...
    if (
        ...
        upgrade_header(7, 95);
        echo "
		<form role=\"form\" class=\"form-horizontal\" id=\"nukeform\" action=\"install.php?op=articles" .
            (isset($start) && $start != 0 ? "&start=$start" : "") .
            "\" method=\"post\">
			<div class=\"wizard-card-container\" style=\"height: 326px;\">
				<div class=\"wizard-card\" data-cardname=\"group\">
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=staticpages",
        "install.php?op=articles&start=$new_start&transfer_counter=$transfer_counter",
        48,
        $transfer_counter
    );
}
...
function upgrade_staticpages($start)
{
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(pid) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_staticpages`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_staticpages` ORDER BY pid ASC LIMIT $start, $transfer_counter"
    );
    $fetched_rows = intval($result->count());
    ...
    if (
        ...
        upgrade_header(7, 95);
        echo "
		<form role=\"form\" class=\"form-horizontal\" id=\"nukeform\" action=\"install.php?op=staticpages" .
            (isset($start) && $start != 0 ? "&start=$start" : "") .
            "\" method=\"post\">
			<div class=\"wizard-card-container\" style=\"height: 326px;\">
				<div class=\"wizard-card\" data-cardname=\"group\">
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=gallery",
        "install.php?op=staticpages&start=$new_start&transfer_counter=$transfer_counter",
        54,
        $transfer_counter
    );
}
...
function upgrade_gallery($start)
{
    ...
    $gallery_posts = $cache->isCached('gallery_posts')
        ? $cache->retrieve('gallery_posts', false)
        : [];

    $result = $db->query(
        "SELECT g.id_cat, g.categories as title, g.catpic as post_image, gp.dateadd as time, gp.picdec as hometext, gp.pictname, gp.picname, gp.count as counter,(SELECT COUNT(g2.id_cat) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_mtgalcat` as g2 LEFT JOIN `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_mtgalpic` as gp2 ON g2.id_cat = gp2.id_cat ) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_mtgalcat` as g LEFT JOIN `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_mtgalpic` as gp ON g.id_cat = gp.id_cat ORDER BY g.id_cat ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                ...
                if (!isset($gallery_posts[$id_cat])) {
                    ...
                    $gallery_posts[$id_cat]['download'] = [];
                    $post_url = trim(sanitize(str2url($row['title'])), "-");
                    $gallery_posts[$id_cat]['post_url'] = get_unique_post_slug(
                        POSTS_TABLE,
                        "sid",
                        '',
                        "post_url",
                    ...
                    $old_post_image =
                        "$old_site_path/modules/MT-Gallery/images/" .
                        $row['post_image'] .
                        "";
                    $new_post_image =
                        "files/Gallery/" .
                    ...
                    if (file_exists($old_post_image)) {
                        if (copy($old_post_image, $new_post_image)) {
                            $gallery_posts[$id_cat][
                                'post_image'
                            ] = $new_post_image;
                        }
                    }
                    $gallery_posts[$id_cat]['time'] = isset($row['time'])
                        ? $row['time']
                        : _NOWTIME;
                ...
                $old_image_path =
                    ...
                    "";
                $new_image_path =
                    "files/Gallery/" . $id_cat . "/" . $row['pictname'] . "";
        ...
        if (isset($gallery_posts) && !empty($gallery_posts)) {
            foreach ($gallery_posts as $imported_id => $row) {
                $insert_query[] = [
                    ...
                    0,
                    0,
                    isset($row['download']) && !empty($row['download'])
                        ? phpnuke_serialize($row['download'])
                        : "",
                    $imported_id,
                ];
            }
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=gallery&gallery_insert=1",
        "install.php?op=gallery&start=$new_start",
        60,
        $run_per_step
    );
}
...
function upgrade_downloads($start)
{
    ...
    if ($cat_result->count() > 0) {
        ...
        foreach ($cat_rows as $cat_row) {
            $download_cats[$cat_row['imported_id']] = [
                'catid' => $cat_row['catid'],
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(lid) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_downloads_downloads`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_downloads_downloads` ORDER BY lid ASC LIMIT $start, $transfer_counter"
    );
    $fetched_rows = intval($result->count());
    ...
    if ($fetched_rows > 0) {
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                ...
                $cat_link = isset($download_cats[$row['cid']]['catid'])
                    ? $download_cats[$row['cid']]['catid']
                    : 0;
                ...
                $download[] = [
                    ...
                    $row['url'],
                    $row['filesize'],
                    "ویرایش " . ($row['version'] != 0 ? $row['version'] : 1),
                    'files',
                ];
    ...
    if (
        ...
        upgrade_header(7, 95);
        echo "
		<form role=\"form\" class=\"form-horizontal\" id=\"nukeform\" action=\"install.php?op=downloads" .
            (isset($start) && $start != 0 ? "&start=$start" : "") .
            "\" method=\"post\">
			<div class=\"wizard-card-container\" style=\"height: 326px;\">
				<div class=\"wizard-card\" data-cardname=\"group\">
    ...
    upgrade_progress_output(
        "انتقال فایلهای بخش دانلود",
        $total_rows,
        $fetched_rows,
        $start,
        "install.php?op=pages",
        "install.php?op=downloads&start=$new_start&transfer_counter=$transfer_counter",
        66,
        $transfer_counter
    );
}
...
function upgrade_pages($start)
{
    ...
    if ($cat_result->count() > 0) {
        $cat_rows = $cat_result->results();
        foreach ($cat_rows as $cat_row) {
            $pages_cats[$cat_row['imported_id']] = [
                'catid' => $cat_row['catid'],
                'parent_id' => $cat_row['parent_id'],
            ];
        }
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(pid) FROM `" .
            ...
            "`.`" .
            OLD_DB_PREFIX .
            "_pages`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_pages` ORDER BY pid ASC LIMIT $start, $transfer_counter"
    );
    $fetched_rows = intval($result->count());
    ...
    if ($fetched_rows > 0) {
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                ...
                $cat_link = isset($pages_cats[$row['cid']]['catid'])
                    ? $pages_cats[$row['cid']]['catid']
                    : 0;
                ...
                $bodytext =
                    $row['text'] . "<br />" . $row['page_footer'] . "<br />";
    ...
    if (
        ...
        upgrade_header(7, 95);
        echo "
		<form role=\"form\" class=\"form-horizontal\" id=\"nukeform\" action=\"install.php?op=pages" .
            (isset($start) && $start != 0 ? "&start=$start" : "") .
            "\" method=\"post\">
			<div class=\"wizard-card-container\" style=\"height: 326px;\">
				<div class=\"wizard-card\" data-cardname=\"group\">
    ...
    upgrade_progress_output(
        ...
        $start,
        "install.php?op=faqs",
        "install.php?op=pages&start=$new_start&transfer_counter=$transfer_counter",
        72,
        $transfer_counter
    );
}
...
function upgrade_faqs($start)
{
    ...
    $result = $db->query(
        "SELECT *, (SELECT COUNT(id) FROM `" .
            ...
            "`.`" .
            OLD_DB_PREFIX .
            "_faqanswer`) as total_rows FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_faqanswer` ORDER BY id ASC LIMIT $start, $transfer_counter"
    );
    $fetched_rows = intval($result->count());
    ...
    if ($fetched_rows > 0) {
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                ...
                $cat_link = isset($faqs_cats[$row['id_cat']]['catid'])
                    ? $faqs_cats[$row['id_cat']]['catid']
                    : 0;
    ...
    if (
        ...
        upgrade_header(7, 95);
        echo "
		<form role=\"form\" class=\"form-horizontal\" id=\"nukeform\" action=\"install.php?op=faqs" .
            (isset($start) && $start != 0 ? "&start=$start" : "") .
            "\" method=\"post\">
			<div class=\"wizard-card-container\" style=\"height: 326px;\">
				<div class=\"wizard-card\" data-cardname=\"group\">
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=comments",
        "install.php?op=faqs&start=$new_start&transfer_counter=$transfer_counter",
        78,
        $transfer_counter
    );
}
	...
	*/
    $modules_comments = [
        "stories_comments" => [
            ...
            "subquery" =>
                "SELECT sid FROM " .
                POSTS_TABLE .
                " WHERE sid = t.sid AND post_type = 'Articles'",
        ...
        "pages_comments" => [
            ...
            "subquery" =>
                "SELECT sid FROM " .
                POSTS_TABLE .
                " WHERE imported_id = t.pageid AND post_type = 'Pages'",
            "progress_link" =>
                "install.php?op=comments&module=pages_comments&start={NEW_START}",
        ...
        "products_comments" => [
            ...
            "subquery" =>
                "SELECT sid FROM " .
                POSTS_TABLE .
                " WHERE imported_id = t.sid AND post_type = 'Products'",
            "progress_link" =>
                "install.php?op=comments&module=products_comments&start={NEW_START}",
        ...
        "pollcomments" => [
            ...
            "subquery" => "s.pollID",
            "progress_link" =>
                "install.php?op=comments&module=pollcomments&start={NEW_START}",
            "finish_link" =>
                "install.php?op=comments&module=staticpages_comments",
        ],
        "staticpages_comments" => [
            "new_table" => POSTS_TABLE,
            ...
            "subquery" =>
                "SELECT sid FROM " .
                POSTS_TABLE .
                " WHERE imported_id = t.staticid AND post_type = 'Statics'",
            "progress_link" =>
                "install.php?op=comments&module=staticpages_comments&start={NEW_START}",
            "finish_link" => "install.php?op=comments_parents",
        ],
    ];

    $module =
        isset($module) && $module != '' && $module != 'stories_comments'
            ? $module
            : "stories_comments";
    ...
    if (intval($result2->count()) > 0) {
        ...
        foreach ($rows2 as $row2) {
            ...
            $fields[$row2['tid']][$comments_config[$row2['cfid']]] =
                $row2['cfvalue'];
        }
    }
    ...
    $insert_query = [];
    $result = $db->query(
        "SELECT t.*, t.$module_pid as pid, s.$module_title as post_title, ($subquery) as new_post_id, (SELECT COUNT(tid) FROM `" .
            OLD_DB .
            "`.`" .
            OLD_DB_PREFIX .
            "_" .
            ...
            "_" .
            $module_table .
            "` as s ON s.$module_cid = t.$module_id ORDER BY tid ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    ...
    if ($fetched_rows > 0) {
        ...
        if (!empty($all_rows)) {
            foreach ($all_rows as $row) {
                $insert_query[$row['tid']] = [
                    ...
                    'date' => $row['date'],
                    'name' =>
                        isset($fields[$row['tid']]['name']) &&
                        $fields[$row['tid']]['name'] != ''
                            ? $fields[$row['tid']]['name']
                            : $row['name'],
                    'email' =>
                        isset($fields[$row['tid']]['email']) &&
                        $fields[$row['tid']]['email'] != ''
                            ? $fields[$row['tid']]['email']
                            : (isset($row['email'])
                                ? $row['email']
                                : ""),
                    'url' =>
                        isset($fields[$row['tid']]['url']) &&
                        $fields[$row['tid']]['url'] != ''
                            ? $fields[$row['tid']]['url']
                            : $row['url'],
        ...
        $db->query("set names '$pn_dbcharset'");
        if (isset($insert_query) && !empty($insert_query)) {
            $db->table(COMMENTS_TABLE)->multiinsert(
                [
                    "pid",
                    "main_parent",
                    "module",
...
function upgrade_comments_parents($start = 0)
{
    ...
    $update_query = [];
    $result = $db->query(
        "SELECT c.cid, (SELECT cid FROM " .
            COMMENTS_TABLE .
            " WHERE imported_id = c.pid AND module = '$module') as pid, (SELECT COUNT(cid) FROM " .
            COMMENTS_TABLE .
            " WHERE pid != '0' AND module = '$module') as total_rows FROM " .
            COMMENTS_TABLE .
            " as c WHERE c.pid != '0' AND c.module = '$module' ORDER BY cid ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    ...
    if ($fetched_rows > 0) {
        ...
        if (isset($update_query) && !empty($update_query)) {
			...
			" .
                    ...
                    "
			END
			WHERE module = '$module' AND cid IN (" .
                    implode(",", array_keys($update_query)) .
                    ")"
            );
        }
    }
...
function upgrade_comments_main_parent($start = 0)
{
    ...
    $update_query = [];
    $result = $db->query(
        "SELECT cid, pid, (SELECT COUNT(cid) FROM " .
            COMMENTS_TABLE .
            " WHERE pid != '0' AND module = '$module') as total_rows FROM " .
            COMMENTS_TABLE .
            " as c WHERE c.pid != '0' AND c.module = '$module' ORDER BY cid ASC LIMIT $start, $run_per_step"
...
function upgrade_comments_replies_time($start = 0)
{
    ...
    // update nuke_comments
    $result = $db->query(
        "SELECT c.cid, c.`date`, (SELECT `date` FROM " .
            COMMENTS_TABLE .
            " WHERE main_parent = c.cid ORDER BY `date` DESC LIMIT 1) as last_replay_time, (SELECT COUNT(cid) FROM `" .
            COMMENTS_TABLE .
            "` WHERE pid = '0') as total_rows FROM " .
            COMMENTS_TABLE .
            " AS c WHERE c.pid = '0' ORDER BY c.cid ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    $when_query = [];
    if ($fetched_rows > 0) {
        ...
        foreach ($rows as $row) {
            ...
            $date = $row['date'];
            $last_replay_time =
                intval($row['last_replay_time']) > 0
                    ? $row['last_replay_time']
                    : $date;

            $when_query[
                $row['cid']
            ] = "WHEN cid = '$cid' THEN '$last_replay_time'";
        }
    ...
    upgrade_progress_output(
        "اصلاح ترتیب نمایش نظرات",
        $total_rows,
        $fetched_rows,
        $start,
        "install.php?op=forum",
        "install.php?op=comments_replies_time&start=$new_start",
        90,
        $run_per_step
    );
}
...
function upgrade_forum()
{
    ...
    $transfer_users =
        isset($install_options['db_info']['db_have_forum']) &&
        $install_options['db_info']['db_have_forum'] == 2
            ? true
            : false;

    $old_users_table_name = "`" . OLD_DB . "`.`" . OLD_DB_PREFIX . "_users`";
    $new_users_table_name = $have_forum
        ? "`" .
            $install_options['db_info']['db_forumname'] .
            "`.`" .
            ($install_options['db_info']['db_forumprefix'] !=
            OLD_DB_PREFIX . '_bb3'
                ? $install_options['db_info']['db_forumprefix'] . "_users"
                : OLD_DB_PREFIX . "_bb3users") .
            "`"
        : USERS_TABLE;
        ...
        if (!empty($tables_result)) {
            if ($have_forum) {
                ...
                foreach ($tables_result as $row) {
                    ...
                    $query = $db->query(
                        "SHOW CREATE TABLE `" . OLD_DB . "`.`$forumtable`"
                    );
                    ...
                    if (
                        ...
                        $new_forumtable = str_replace(
                            OLD_DB_PREFIX . '_bb3',
                            $install_options['db_info']['db_forumprefix'] . "_",
                            $forumtable
                        );
                    }
                ...
                $users_query = $db->query(
                    "SHOW CREATE TABLE $old_users_table_name"
                );
                $users_query_results = $users_query->results();
                $users_query_result = $users_query_results[0];
                $users_sql = str_replace(
                    "CREATE TABLE `" . OLD_DB_PREFIX . "_users`",
                    "CREATE TABLE $new_users_table_name",
                    $users_query_result['Create Table']
                );
                $users_sql = preg_replace(
                    ["#latin1([a-z0-9A-Z\_+]*)#i", "#utf8([a-z0-9A-Z\_+]*)#i"],
                    "latin1_general_ci",
                    $users_sql
                );
                $users_sql = str_replace(
                ...
                $db->query("set names '$pn_dbcharset'");
                $db->query(
                    "INSERT INTO $new_users_table_name SELECT * FROM $old_users_table_name"
                );
                // update users table
                ...
                $db->query(
                    "ALTER TABLE `" .
                        ...
                        "`.`" .
                        $install_options['db_info']['db_forumprefix'] .
                        "_profile_fields_data` ADD `pf_user_phone` varchar(255) COLLATE utf8_bin NOT NULL DEFAULT ''"
                );
                $db->query(
                    "ALTER TABLE " .
                        $new_users_table_name .
                        " ADD `user_credit` bigint(20) UNSIGNED NOT NULL DEFAULT '0'"
                );
                ...
                $db->query(
                    "UPDATE `" .
                        ...
                        "`.`" .
                        $install_options['db_info']['db_forumprefix'] .
                        "_config` SET config_value = '0' WHERE config_name = 'server_port'"
                );
                ...
                $db->query(
                    "INSERT INTO `" .
                        ...
                        "`.`" .
                        $install_options['db_info']['db_forumprefix'] .
                        "_modules` (`module_id`, `module_enabled`, `module_display`, `module_basename`, `module_class`, `parent_id`, `left_id`, `right_id`, `module_langname`, `module_mode`, `module_auth`) VALUES
				...
				(150, 1, 1, 'logs', 'mcp', 144, 52, 53, 'MCP_LOGS_FORUM_VIEW', 'forum_logs', 'acl_m_,$" .
                        "id'),
				(151, 1, 1, 'logs', 'mcp', 144, 54, 55, 'MCP_LOGS_TOPIC_VIEW', 'topic_logs', 'acl_m_,$" .
                        "id'),
				(152, 1, 1, 'main', 'mcp', 139, 2, 3, 'MCP_MAIN_FRONT', 'front', ''),
				(153, 1, 1, 'main', 'mcp', 139, 4, 5, 'MCP_MAIN_FORUM_VIEW', 'forum_view', 'acl_m_,$" .
                        "id'),
				(154, 1, 1, 'main', 'mcp', 139, 6, 7, 'MCP_MAIN_TOPIC_VIEW', 'topic_view', 'acl_m_,$" .
                        "id'),
				(155, 1, 1, 'main', 'mcp', 139, 8, 9, 'MCP_MAIN_POST_DETAILS', 'post_details', 'acl_m_,$" .
                        "id || (!$" .
                        "id && aclf_m_)'),
				...
				(163, 1, 1, 'queue', 'mcp', 140, 16, 17, 'MCP_QUEUE_APPROVE_DETAILS', 'approve_details', 'acl_m_approve,$" .
                        "id || (!$" .
                        "id && aclf_m_approve)'),
				...
				(166, 1, 1, 'reports', 'mcp', 141, 30, 31, 'MCP_REPORT_DETAILS', 'report_details', 'acl_m_report,$" .
                        "id || (!$" .
                        "id && aclf_m_report)'),
				...
				(170, 1, 1, 'warn', 'mcp', 143, 46, 47, 'MCP_WARN_POST', 'warn_post', 'acl_m_warn && acl_f_read,$" .
                        "id'),
	...
	if ((stristr(" .
                    '$' .
                    "queryString,'%20union%20')) OR (stristr(" .
                    '$' .
                    "queryString,'%2f%2a')) OR (stristr(" .
                    '$' .
                    "queryString,'%2f*')) OR (stristr(" .
                    '$' .
                    "queryString,'/*')) OR (stristr(" .
                    '$' .
                    "queryString,'*/union/*')) OR (stristr(" .
                    '$' .
                    "queryString,'c2nyaxb0')) OR (stristr(" .
                    '$' .
                    "queryString,'+union+'))  OR ((stristr(" .
                    '$' .
                    "queryString,'cmd=')) AND (!stristr(" .
                    '$' .
                    "queryString,'&cmd'))) OR ((stristr(" .
                    '$' .
                    "queryString,'exec')) AND (!stristr(" .
                    '$' .
                    "queryString,'execu'))) OR (stristr(" .
                    '$' .
                    "queryString,'concat'))) {
	...
	?>";
                if (
                    isset($install_options['db_info']['db_forumpath']) &&
                    $install_options['db_info']['db_forumpath'] != '' &&
                    file_exists(
                        $install_options['db_info']['db_forumpath'] .
                            "/config.php"
    ...
    upgrade_progress_output(
        ...
        100,
        0,
        "install.php?op=" . ($transfer_users ? "users" : "final") . "",
        "",
        99,
        1000
    );
...
function upgrade_users($start)
{
    ...
    $db->query("set names 'latin1'");
    $result = $db->query(
        "SELECT *, (SELECT COUNT(user_id) FROM $old_users_table_name) as total_rows FROM $old_users_table_name ORDER BY user_id ASC LIMIT $start, $run_per_step"
    );
    $fetched_rows = intval($result->count());
    if ($fetched_rows > 0) {
        $rows = $result->results();
        ...
        foreach ($rows as $row) {
            ...
            $insert_query[] = [
                ...
                $row['user_lastvisit'],
                $row['user_lastpage'],
                $row['user_inactive_reason'],
                $row['user_inactive_time'],
                $row['user_lang'],
    ...
    upgrade_progress_output(
        ...
        $fetched_rows,
        $start,
        "install.php?op=final",
        "install.php?op=users&start=$new_start",
        99,
        $run_per_step
    );
}
...
function upgrade_final()
{
    ...
    if ($install_options['mode'] == 'install') {
        ...
        $db->table(POSTS_TABLE)->insert([
            ...
            'ip' => $visitor_ip,
            'hometext' => addslashes(
                '<p align="justify">آیا تا به حال به این فکر افتاده اید که برای خود یک وب سایت در دنیای اینترنت داشته باشید یا اینکه برای معرفی شرکت یا محصول خود از طریق اینترنت یک سایت اختصاصی ایجاد کنید.</p><p>مشهدتیم آمادگی خود را برای ارائه سرویس های هاستینگ و ثبت دامنه با مناسب ترین قیمت و بهترین امکانات اعلام میکند . سرویس هاستینگ مشهدتیم با کادر مجرب بصورت 24 ساعته پاسخگوی شما خواهد بود . پس از اجاره فضا اگر شما تمایل به استفاده از نسخه های مختلف php-nuke فارسی را دارید گروه مشهدتیم این سیستم مدیریت محتوا را برای شما بصورت رایگان نصب و تنظیم مینماید ، برای کسب اطلاعات بیشتر به بخش <a href="http://www.phpnuke.ir/index.php?modname=hosting&amp;op=register">میزبانی وب</a> مراجعه فرمائید.</p>'
            ),
            'post_url' => 'مطلب-ابتدایی',
            'alanguage' => '',
            'ihome' => 1,
            'allow_comment' => 1,
            'cat' => 1,
        ...
        // add new blocks
        $default_blocks = [
            [1, 'Articles tags', 'block-Articles_tags.php'],
            [2, 'Languages', 'block-Languages.php'],
            [3, 'Last 5 Articles', 'block-Last_5_Articles.php'],
            [4, 'MT-Forums', 'block-MT-Forums.php'],
            [5, 'MT-ForumsTabed', 'block-MT-ForumsTabed.php'],
            [6, 'Search', 'block-Search.php'],
            [7, 'User Info', 'block-User_Info.php'],
            [8, 'archive', 'block-archive.php'],
            [9, 'comments', 'block-comments.php'],
            [10, 'surveys', 'block-surveys.php'],
            [11, 'Last comments', 'block-Last_comments.php'],
            [12, 'credits', 'block-Credits.php'],
            [13, 'invitation', 'block-invitation.php'],
        ];

        $db->table(BLOCKS_TABLE)->multiinsert(
            ['bid', 'title', 'blockfile'],
            $default_blocks
        );
        ...
        $topcenter_blocks_data = addslashes(
            phpnuke_serialize($topcenter_blocks_data)
        );
        $comments_blocks_data = addslashes(
            phpnuke_serialize($comments_blocks_data)
        );
        ...
        $default_cat = [
            [
                ...
                'Articles',
                'uncategorized',
                'a:2:{s:7:"english";s:13:"uncategorized";s:5:"farsi";s:19:"بدون موضوع";}',
                'uncategorized',
            ],
            [
                2,
        ...
        $db->table(CATEGORIES_TABLE)->multiinsert(
            ['catid', 'type', 'module', 'catname', 'cattext', 'catdesc'],
            $default_cat
        );

        // add new poll
        $db->table(SURVEYS_TABLE)->insert([
            'pollID' => 1,
            'status' => 1,
            'aid' => $install_options['admininfo']['aid'],
            'canVote' => 1,
            ...
            'start_time' => _NOWTIME,
            'options' =>
                'a:4:{i:0;a:2:{i:0;s:8:"عالی";i:1;i:0;}i:1;a:2:{i:0;s:6:"خوب";i:1;i:0;}i:2;a:2:{i:0;s:10:"متوسط";i:1;i:0;}i:3;a:2:{i:0;s:8:"ضعیف";i:1;i:0;}}',
        ]);
        ...
        $db->table(USERS_TABLE)->multiinsert(
            [
                'user_id',
                'group_id',
                'user_groups',
    ...
    $default_groups = [
        [
            ...
            1,
            'GUESTS',
            'a:2:{s:7:"english";s:6:"guests";s:5:"farsi";s:14:"مهمانان";}',
            0,
            '#000000',
        ],
        [
            ...
            1,
            'REGISTERED',
            'a:2:{s:7:"english";s:10:"registered";s:5:"farsi";s:14:"کاربران";}',
            0,
            '#DE21EB',
        ],
        [
            ...
            1,
            'ADMINISTRATORS',
            'a:2:{s:7:"english";s:6:"admins";s:5:"farsi";s:12:"مديران";}',
            0,
            '#FF0000',
        ],
    ];

    $db->table(GROUPS_TABLE)->multiinsert(
        [
            'group_id',
            'group_type',
            'group_name',
    ...
    $modules_data = [
        [
            1,
            'Articles',
            'a:2:{s:7:"english";s:8:"Articles";s:5:"farsi";s:10:"مطالب";}',
            1,
            '0',
            '',
            1,
            1,
            1,
            'a:5:{s:5:"index";s:35:"right|left|topcenter|bottomcenter||";s:4:"more";s:18:"right|||comments||";s:7:"archive";s:10:"right|||||";s:12:"send_article";s:10:"right|||||";s:8:"category";s:10:"right|||||";}',
        ],
        [
            2,
            'Search',
            'a:2:{s:7:"english";s:6:"Search";s:5:"farsi";s:10:"جستجو";}',
            1,
            '0',
            '',
            1,
            0,
            1,
            'a:1:{s:5:"index";s:10:"right|||||";}',
        ],
        [
            3,
            'Surveys',
            'a:2:{s:7:"english";s:7:"Surveys";s:5:"farsi";s:20:"نظر سنجی ها";}',
            1,
            '0',
            '',
            1,
            0,
            1,
            'a:2:{s:5:"index";s:10:"right|||||";s:7:"results";s:10:"right|||||";}',
        ],
        [
            4,
            'Feedback',
            'a:2:{s:7:"english";s:10:"Contact Us";s:5:"farsi";s:18:"تماس با ما";}',
            1,
            '0',
            '',
            1,
        ...
        [
            6,
            'Statistics',
            'a:2:{s:7:"english";s:10:"Statistics";s:5:"farsi";s:21:"آمار بازدید";}',
            1,
            '0',
            '',
            1,
            0,
            1,
            'a:2:{s:5:"index";s:10:"right|||||";s:8:"advanced";s:10:"right|||||";}',
        ],
        [
            8,
            'Feed',
            'a:2:{s:7:"english";s:4:"feed";s:5:"farsi";s:14:"خبرخوان";}',
            1,
            '0',
            '',
            0,
        ...
        [
            9,
            'Users',
            'a:2:{s:7:"english";s:5:"users";s:5:"farsi";s:23:"سیستم کاربری";}',
            0,
            '0',
            '',
            1,
            0,
            0,
            'a:3:{s:12:"login_signup";s:10:"right|||||";s:4:"edit";s:10:"right|||||";s:7:"profile";s:10:"right|||||";}',
        ],
        [
            10,
            'Credits',
            'a:2:{s:7:"english";s:14:"credits system";s:5:"farsi";s:27:"سیستم اعتبارات";}',
            1,
            '0',
            '',
            1,
            0,
            1,
            'a:2:{s:4:"list";s:10:"right|||||";s:4:"form";s:10:"right|||||";}',
        ],
    ];

    $db->table(MODULES_TABLE)->multiinsert(
        [
            'mid',
            'title',
            'lang_titles',
    ...
    $nav_menus_data = [
        [
            ...
            'صفحه اصلی',
            'index.html',
            'a:4:{s:6:"target";s:0:"";s:3:"xfn";s:0:"";s:7:"classes";s:0:"";s:6:"styles";s:0:"";}',
            'custom',
            '',
            0,
        ],
    ...
    $db->table(NAV_MENUS_TABLE)->insert([
        "nav_id" => 1,
        "nav_title" => 'فهرست اصلی',
        "lang_nav_title" =>
            'a:2:{s:7:"english";s:11:"main navbar";s:5:"farsi";s:19:"فهرست سایت";}',
        "nav_location" => 'primary',
        "status" => 1,
        "date" => '1496770474',
    ]);

    $db->table(NAV_MENUS_DATA_TABLE)->multiinsert(
        [
            'nid',
            'status',
            'nav_id',
    ...
    // add configs data
    $install_options['siteinfo']['nukeurl'] = isset(
        $install_options['siteinfo']['nukeurl']
    )
        ? $install_options['siteinfo']['nukeurl']
        : $Req_URL;
    $install_options['siteinfo']['sitename'] = isset(
    ...
    $install_options['db_info']['db_have_forum'] = isset(
        ...
        ? $install_options['db_info']['db_have_forum']
        : 0;
    $install_options['db_info']['db_forumcms'] = isset(
        $install_options['db_info']['db_forumcms']
    )
        ? $install_options['db_info']['db_forumcms']
        : "";
    $install_options['db_info']['db_forumprefix'] = isset(
        $install_options['db_info']['db_forumprefix']
    )
        ? $install_options['db_info']['db_forumprefix']
        : "";
    $install_options['db_info']['db_forumname'] = isset(
        $install_options['db_info']['db_forumname']
    )
        ? $install_options['db_info']['db_forumname']
        : "";
    $install_options['db_info']['db_forumpath'] = isset(
        $install_options['db_info']['db_forumpath']
    )
        ? $install_options['db_info']['db_forumpath']
        : "";
    ...
    $config_data = [
        ...
        "'",
        "WHEN config_name = 'nukeurl' THEN '" .
        $install_options['siteinfo']['nukeurl'] .
        "'",
        "WHEN config_name = 'sitename' THEN '" .
        $install_options['siteinfo']['sitename'] .
        "'",
        "WHEN config_name = 'have_forum' THEN '" .
        $install_options['db_info']['db_have_forum'] .
        "'",
        "WHEN config_name = 'forum_system' THEN '" .
        $install_options['db_info']['db_forumcms'] .
        "'",
        "WHEN config_name = 'forum_prefix' THEN '" .
        $install_options['db_info']['db_forumprefix'] .
        "_'",
        "WHEN config_name = 'forum_db' THEN '" .
        $install_options['db_info']['db_forumname'] .
        "'",
        "WHEN config_name = 'forum_path' THEN '" .
        $install_options['db_info']['db_forumpath'] .
        "'",
        "WHEN config_name = 'forum_collation' THEN '" .
        $install_options['db_info']['db_forumunicode'] .
        "'",
    ];
    ...
    if (
        !rename("install", $random_install_folder_name) ||
        !rename("install.php", "install" . rand(2000, 9999) . ".php.back")
    ) {
        $rename_error =
            "<br />فایل و پوشه install قابل تغییر نام نیستند. لطفاً ابتدا این فایل و پوشه را حذف یا تغییر نام دهید و سپس به بخشهای دیگر بروید";
    }
