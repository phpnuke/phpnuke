...
<head>
    ...
    if (
        file_exists(
            "themes/" . $nuke_configs['ThemeSel'] . "/images/favicon.ico"
        )
    ) {
        $contents .=
	...
	<style>
        ...
        $contents .=
            "	<link rel=\"stylesheet\" href=\"includes/Ajax/jquery/jquery-ui.min.rtl.css\" type=\"text/css\" />";
    }
    $contents .= "
...
function adminheader($has_micrometa)
{
    ...
    $pagetitle = end($hooks->apply_filters("set_page_title", []));
    ...
    if (defined('IS_POPUP')) {
        ...
        $admin_top_menus = [];
        $admin_top_menus = $hooks->apply_filters(
            "admin_top_menus",
            $admin_top_menus
        );
        $admin_top_menus = array_flatten(
            $admin_top_menus,
            0,
            'id',
            'parent_id',
        ...
        $args = [$admin_top_menus, 'id', 'parent_id', 0, $args];
        $top_admin_menus_output .= call_user_func_array(
            [$walker, "walk"],
            $args
        );
...
<head>
	...
	<script src=\"includes/Ajax/jquery/jquery.min.js\" type=\"text/javascript\"></script>";
            if (_DIRECTION == "ltr") {
                $contents .=
                    "<link rel=\"stylesheet\" href=\"admin/template/css/login-ltr.css\" type=\"text/css\" media=\"screen\" title=\"default\" />\n";
            }
            if ($has_micrometa) {
...
<body>";
            ...
            $counter_result = $db->query(
                "SELECT COUNT(cid) AS comments_counts, (SELECT COUNT(fid) FROM " .
                    FEEDBACKS_TABLE .
                    " WHERE replys = '' OR replys IS NULL) AS feedbacks_count FROM " .
                    COMMENTS_TABLE .
                    " WHERE status = '0'"
            );
	...
	<header>
		<div class=\"topnav\">
			...
			<span><i class=\"fa fa-calendar\"></i> " .
                _TODAY .
                " : " .
                nuketimes(_NOWTIME, false, false, false, 3) .
                "</span>
		</div>
		...
		<div class=\"top-counters\">
			<a href=\"" .
                $admin_file .
                ".php?op=comments&status=0\"><div class=\"nukecomment\"><span>" .
                _UNAPPROVED_COMMENTS .
                "</span><strong>$comments_counts</strong></div></a>
			<a href=\"" .
                $admin_file .
                ".php?op=feedbacks\"><div class=\"nukefeedback\"><span>" .
                _FEEDBACKS_NUMBBER .
                "</span><strong>$feedbacks_count</strong></div></a>
		</div>
		<div class=\"clear\"></div>
		<div id=\"main-menu\">
			...
			<ul class=\"botnav\">
				$top_admin_menus_output
				<li class=\"admin_link\"><a href=\"" .
                $admin_file .
                ".php\"><i class=\"fa fa-tachometer\"></i> " .
                _ADMIN_PAGE .
                "</a></li>
				<li class=\"logout_link\"><a href=\"" .
                $admin_file .
                ".php?op=logout\"><i class=\"fa fa-sign-out\"></i> " .
                _ADMIN_LOGOUT .
                "</a></li>
			</ul>
			</div>
...
function pageselector(
    ...
    foreach ($pages as $i) {
        $gotopage .=
            ...
            ($i == $pageNow
                ? ""
                : "href=\"" . str_replace("{PAGE}", "$i", $linkto) . "\"") .
            " class=\"action-pagenum\"> &nbsp;$i</a>\n";
    }
    return $gotopage;
}
...
function admin_pagination(
    ...
    $str_page =
        strpos($link_to, "?") === false ? "?page$pageid" : "&amp;page$pageid";
    ...
    $paginate_links =
        "<div align=\"center\"><table align=\"center\" border=\"0\"><tr><td style=\"border:0\" align=\"center\">" .
        $first .
        $prev .
        $nav .
        $next .
...
function get_latest_info()
{
    ...
    $alerts_messages = [];
    $alerts_messages = $hooks->apply_filters(
    ...
    if (is_array($alerts_messages) && !empty($alerts_messages)) {
        ...
        foreach (
            $alerts_messages
            as $alerts_message_key => $alerts_messages_value
        ) {
            if (!isset($alerts_messages_value['table'])) {
                continue;
            }

            $count_prefix = isset($alerts_messages_value['prefix'])
                ? $alerts_messages_value['prefix']
                : "c$counter_prefix";
            $count_by = isset($alerts_messages_value['by'])
                ? $alerts_messages_value['by']
                : "id";
            $count_table = $alerts_messages_value['table'];
            $count_where =
                isset($alerts_messages_value['where']) &&
                $alerts_messages_value['where'] != ''
                    ? "WHERE " . $alerts_messages_value['where']
                    : "";
    ...
    foreach ($rows as $row_key => $row_value) {
        if ($row_value != 0) {
            ...
            $text = sprintf(
                defined($alerts_messages[$row_key]['text'])
                    ? constant($alerts_messages[$row_key]['text'])
                    : $alerts_messages[$row_key]['text'],
                $row_value
            );
            $contents .= "
	...
	<div class="container">
        ...
        '
					...
					<div class="error-actions">
						<a href="' .
        _GOBACK_CLEAN .
        '" class="btn btn-info btn-lg"><span class="glyphicon glyphicon-share-alt"></span>
							' .
        _GOBACK_TEXT .
        ' </a>
							<a href="' .
        $admin_file .
        '.php" class="btn btn-primary btn-lg"><span class="glyphicon glyphicon-home"></span>
							' .
        _GO_TO_MAIN_PAGE .
        ' </a>
					</div>
