...
if (check_admin_permission($filename)) {
    ...
    function backup_tables($tables = '*', $comments)
    {
        ...
        $filename =
            $folder_name . date('d-m-Y') . '_' . $DB_obj->database . '.sql';
        ...
        for ($count = 0; $count < $table_count; $count++) {
            list($table_name, $row_count, $start) = explode(
                ':',
                str_replace(
                    "\r",
                    "",
        ...
        if (gzcompressfile($filename, 1) == false) {
            $contents .=
                ...
                _BACKUPCREATIONERROR .
                "</b></font></div>";
        } else {
            $contents .=
                "<meta http-equiv=\"refresh\" content=\"1; url=$filename.gz\"><div class=\"text-center\"><font class=\"title\"><b>" .
                sprintf(_BACKUPCREATIONOK, "$filename.gz") .
                "</b></font><br /><br />" .
                _GOBACK .
                "</div>";
            @unlink($filename);
    ...
    function database()
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["database" => _DATABASEMANAGE];
        });
        $contents = '';
        $contents .= GraphicAdmin();
        $contents .= OpenAdminTable();
		...
		<form method=\"post\" name=\"backup\" action=\"" .
            $admin_file .
            ".php\">";
        $contents .=
            "<table width=\"100%\" class=\"id-form product-table no-border database\"><tr>";
		...
		<script>
			$(document).ready(function(){
				$('.multiple-select').multiSelect({
					selectableHeader: \"<div class='custom-header'><a href='#' id='select-all'><b>" .
            _CHECKALL .
            "</b></a> &nbsp; <input type='text' class='search-input' placeholder='search ...' autocomplete='off' dir='ltr'></div>\",
					selectionHeader: \"<div class='custom-header'><a href='#' id='deselect-all'><b>" .
            _UNCHECKALL .
            "</b></a> &nbsp; <input type='text' class='search-input' autocomplete='off' placeholder='search ...' dir='ltr'></div>\",
					afterInit: function(ms){
						var that = this,
						$" .
            "selectableSearch = that.$" .
            "selectableUl.prev().find('input'),
						$" .
            "selectionSearch = that.$" .
            "selectionUl.prev().find('input'),
						selectableSearchString = '#'+that.$" .
            "container.attr('id')+' .ms-elem-selectable:not(.ms-selected)',
						selectionSearchString = '#'+that.$" .
            "container.attr('id')+' .ms-elem-selection.ms-selected';
            ...
            "selectableUl.focus();
						...
						that.qs2 = $" .
            "selectionSearch.quicksearch(selectionSearchString)
						.on('keydown', function(e){
							if (e.which == 40){
								that.$" .
            "selectionUl.focus();
        ...
        $contents .=
            ...
            "</th>
								<td><span dir=\"ltr\">" .
            $nuke_configs['Version_Num'] .
            "</span></td>
								<th>" .
            _INSTALLED_NUKE_PATH .
            "</th>
								<td><span dir=\"ltr\">" .
            PHPNUKE_ROOT_PATH .
            "</span></td>
    ...
    function operation($mode, $tables)
    {
        ...
        $hooks->add_filter("set_page_title", function () use ($type) {
            return [
                "operation" =>
                    _DATABASEMANAGE . " - " . _OPERATION . " " . $type,
            ];
        });
        $contents = '';
        $contents .= GraphicAdmin();
        ...
        $contents .= OpenAdminTable();
        $contents .=
            "<div class=\"text-center\"><font class=\"title\"><b>$type " .
            _TABLES .
            "</b></font></div>";
        $contents .= CloseAdminTable();
        $contents .= "<br>";
            ...
            if ($db->count() > 0) {
                foreach ($result as $row) {
                    ...
                    foreach ($row as $value) {
                        $contents .=
                            "<td align=\"center\" dir=ltr>" . $value . "</td>";
                    }
                    $contents .= "</tr>";
                    $c++;
                }
