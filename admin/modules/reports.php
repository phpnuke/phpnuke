...
if (check_admin_permission($filename)) {
    function reports(
        ...
        $all_modules_reports = [];
        $all_modules_reports = $hooks->apply_filters(
            'modules_have_reports',
            $all_modules_reports
        );

        $all_modules_reports_link[] =
            "<option value=\"" .
            $admin_file .
            ".php?op=reports&module_name=comments" .
            ($post_id != 0 ? "&post_id=$post_id" : "") .
            "\" " .
            ($module_name == 'comments' ? "selected" : "") .
            ">" .
            _COMMENTS .
            "</option>";
        ...
        foreach ($all_modules_reports as $modules_report) {
            ...
            $all_modules_reports_link[] =
                "<option value=\"" .
                $admin_file .
                ".php?op=reports&module_name=$modules_report" .
                ($post_id != 0 ? "&post_id=$post_id" : "") .
                "\" $sel>$modules_report</option>";
        }
        $contents .=
            "<div align=\"center\" style=\"margin:20px 0;\">" .
            _VIEW_REPORTS_IN_MODULE .
        ...
        $where_values = !empty($where_values) ? $where_values : [];
        ...
        $order_by =
            ...
            in_array($order_by, [
                ...
                ? filter($order_by, "nohtml")
                : 'rid';
        $sort_reverse = $sort == 'ASC' ? "DESC" : "ASC";

        if (!isset($post_id) or $post_id == 0) {
            $select2 =
                ", (SELECT COUNT(r3.rid) FROM " .
                REPORTS_TABLE .
                " AS r3" .
                ($where != ""
                    ? " " .
                        str_replace("r.", "r3.", $where) .
                        " AND r3.post_id = r.post_id"
                    : " WHERE r3.post_id = r.post_id") .
                ") as total_post_reports";
        }
        ...
        $result = $db->query(
			...
			u." .
                ...
                " as username,
			(SELECT COUNT(r2.rid) FROM " .
                REPORTS_TABLE .
                " AS r2" .
                ($where != "" ? " " . str_replace("r.", "r2.", $where) : "") .
                ") as total_rows
			$select2 
			FROM " .
                REPORTS_TABLE .
			...
			" .
                ($where != "" ? " $where" : "") .
                "
			ORDER BY r.$order_by $sort LIMIT :start_at,:entries_per_page",
        ...
        $contents .=
			...
			<thead>
			<tr>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:70px;\"><a href=\"" .
            $admin_file .
            ".php?op=reports&order_by=rid&sort=" .
            $sort_reverse .
            "" .
            $link_to_more .
            "\"" .
            ...
            $admin_file .
            ".php?op=reports&order_by=post_title&sort=" .
            $sort_reverse .
            "" .
            $link_to_more .
            "\"" .
            ...
            "</a></th>
				<th class=\"table-header-repeat line-left\"><a href=\"" .
            $admin_file .
            ".php?op=reports&order_by=subject&sort=" .
            $sort_reverse .
            "" .
            $link_to_more .
            "\"" .
            ...
            "</a></th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:70px;\"><a href=\"" .
            $admin_file .
            ".php?op=reports&order_by=module&sort=" .
            $sort_reverse .
            "" .
            $link_to_more .
            "\"" .
            ...
            _MODULE .
            "</a></th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:120px;\"><a href=\"" .
            $admin_file .
            ".php?op=reports&order_by=time&sort=" .
            $sort_reverse .
            "" .
            $link_to_more .
            "\"" .
            ...
            "</a></th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:120px;\"><a href=\"" .
            $admin_file .
            ".php?op=reports&order_by=user_id&sort=" .
            $sort_reverse .
            "" .
            $link_to_more .
            "\"" .
            ...
            _AUTHOR .
            "</a></th>
				<th class=\"table-header-repeat line-left no-padding\" style=\"text-align:center;width:120px;\"><input data-label=\"" .
            _OPERATION .
            "\" type=\"checkbox\" class=\"styled select-all\" data-element=\"#reports\"></th>
			</tr>
			</thead>
			<tbody>";
        ...
        if ($db->count() > 0) {
            foreach ($result as $row) {
                ...
                $module_link_to =
                    "" . $admin_file . ".php?op=reports&module_name=$module";
                ...
                $contents .=
                    ...
                    ($post_link != ''
                        ...
                        : "$post_title ") .
                    "" .
                    (isset($row['total_post_reports']) &&
                    intval($row['total_post_reports']) != 0
                        ? " <a href=\"$module_link_to&post_id=$post_id&post_title=$post_title\">$total_post_reports<a/>"
                        : "") .
                    "</td>
						<td>$subject</td>
                    ...
                    "');\"></a>
							<a href=\"#\" title=\"" .
                    _VIEW .
                    "\" class=\"table-icon icon-7 info-tooltip showindialog\" data-message=\"$message\"></a>
        ...
        $contents .=
            "
				<tr>
					<td colspan=\"7\" align=\"center\"><input type=\"submit\" value=\"" .
            _DELETE_SELECTED .
            "\" onclick=\"return confirm('" .
            _REPORTS_DELETE_CONFIRM .
            "');\" /></td>
				</tr>
			</tbody>
		</table>
