...
if (check_admin_permission($filename)) {
    ...
    function add_remove_blocks()
    {
        ...
        $exept_files = ["", ".", "..", ".htaccess", "index.html"];
        $default_blocks = get_dir_list("blocks", 'files', false, $exept_files);
        $custom_blocks = get_dir_list(
            "themes/" . $nuke_configs['ThemeSel'] . "/blocks",
            'files',
            false,
            $exept_files
        );
        ...
        foreach ($blockslist as $block_name) {
            $is_new = true;
            if (
                isset($nuke_blocks_cacheData['blocks']) &&
                !empty($nuke_blocks_cacheData['blocks'])
            ) {
                foreach (
                    $nuke_blocks_cacheData['blocks']
                    as $bid => $block_data
                ) {
        ...
        if (is_array($query_sets) && !empty($query_sets)) {
            $insert_db = $db
                ->table(BLOCKS_TABLE)
                ->multiinsert(['title', 'blockfile'], $query_sets);
            if ($insert_db) {
                $blocks_changed = true;
    ...
    function updateweight(
        ...
        $mode,
        $new_box_title,
        $new_box_theme_location,
        $new_box_theme_priority,
        $new_box_status
    ) {
        ...
        if ($mode == 'remove') {
            ...
            if ($db->count() > 0) {
                ...
                $db->table(BLOCKS_BOXES_TABLE)->insert([
                    'box_id' => $target_box_id,
                    'box_status' => 1,
                ]);

                $boxes_data[$target_box_id] = [
                    "box_id" => $target_box_id,
                    "box_blocks" => '',
                    "box_blocks_data" => '',
                    "box_status" => 1,
            ...
            $source_box_block_data = [];
            if ($source_box_id != 'all') {
                $source_box_blocks =
                    $boxes_data[$source_box_id]['box_blocks'] != ''
                        ? explode(
                            ",",
                            $boxes_data[$source_box_id]['box_blocks']
                        )
                        : [];
                $source_box_blocks_data =
                    $boxes_data[$source_box_id]['box_blocks_data'] != ''
                        ? phpnuke_unserialize(
                            stripslashes(
                                $boxes_data[$source_box_id]['box_blocks_data']
                            )
                ...
                $source_box_blocks = array_diff($source_box_blocks, [$bid]);
                $source_box_block_data = $source_box_blocks_data[$bid];
                unset($source_box_blocks_data[$bid]);
            }

            $target_box_blocks_data =
                isset($boxes_data[$target_box_id]['box_blocks_data']) &&
                $boxes_data[$target_box_id]['box_blocks_data'] != ''
                    ? phpnuke_unserialize(
                        stripslashes(
                            $boxes_data[$target_box_id]['box_blocks_data']
                        )
            ...
            $target_box_blocks_data[$bid] =
                $source_box_id != 'all' && !empty($source_box_block_data)
                    ? $source_box_block_data
                    : [
                        "title" =>
                            $nuke_blocks_cacheData['blocks'][$bid]['title'],
                        "lang_titles" => '',
                        "blanguage" => '',
                        "weight" => 0,
                        "active" => 1,
            ...
            $target_box_blocks = [];
            $new_target_box_blocks_data = [];
            foreach ($bids_weight as $bid_key => $bid_val) {
                $target_box_blocks[] = $bid_val;
                $new_weight = $bid_key + 1;
                $new_target_box_blocks_data[$bid_val] =
                    $target_box_blocks_data[$bid_val];
            ...
            if ($source_box_id != 'all') {
                ...
                $source_box_blocks_data = !empty($source_box_blocks_data)
                    ? addslashes(phpnuke_serialize($source_box_blocks_data))
                    : '';
            ...
            $target_box_blocks_data = !empty($new_target_box_blocks_data)
                ? addslashes(phpnuke_serialize($new_target_box_blocks_data))
                : '';
            ...
            $box_blocks_data =
                isset($row['box_blocks_data']) && $row['box_blocks_data'] != ''
                    ? phpnuke_unserialize(stripslashes($row['box_blocks_data']))
                    : [];
            ...
            $box_blocks_data = !empty($new_box_blocks_data)
                ? addslashes(phpnuke_serialize($new_box_blocks_data))
                : '';
    ...
    function BlocksAdmin()
    {
        global $db,
            ...
            $admin_file,
            $nuke_configs,
            $nuke_blocks_cacheData,
            $nuke_headlines_cacheData,
            $theme_setup;

        $hooks->add_filter("set_page_title", function () {
            return ["BlocksAdmin" => _BLOCKSADMIN];
        });
        $contents = '';
        ...
        $theme_widgets = isset($theme_setup['theme_widgets'])
            ? $theme_setup['theme_widgets']
            : [];
        ...
        $all_installed_blocks = [];

        foreach (
            $nuke_blocks_cacheData['blocks_boxes']
            as $box_id => $box_data
        ) {
            if (isset($box_data['blocks'])) {
                foreach ($box_data['blocks'] as $bid => $block_data) {
                    $all_installed_blocks = array_merge($all_installed_blocks, [
                        $bid,
                    ]);
		...
		<div id=\"blocks-boxes\">
            ...
            _ADD_BOX .
            " &nbsp; 
								<input type=\"text\" class=\"inp-form\" id=\"new-box-name\" placeholder=\"" .
            _ENTER_BOXID .
            "\" /> 
            ...
            "
							...
							<td width=\"340\">
								<a href=\"#\" id=\"add-html-block\">" .
            sprintf(_ADD_SP_BLOCK, "Html") .
            "</a>
								<a href=\"#\" id=\"add-rss-block\">" .
            sprintf(_ADD_SP_BLOCK, "Rss") .
            "</a>
							</td>
						</tr>
						<tr>
							<td colspan=\"2\" id=\"blocks-help\">
								...
								<b>راهنما</b>
								<ul>
									<li><span class=\"color-box\" style=\"background:#ccc;\"></span><span class=\"help-text\">" .
            _NOT_INSTALLED .
            " ($not_installed_blocks " .
            _BLOCK .
            ")</span></li>
									<li><span class=\"color-box\" style=\"background:#fff;\"></span><span class=\"help-text\">" .
            _INSTALLED_AND_ACTIVE .
            " ($active_blocks " .
            _BLOCK .
            ")</span></li>
									<li><span class=\"color-box\" style=\"background:#fceded;\"></span><span class=\"help-text\"> " .
            _INSTALLED_AND_INACTIVE .
            " ($inactive_blocks " .
            _BLOCK .
            ")</span></li>
									<li><span class=\"color-box\" style=\"background:#fcf6d8;\"></span><span class=\"help-text\">" .
            _ACTIVE_FUTURE_EXPIRE .
            " ($in_expire_blocks " .
            _BLOCK .
            ")</span></li>
									<li><span class=\"color-box\" style=\"background:#e2f8fc;\"></span><span class=\"help-text\">" .
            _INACTIVE_FUTURE_PUBLISH .
            " ($future_blocks " .
            _BLOCK .
            ")</span></li>
								</ul>								
            ...
            "
						<span class=\"blocks-box-tools\">
							<a class=\"toggle-box table-icon icon-11 info-tooltip\" href=\"#\" title=\"" .
            _SHOW_HIDE .
            "\"></a>
						</span>
					</div>
        ...
        foreach ($nuke_blocks_cacheData['blocks'] as $bid => $block_data) {
            ...
            $contents .=
                "<li class=\"blocks-draggable-item blocks-sortable-item\" id=\"block-all-$bid\" data-bid=\"$bid\">
								<span class=\"block-title\">$title</span>
								<span class=\"blocks-tools\">
									<a class=\"infblock-block table-icon icon-15 info-tooltip\" href=\"#\" title=\"" .
                _INFO .
                "\"></a>
									<a class=\"preview-block table-icon icon-7 info-tooltip\" href=\"#\" title=\"" .
                _SHOW .
                "\"></a>
									<a class=\"remove-block table-icon icon-2 info-tooltip\" href=\"#\" title=\"" .
                _DELETE .
                "\"></a>
									<a class=\"edit-block table-icon icon-6 info-tooltip\" href=\"#\" title=\"" .
                _EDIT .
                "\"></a>
									<a class=\"active-block table-icon icon-13 info-tooltip\" href=\"#\" title=\"" .
                _DEACTIVATE .
                "\"></a>
								</span>
							</li>\n";
        ...
        foreach ($primary_boxes as $primary_box) {
            if (!isset($nuke_blocks_cacheData['blocks_boxes'][$primary_box])) {
                $nuke_blocks_cacheData['blocks_boxes'][$primary_box] = [];
            }
        }
        ...
        foreach (
            ...
            $box_data['box_theme_location'] = isset(
                $box_data['box_theme_location']
            )
                ? $box_data['box_theme_location']
                : "";
            $box_data['box_theme_priority'] = isset(
                $box_data['box_theme_priority']
            )
                ? $box_data['box_theme_priority']
                : "";

            if ($i % 3 == 0) {
                $class_name = 'blocks-column fl first clearer';
            } else {
                $class_name = 'blocks-column fl';
            }

            $contents .=
                "<div class=\"$class_name\" data-box_id=\"$box_id\" data-theme-location=\"" .
                $box_data['box_theme_location'] .
                "\" data-theme-priority=\"" .
                $box_data['box_theme_priority'] .
                "\">
                ...
                _BOXNAME .
                " : <b>" .
                str_replace("_", " ", $box_id) .
                "</b></span>
							<span class=\"blocks-box-tools\">
								<a class=\"remove-box table-icon icon-2 info-tooltip\" href=\"#\" title=\"" .
                _DELETE .
                "\"></a>
								<a class=\"edit-box table-icon icon-6 info-tooltip\" href=\"#\" title=\"" .
                _EDIT .
                "\"></a>
								<a class=\"toggle-box table-icon icon-11 info-tooltip\" href=\"#\" title=\"" .
                _SHOW_HIDE .
            ...
            if (isset($box_data['blocks']) && !empty($box_data['blocks'])) {
                foreach ($box_data['blocks'] as $bid => $block_data) {
                    $title = filter($block_data['title'], "nohtml");
                    $lang_titles =
                        $block_data['lang_titles'] != ""
                            ? phpnuke_unserialize(
                                stripslashes($block_data['lang_titles'])
                            )
                            : "";
                    if (
                        $lang_titles != "" &&
                        $nuke_configs['multilingual'] == 1
                    ) {
                        $title = filter(
                            $lang_titles[$nuke_configs['currentlang']],
                            "nohtml"
                        );
                    }

                    $active_class =
                        $block_data['active'] == 1 ? "" : " deactivated";

                    $active_class .=
                        $block_data['expire'] != 0 ? " in-expire" : "";

                    $active_class .=
                        $block_data['publish'] != 0 ? " future" : "";

                    $active_icon_class =
                        $block_data['active'] == 1 ? "13" : "5";

                    $active_title =
                        $block_data['active'] == 1 ? _DEACTIVATE : _ACTIVATE;

                    $contents .=
                        "<li class=\"blocks-sortable-item blocks-sortable-item-inbox$active_class\" id=\"block-$box_id-$bid\" data-bid=\"$bid\">
										<span class=\"block-title\">$title</span>
										<span class=\"blocks-tools\">
											<a class=\"infblock-block table-icon icon-15 info-tooltip\" href=\"#\" title=\"" .
                        _INFO .
			...
			$(document).ready(function(){ 
				$(function() {
					$(\"#contentLeft table tbody\").sortable({ opacity: 0.6, cursor: 'move', update: function() {
						var order = 'csrf_token=" .
            _PN_CSRF_TOKEN .
            "\"action=updateRecordsListings&' + $(this).sortable(\"serialize\");
						$.post(\"" .
            $admin_file .
            ".php?op=updateweight\", order, function(theResponse){
    ...
    function remove_block($box_id, $bid)
    {
        ...
        if ($box_id != "all") {
            ...
            $box_blocks =
                isset($row['box_blocks']) && $row['box_blocks'] != ''
            ...
            if (
                $nuke_blocks_cacheData['blocks'][$bid]['content'] != '' ||
                $nuke_blocks_cacheData['blocks'][$bid]['url'] != ''
            ) {
            ...
            if (in_array($bid, $box_blocks)) {
                $box_blocks = array_diff($box_blocks, [$bid]);
            }
    ...
    function change_block_status($box_id, $bid)
    {
        ...
        if ($result) {
            ...
            $response = json_encode([
                "status" => "success",
                "message" => sprintf(
                    _BLOCK_ACT_DEACT_MSG,
                    $new_status == 1 ? _ACTIVE : _INACTIVE
                ),
    ...
    function info_block($box_id, $bid)
    {
        ...
        $block_permissions =
            $row['permissions'] != 0 && $row['permissions'] != ''
        ...
        $lang_titles =
            $row['lang_titles'] != ""
                ? phpnuke_unserialize(stripslashes($row['lang_titles']))
                : "";
        ...
        $title .=
            $nuke_configs['multilingual'] == 1 &&
            isset($lang_titles[$nuke_configs['currentlang']])
                ? " <u>" . $lang_titles[$nuke_configs['currentlang']] . "</u>"
                : '';
        ...
        if ($nuke_configs['multilingual'] != 1) {
            ...
            foreach ($languageslists as $languageslist) {
                if ($languageslist != "") {
                    if (
                        $languageslist == 'index.html' ||
                        $languageslist == '.htaccess' ||
                        $languageslist == 'alphabets.php'
                    ) {
                        continue;
                    }
                    ...
                    $contents .=
                        ...
                        _TITLE_INLANG .
                        " : " .
                        ucfirst($languageslist) .
                        "</th>
							<td id=\"edit-block-lang_titles\">
								" .
                        (isset($lang_titles[$languageslist])
                            ? $lang_titles[$languageslist]
                            : "") .
                        "
							</td>
						</tr>";
        ...
        if ($publish != 0) {
            ...
            $contents .=
                "$publish_date " .
                (intval($publish_hour) != 0 && intval($publish_min) != 0
                    ? "" . _HOUR . " $publish_hour:$publish_min"
                    : "") .
                "</td>
			</tr>";
        }
        if ($expire != 0) {
            ...
            $contents .=
                "$expire_date " .
                (intval($expire_hour) != 0 && intval($expire_min) != 0
                    ? "" . _HOUR . " $expire_hour:$expire_min"
                    : "") .
                "</td>
			</tr>";
        }
    ...
    function BlocksEdit($box_id, $bid, $block_type, $all_box_ids)
    {
        ...
        $block_type =
            isset($block_type) && $block_type != '' ? $block_type : '';
        ...
        if (!$new_block) {
            $bid = intval($bid);
            $row =
                $nuke_blocks_cacheData['blocks_boxes'][$box_id]['blocks'][$bid];
        }
        ...
        $blockcontent =
            $new_block && ($block_type == 'html' || $block_type == 'rss')
                ? ""
                : $row['content'];
        ...
        $contents .=
            "<div class=\"text-center\"><font class=\"option\"><b>" .
            ($new_block ? _ADD : _EDIT) .
            " " .
            _BLOCK .
            ": " .
            ($new_block ? $block_type : "$title") .
            " $type</b></font></div><br /><br />";
        $contents .= "
        ...
        if ($new_block) {
            ...
            foreach ($all_box_ids as $this_box_id) {
                $contents .=
                    "<option value=\"$this_box_id\">" .
                    str_replace("_", " ", ucfirst($this_box_id)) .
                    "</option>";
            }
            $contents .= "
						</select>
        ...
        if ($nuke_configs['multilingual'] != 1) {
            ...
            foreach ($languageslists as $languageslist) {
                if ($languageslist != "") {
                    ...
                    $contents .=
                        ...
                        "</th>
							<td id=\"edit-block-lang_titles\">
								<input class=\"inp-form\" type=\"text\" data-language=\"$languageslist\" size=\"30\" maxlength=\"60\" value=\"" .
                        (isset($lang_titles[$languageslist])
                            ? $lang_titles[$languageslist]
                            : "") .
                        "\">
            ...
            $contents .=
                "<input type=\"hidden\" name=\"title\" id=\"edit-block-title\" value=\"\">";
        }
        ...
        if (is_dir("themes/" . $nuke_configs['ThemeSel'] . "/blocks/themes/")) {
            $contents .=
                ...
                "</option>";
            $langdir = dir(
                "themes/" . $nuke_configs['ThemeSel'] . "/blocks/themes/"
            );
            ...
            if (isset($block_list) && !empty($block_list)) {
                ...
                foreach ($block_list as $block_template) {
                    $selected =
                        $theme_block == $block_template && !$new_block
                            ? "selected"
                            : "";
                    $contents .=
                        "<option value=\"$block_template\" $selected>" .
                        ucfirst($block_template) .
                        "</option>";
                }
            }
            $contents .= "</select>
        ...
        if ($nuke_configs['multilingual'] == 1) {
            ...
            foreach ($languageslists as $languageslist) {
                if ($languageslist != "") {
                    ...
                    $languageslist = str_replace(".php", "", $languageslist);
                    $selected =
                        $languageslist == $blanguage && !$new_block
                            ? "selected"
                            : "";
                    $contents .=
                        "<option value=\"" .
            ...
            $contents .=
                "<input type=\"hidden\" id=\"edit-block-language\" value=\"\">";
        }
        ...
        $contents .=
            "<input data-label=\"" .
            _YES .
            "\" type=\"radio\" class=\"styled\" name=\"active\" id=\"edit-block-activeated\" value=\"1\" $sel1>&nbsp;&nbsp;&nbsp;
					<input data-label=\"" .
            _NO .
            "\" type=\"radio\" class=\"styled\" name=\"active\" id=\"edit-block-deactiveated\" value=\"0\" $sel2>&nbsp;&nbsp;&nbsp;
        ...
        $publish_date =
            $publish != 0 ? nuketimes($publish, false, false, false, 1) : 0;
        ...
        $contents .=
            "</select>
					" .
            bubble_show(
                "<div style=\"margin-top:-7px;\"><span style=\"float:right;margin-top:7px\">" .
                    _PUBLISH_AT_FUTURE .
                    "</span> <input data-label=\"" .
                    _YES .
                    "\" type=\"checkbox\" class=\"styled\" name=\"active\" id=\"edit-block-publish-check\" value=\"1\" $publish_chk /></div>"
            ) .
            "
				</td>
			</tr>
        ...
        $expire_date =
            $expire != 0 ? nuketimes($expire, false, false, false, 1) : 0;
        ...
        $contents .=
            "
						<option name=\"action\" value=\"d\" $selected1>" .
            _DEACTIVATE .
            "</option>
						<option name=\"action\" value=\"r\" $selected2>" .
            _DELETE .
            "</option>
					</select>
				</td>
        ...
        $contents .=
			...
			<tr>
				<td colspan=\"2\"><input type=\"button\" class=\"form-submit\" value=\"" .
            _SAVEBLOCK .
            "\" id=\"edit-form-submit\"> <div id=\"edit-loading\" class=\"loading\" style=\"display:none;\"></div></form><div class=\"clear\"></td>
			</tr>
		...
		$(\"#edit-form-submit\").click(function(){
			$(\"#edit-loading\").html('" .
            _PLEASE_WAIT .
            "').css({'display':'inline-block', 'padding':'0 40px', 'width':'180px', 'background-position':'" .
            _TEXTALIGN1 .
            "'});
            ...
            ".php',
				...
				success: function(theResponse){
					...
					if(new_block)
					{
						...
						var active_title = (block_fields.edited_active == 1) ? '" .
            _DEACTIVATE .
            "':'" .
            _ACTIVATE .
            "';
						...
						new_block_content += '						<a class=\"infblock-block table-icon icon-15 info-tooltip\" href=\"#\" title=\"" .
            _INFO .
            "\"></a>';
						new_block_content += '						<a class=\"preview-block table-icon icon-7 info-tooltip\" href=\"#\" title=\"" .
            _SHOW .
            "\"></a>';
						new_block_content += '						<a class=\"remove-block table-icon icon-2 info-tooltip\" href=\"#\" title=\"" .
            _DELETE .
            "\"></a>';
						new_block_content += '						<a class=\"edit-block table-icon icon-6 info-tooltip\" href=\"#\" title=\"" .
            _EDIT .
            "\"></a>';
					...
					else
					{
						...
						if($(\"#edit-block-activeated\").is(':checked'))
						{
							...
							lis.find('.active-block').tooltipText = '" .
            _DEACTIVATE .
            "';
						}
						else
    ...
    function BlocksSave($block_fields, $box_id, $bid, $new_block)
    {
        ...
        if (
            $block_fields['edited_title'] == "" ||
            $block_fields['edited_title'] == "undefined"
        ) {
            if (
                isset(
                    $nuke_blocks_cacheData['blocks_boxes'][$box_id]['blocks'][
                        $bid
                    ]['blockfile']
                ) &&
                $nuke_blocks_cacheData['blocks_boxes'][$box_id]['blocks'][$bid][
                ...
                $block_query_set['title'] = $box_block_query_set[
                    'title'
                ] = @str_replace(
                    ["block-", ".php", "_"],
                    ["", "", " "],
        ...
        if ($nuke_configs['multilingual'] == 1) {
            $box_block_query_set['lang_titles'] = phpnuke_serialize(
                $block_fields['edited_lang_titles']
            );
        } else {
        ...
        if (
            $block_fields['edited_rss'] != '' &&
            $block_fields['edited_rss'] != "undefined"
        ) {
            $block_fields['edited_url'] =
                $nuke_headlines_cacheData[$block_fields['edited_rss']][
                    'headlinesurl'
                ];
        }

        $block_fields['edited_contents'] =
            isset($block_fields['edited_contents']) &&
            $block_fields['edited_contents'] != ''
                ? base64_decode($block_fields['edited_contents'])
                : "";

        if (
            $block_fields['edited_url'] != '' &&
            $block_fields['edited_url'] != "undefined"
        ) {
            if (!@preg_match("#http:\/\/#", $block_fields['edited_url'])) {
                $block_fields['edited_url'] =
                    "http://" . $block_fields['edited_url'];
            }
            ...
            if (is_array($items) && !empty($items)) {
                $block_fields['edited_contents'] =
                    "<ul class=\"rss_block_content\" id=\"rss_block_" .
                    $bid .
                    ">";
            ...
            $block_fields['edited_contents'] == '' ||
            $block_fields['edited_contents'] == "undefined"
        ) {
            $block_fields['edited_contents'] = '';
        }

        $block_query_set['content'] = addslashes(
            $block_fields['edited_contents']
        );
        ...
        if (
            $block_fields['edited_expire'] != 0 &&
            $block_fields['edited_expire'] != 'undefined'
        ) {
            $block_fields['edited_expire'] = to_mktime(
                $block_fields['edited_expire'],
                $block_fields['edited_expire_hour'] .
                    ":" .
                    $block_fields['edited_expire_min'] .
                    ":0"
            );
            $box_block_query_set['expire'] = $block_fields['edited_expire'];
            $box_block_query_set['action'] =
                $block_fields['edited_expire_action'];
        }

        if (
            $block_fields['edited_publish'] != 0 &&
            $block_fields['edited_publish'] != 'undefined'
        ) {
            $block_fields['edited_publish'] = to_mktime(
                $block_fields['edited_publish'],
                $block_fields['edited_publish_hour'] .
                    ":" .
                    $block_fields['edited_publish_min'] .
                    ":0"
            );
        ...
        if (
            $block_fields['edited_block_refresh'] != '' &&
            $block_fields['edited_block_refresh'] != 'undefined'
        ) {
            $block_query_set['refresh'] = $block_fields['edited_block_refresh'];
        }
        if (
            $block_fields['edited_permissions'] != '' &&
            $block_fields['edited_permissions'] != 'undefined'
        ) {
            $box_block_query_set['permissions'] =
                $block_fields['edited_permissions'];
        }
        ...
        if ($new_block && !empty($block_query_set)) {
            $result1 = $db->table(BLOCKS_TABLE)->insert($block_query_set);
        ...
        $box_blocks_data[$bid] = [
            "title" => isset($box_block_query_set['title'])
                ? $box_block_query_set['title']
                : '',
            "lang_titles" => isset($box_block_query_set['lang_titles'])
                ? $box_block_query_set['lang_titles']
                : '',
            "blanguage" => isset($box_block_query_set['blanguage'])
                ? $box_block_query_set['blanguage']
                : '',
            "weight" => sizeof($box_blocks_data) + 1,
            "active" => isset($box_block_query_set['active'])
                ? $box_block_query_set['active']
                : 1,
            "time" => isset($box_block_query_set['time'])
                ? $box_block_query_set['time']
                : _NOWTIME,
            "permissions" => isset($box_block_query_set['permissions'])
                ? $box_block_query_set['permissions']
                : 0,
            "publish" => isset($box_block_query_set['publish'])
                ? $box_block_query_set['publish']
                : 0,
            "expire" => isset($box_block_query_set['expire'])
                ? $box_block_query_set['expire']
                : 0,
            "action" => isset($box_block_query_set['action'])
                ? $box_block_query_set['action']
                : '',
            "theme_block" => isset($box_block_query_set['theme_block'])
                ? $box_block_query_set['theme_block']
                : '',
        ];
        ...
        if ($result1 && $result2) {
            ...
            $lang_titles = isset($box_block_query_set['lang_titles'])
                ? phpnuke_unserialize(
                    stripslashes($box_block_query_set['lang_titles'])
                )
                : '';
            $title = isset($box_block_query_set['lang_titles'])
                ? $box_block_query_set['title']
                : $block_query_set['title'];
            ...
            $response = json_encode([
                ...
                "new_bid" => $bid,
                "new_title" =>
                    $nuke_configs['multilingual'] == 1 && $lang_titles != ''
                        ? $lang_titles[$nuke_configs['currentlang']]
                        : $title,
                "new_expire" =>
                    isset($box_block_query_set['expire']) &&
                    $box_block_query_set['expire'] != 0
                        ? nuketimes(
                            $block_fields['edited_expire'],
                            true,
                            true,
    ...
    function HeadlinesAdmin()
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["HeadlinesAdmin" => _HEADLINESADMIN];
        });
        $contents = '';
        $contents .= GraphicAdmin();
        $contents .= OpenAdminTable();
        ...
        foreach ($nuke_headlines_cacheData as $hid => $nuke_headlines_info) {
            ...
            $contents .=
                ...
                $admin_file .
                ".php?op=HeadlinesEdit&amp;hid=$hid\" class=\"table-icon icon-1 info-tooltip\" title=\"" .
                _EDIT .
                "\"></a> <a class=\"table-icon icon-2 info-tooltip\" href=\"" .
                $admin_file .
                ".php?op=HeadlinesDel&amp;hid=$hid&ok=1&csrf_token=" .
                _PN_CSRF_TOKEN .
                "\" onclick=\"return confirm('" .
                _CONFIRM_HEADLINE_DELETE .
                "');\" title=\"" .
                _DELETE .
                "\"></a></td><tr>";
        }
        ...
        $contents .=
            ...
            _SITENAME .
            ":</th><td><input type=\"text\" class=\"inp-form\" name=\"xsitename\" size=\"31\" maxlength=\"30\"></td></tr>" .
            "<tr><th>" .
            _RSSFILE .
            ":</th><td><input type=\"text\" class=\"inp-form\" name=\"headlinesurl\" size=\"50\" maxlength=\"200\"></td></tr>" .
            "<tr><td><input class=\"form-submit\" type=\"submit\" value=\"" .
            _ADD .
            "\"></td></tr></table>" .
            "<input type=\"hidden\" name=\"op\" value=\"HeadlinesAdd\"><input type=\"hidden\" name=\"csrf_token\" value=\"" .
            _PN_CSRF_TOKEN .
            "\" /> " .
            "</form>";
        $contents .= CloseAdminTable();
    ...
    function HeadlinesAdd($xsitename, $headlinesurl)
    {
        ...
        $db->table(HEADLINES_TABLE)->insert([
            'sitename' => $xsitename,
    ...
    function HeadlinesEdit($hid)
    {
        ...
        $xsitename = filter(
            $nuke_headlines_cacheData[$hid]['sitename'],
            "nohtml"
        );
        $headlinesurl = filter(
            $nuke_headlines_cacheData[$hid]['headlinesurl'],
            "nohtml"
        );

        $hooks->add_filter("set_page_title", function () use ($xsitename) {
            return ["HeadlinesEdit" => sprintf(_EDITHEADLINE_OK, $xsitename)];
        });
        $contents = '';
        $contents .= GraphicAdmin();
        $contents .= OpenAdminTable();
        $contents .=
            "<br /><br /><div class=\"text-center\"><font class=\"title\"><b>" .
            _HEADLINESADMIN .
            "</b></font></div>";
        $contents .= CloseAdminTable();
        $contents .= "<br /><br />";
	...
	<tr><th>" .
            _SITENAME .
            ":</th><td><input class=\"inp-form\" type=\"text\" name=\"xsitename\" size=\"31\" maxlength=\"30\" value=\"$xsitename\"></td></tr>
	<tr><td>" .
            _RSSFILE .
            ":</td><td><input class=\"inp-form\" type=\"text\" name=\"headlinesurl\" size=\"50\" maxlength=\"200\" value=\"$headlinesurl\"></td></tr><tr><td><input class=\"form-submit\" type=\"submit\" value=\"" .
            _SAVECHANGES .
            "\"></td></tr></table>
    ...
    $new_box_title = isset($new_box_title)
        ? filter($new_box_title, "nohtml")
        : '';
    $new_box_theme_location = isset($new_box_theme_location)
        ? filter($new_box_theme_location, "nohtml")
        : '';
    $new_box_theme_priority = isset($new_box_theme_priority)
        ? filter($new_box_theme_priority, "nohtml")
        : '';
    $new_box_status = isset($new_box_status)
        ? filter($new_box_status, "nohtml")
        : '';
    $new_block = isset($new_block) ? $new_block : false;
    $all_box_ids = isset($all_box_ids) ? $all_box_ids : [];
    $block_fields = isset($block_fields) ? $block_fields : [];
    $bids_weight = isset($bids_weight) ? $bids_weight : [];
