...
if (check_admin_permission($filename)) {
    function updatebookmarksweight($bookmarks_ids)
    {
        ...
        foreach ($bookmarks_ids as $key => $recordIDValue) {
            ...
            $bookmarks_query[] =
                "WHEN bid = '$recordIDValue' THEN '" . $listingCounter . "'";
            $bids[] = $recordIDValue;
            $listingCounter++;
        }
        //$bids = implode(",", $bids);
        ...
        $result = $db->query(
            "UPDATE " .
                BOOKMARKSITE_TABLE .
                " SET weight = CASE $bookmarks_query END WHERE bid IN (" .
                implode(",", $bids) .
                ")"
        );
    ...
    function bookmarks()
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["HeadlinesEdit" => _BOOKMARKS_ADMIN];
        });
        $contents = '';
		...
		<div align=\"center\"><br /><br /><br /><br />
            ...
            _ACTIVE .
            "</th>
						<td><label for=\"active-bookmark\">" .
            _YES .
            "</label><input name=\"active\" checked type=\"radio\" class=\"styled\" value=\"1\" id=\"active-bookmark\"> &nbsp;&nbsp; <label for=\"inactive-bookmark\">" .
            _NO .
            "</label><input name=\"active\" type=\"radio\" class=\"styled\" value=\"0\" id=\"inactive-bookmark\"></td>
					</tr>
					<tr>
				...
				<input type=\"hidden\" name=\"op\" value=\"save_bookmarks\"><input type=\"hidden\" name=\"csrf_token\" value=\"" .
            _PN_CSRF_TOKEN .
            "\" /> </form>
			</div>
            ...
            "</a></th>
				...
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:120px;\"><a>" .
            _OPERATION .
            "</a></th>
			</tr>
			</thead>
			<tbody>";
        $nuke_bookmarksite_cacheData_by_weight = array_sort_by_values(
            $nuke_bookmarksite_cacheData,
            'bid',
            'weight',
            $mode = "asc"
        ...
        foreach (
            $nuke_bookmarksite_cacheData_by_weight
            as $weight => $nuke_bookmarksite_info
        ) {
            ...
            $contents .=
                "<tr id=\"bookmark_$bid\">
					...
					<td><a href=\"" .
                $admin_file .
                ".php?op=editbookmark&bid=$bid\" title=\"" .
                _EDIT .
                "\" class=\"table-icon icon-1 info-tooltip\"></a> <a href=\"" .
                $admin_file .
                ".php?op=save_bookmarks&delete=1&bid=$bid&csrf_token=" .
                _PN_CSRF_TOKEN .
                "\" title=\"" .
                _DELETE .
                "\" class=\"table-icon icon-2 info-tooltip\"></a></td>
					</tr>";
        }
        $contents .=
            "
    ...
    function editbookmark($bid)
    {
        ...
        $hooks->add_filter("set_page_title", function () use ($title) {
            return ["HeadlinesEdit" => sprintf(_BOOKMARK_EDIT, $title)];
        });
        ...
        $contents .=
            ...
            "</th>
					<td><label for=\"active-bookmark\">" .
            _YES .
            "</label><input name=\"active\" checked type=\"radio\" class=\"styled\" value=\"1\" id=\"active-bookmark\" $sel1> &nbsp;&nbsp; <label for=\"inactive-bookmark\">" .
            _NO .
            "</label><input name=\"active\" type=\"radio\" class=\"styled\" value=\"0\" id=\"inactive-bookmark\" $sel2></td>
				</tr>
				<tr>
					<td colspan=\"2\" align=\"center\">
			...
			</table><input type=\"hidden\" name=\"op\" value=\"save_bookmarks\"><input type=\"hidden\" name=\"bid\" value=\"$bid\"><input type=\"hidden\" name=\"csrf_token\" value=\"" .
            _PN_CSRF_TOKEN .
            "\" /> </form></div>
		</table>";
    ...
    function save_bookmarks(
        ...
        if ($op == 'insert') {
            $result1 = $db->query(
                "SELECT MAX(weight) as last_weight FROM " .
                    BOOKMARKSITE_TABLE .
                    ""
            );
            ...
            $result = $db->table(BOOKMARKSITE_TABLE)->insert([
                'title' => $title,
                'iconpath' => $iconpath,
                'active' => $active,
                'url' => $url,
