...
if (check_admin_permission($filename)) {
    ...
    function common()
    {
		...
		<p align=\"center\">
            ...
            $admin_file .
            ".php?op=language\">" .
            _LANGUAGE_PHRASES_ADMIN .
            "</a>";
        if ($op != 'edit_language_word') {
            $contents .=
                "| <a href=\"#\" id=\"add_lang_button\">" .
                _ADD_PHRASE .
                "</a>";
        }
        $contents .= " ]
    ...
    function language($new_word = "", $new_word_equals = "", $word_search = "")
    {
        ...
        $pagetitle .=
            isset($word_search) && $word_search != ""
                ? " - " . _PHRASE_SEARCH . " » ($word_search)"
                : "";
        ...
        if (
            isset($new_word) &&
            $new_word != "" &&
            isset($new_word_equals) &&
            is_array($new_word_equals) &&
            !empty($new_word_equals)
        ) {
            if (!array_key_exists($new_word, $nuke_languages_cacheData)) {
                $db->table(LANGUAGES_TABLE)->insert([
                    'main_word' => $new_word,
                ...
                header(
                    "location: " .
                        $admin_file .
                        ".php?op=language&new_word=$new_word"
                );
            }
            die();
        }
        ...
        if (isset($new_word) && $new_word != "") {
            $contents .=
                "<br /><br /><p align=\"center\"><b><a href=\"" .
                $admin_file .
                ".php?op=edit_language_word&word=$new_word\">" .
                _PHRASE_EXISTS .
                "</a></b></p><br /><br />";
        }
        $contents .=
        ...
        foreach ($languageslists as $languageslist) {
            if ($languageslist != "") {
                ...
                $contents .=
                    ...
                    (isset($nuke_languages[$languageslist]['_DIRECTION']) &&
                    $nuke_languages[$languageslist]['_DIRECTION'] == 'ltr'
                        ? " dleft"
                        : "") .
                    "\" type=\"text\" size=\"70\" name=\"new_word_equals[$languageslist]\" />
						</td>
					</tr>";
            }
        }
		...
		<div class=\"aleft dleft\" style=\"margin-bottom:10px;\">
			<input type=\"text\" class=\"inp-form dleft\" name=\"word_search\" size=\"50\" placeholder=\"" .
            _PHRASE_SEARCH .
            "\" value=\"$word_search\">
			<input type=\"hidden\" name=\"op\" value=\"language\" />
			<input type=\"hidden\" name=\"csrf_token\" value=\"" .
		...
		<tr>
			<th class=\"table-header-repeat line-left\" style=\"width:250px;\">" .
            _PHRASE .
            "</th>
			<th class=\"table-header-repeat line-left\">" .
            _EQUAL_TO_IN_CURLANG .
            " (" .
            $nuke_configs['language'] .
            ")</th>
			<th class=\"table-header-repeat line-left\" style=\"width:120px;\">" .
            _OPERATION .
            "</th>
		</tr>";
        ...
        $link_to =
            ...
            (isset($word_search) && $word_search != ''
                ...
                : "");
        $direction =
            $nuke_languages[$nuke_configs['currentlang']]['_DIRECTION'];

        if (isset($word_search) && $word_search != '') {
            $searched_nuke_languages = [];
            foreach ($nuke_languages as $inlang => $language_values) {
                ...
                foreach ($language_values_by_arr as $v) {
                    if (
                        mb_stristr($v, $word_search) or
                        mb_stripos($v, $word_search) !== false or
                        mb_stristr($language_values[$v], $word_search) or
                        mb_stripos($language_values[$v], $word_search) !== false
                    ) {
                        $searched_nuke_languages[$inlang][$v] =
                            $language_values[$v];
                        foreach ($languageslists as $languageslist) {
                            if (isset($nuke_languages[$languageslist][$v])) {
                                $searched_nuke_languages[$languageslist][$v] =
                                    $nuke_languages[$languageslist][$v];
                            }
                        }
                    }
                }
        ...
        $newICounter =
            $start_at + $entries_per_page <= sizeof($all_language_data)
                ? $start_at + $entries_per_page
                : sizeof($all_language_data);
        $nuke_languages_by_arr = array_keys($all_language_data);
        $j = 1;

        for ($i = $start_at; $i < $newICounter; $i++) {
            $word = $nuke_languages_by_arr[$i];
            $equal_in_current_lang = isset(
                $nuke_languages[$nuke_configs['currentlang']][$word]
            )
                ? $nuke_languages[$nuke_configs['currentlang']][$word]
                : "";
            ...
            $contents .=
                ...
                highlightWords($word, $word_search) .
                "</td>
				<td align=\"center\">" .
                highlightWords($equal_in_current_lang, $word_search) .
                "</td>
				<td align=\"center\">";
            if (
                isset($nuke_languages_cacheData[$word]) &&
                !empty($nuke_languages_cacheData[$word])
            ) {
                $contents .=
                    " <a class=\"table-icon icon-2 info-tooltip\" href=\"" .
                    $admin_file .
                    ".php?op=delete_language_word&word=$word&csrf_token=" .
                    _PN_CSRF_TOKEN .
                    "\" title=\"" .
                    _DELETE .
                    "\" onclick=\"return confirm('" .
                    _DELETE_PHRASE_CONFIRM .
                    "');\"></a>";
            }
            $contents .=
                "<a class=\"table-icon icon-1 info-tooltip\" href=\"" .
                $admin_file .
                ".php?op=edit_language_word&word=$word\" title=\"" .
                _EDIT .
                "\"></a>
					<a class=\"table-icon icon-7 info-tooltip thickbox\" href=\"#TB_inline?height=300&inlineId=word_data_" .
                $j .
                "&width=600\" title=\"" .
                _VIEW .
                "\"></a>
            ...
            foreach ($nuke_languages as $inlang => $language_values) {
                $contents .=
                    "<tr>
										...
										<td" .
                    ($direction == 'ltr' ? " style=\"direction:ltr;\"" : "") .
                    ">" .
                    (isset($language_values[$word])
                        ? $language_values[$word]
                        : "") .
                    "</td>
									</tr>";
            }
    ...
    function edit_language_word($word, $word_equals)
    {
        ...
        $pagetitle =
            "" . _LANGUAGE_ADMIN_EDIT . " (<span dir=\"ltr\">$word</span>)";
        $hooks->add_filter("set_page_title", function () use ($pagetitle) {
            return ["comments" => $pagetitle];
        });
        $contents = '';
        ...
        foreach ($languageslists as $languageslist) {
            if ($languageslist != "") {
                ...
                $languageslist = str_replace(".php", "", $languageslist);
                $equal = isset($nuke_languages_cacheData[$word][$languageslist])
                    ? $nuke_languages_cacheData[$word][$languageslist]
                    : (isset($nuke_languages[$languageslist][$word])
                        ? $nuke_languages[$languageslist][$word]
                        : "");
                ...
                $contents .=
                    ...
                    ($nuke_languages[$languageslist]['_DIRECTION'] == 'ltr'
                        ? " dleft"
                        : "") .
                    "\" type=\"text\" size=\"70\" name=\"word_equals[$languageslist]\" value=\"" .
                    htmlentities($equal) .
                    "\" />
						</td>
					</tr>";
