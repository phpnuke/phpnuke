...
class BackupMySQL
{
    ...
    function first_Execute($file_name)
    {
        ...
        if ($this->comments) {
            ...
            $sql .=
                '# Created by Database Backup class, ver. ' .
                MSB_VERSION .
                MSB_NL;
            $sql .= '#' . MSB_NL;
            ...
            $sql .=
                ...
                MSB_NL;
            $sql .=
                '# MySQL version: ' . $mysql_version[0]['version()'] . MSB_NL;
    ...
    {
        if (!($sql = $this->_Retrieve($table_name, $start, $end, $max_rows))) {
            return false;
        }
        return $this->_SaveToFile($file_name, $sql);
    }
    ...
    function _GetTables()
    {
        ...
        if (
            !($result = $db->query(
                "SELECT table_name, table_rows FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$pn_dbname';"
            ))
        ) {
            return false;
        }
        ...
        if ($db->count() > 0) {
            foreach ($result as $row) {
                ...
                if (
                    !empty($this->tables) &&
                    !in_array($table_name, $this->tables)
                ) {
                    continue;
                }
    ...
    function _DumpTable($table, $start, $end, $row_count)
    {
        ...
        if ($start == 0) {
            if ($this->comments) {
                $value .= '#' . MSB_NL;
                $value .=
                    '# Table structure for table `' . $table . '`' . MSB_NL;
                $value .= '#' . MSB_NL . MSB_NL;
            }
            if ($this->drop_tables) {
        ...
        if ($this->db_connection_charset !== '') {
            $this->db->query(
                "set character_set_server='" .
                    $this->db_connection_charset .
                    "'"
            );
            $this->db->query(
                "SET NAMES '" . $this->db_connection_charset . "'"
            );
        }
            ...
            foreach ($this->pre_query as $pre_query_value) {
                if (!$this->db->query($pre_query_value)) {
                    ...
                    $error = [
                        ...
                        "message" => [
                            "line" => trim(
                                nl2br(htmlentities($pre_query_value))
                            ),
                            "error_exp" => $this->db->getErrors('last')[
                                'message'
                            ],
                        ],
                    ];
        ...
        while ($linenumber < $start + $this->linespersession || $query != "") {
            ...
            if (!$inparents) {
                ...
                foreach ($this->comment as $comment_value) {
                    if (
                        trim($dumpline) == "" ||
                        strpos(trim($dumpline), $comment_value) === 0
                    ) {
                        $skipline = true;
                        break;
                    }
            ...
            $parents =
                substr_count($dumpline_deslashed, $this->string_quotes) -
                substr_count(
                    $dumpline_deslashed,
                    "\\" . $this->string_quotes . ""
                );
            ...
            if ($querylines > $this->max_query_lines) {
                $error = [
                    ...
                    "message" => [
                        "line" => '',
                        "error_exp" =>
                            "توقف در رديف $linenumber.در اين محل کوئري مورد نظر شامل بيش از " .
                            $this->max_query_lines .
                            " رديف است.",
                    ],
                ];
                return json_encode($error);
            ...
            $error = [
                ...
                "message" => [
                    ...
                    "error_exp" =>
                        "Press <b><a href=\"" .
                        $_SERVER["PHP_SELF"] .
                        "\">STOP</a></b> to abort the import <b>OR WAIT!<br /><br />waiting ...",
                ],
                "start" => $linenumber,
                "foffset" => $nfoffset,
