...
if (check_admin_permission($filename)) {
    function hreferrer()
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["hreferrer" => _REFERRERS_ADMIN];
        });
        $contents = '';
        $contents .= GraphicAdmin();
        $contents .= OpenAdminTable();
        $contents .=
            "<br /><br /><div class=\"text-center\"><b>" .
            _REFERRERS .
            "</b></div><br><br>";
		...
		<th class=\"table-header-repeat line-left\" style=\"width:50px;\"><a>" .
            _ROW .
            "</a></th>
		<th class=\"table-header-repeat line-left\" style=\"width:200px;\"><a>" .
            _DATE .
            "</a></th>
		<th class=\"table-header-repeat line-left\" style=\"width:120px;\"><a>" .
            _IP_INPUT .
            "</a></th>
		<th class=\"table-header-repeat line-left\"><a>" .
            _REFERRAL_LINK .
            "</a></th>
		<th class=\"table-header-repeat\"><a>" .
            _ENTRY_LINK .
            "</a></th>
		<th class=\"table-header-repeat no-padding\" style=\"width:110px\"><input data-label=\"" .
            _OPERATION .
            "\" type=\"checkbox\" class=\"styled select-all\" data-element=\"#http-referrers\"></th>";
        if ($db->count() > 0) {
            foreach ($result as $row) {
                ...
                $time =
                    nuketimes($row['time']) . " " . date("H:i:s", $row['time']);
                $url_decoded = LinkToGT(rawurldecode($url));
                $path_decoded = LinkToGT(rawurldecode($path));

                $url_short = rawurldecode(
                    mb_word_wrap($url_decoded, 140, '...')
                );
                ...
                $contents .=
					...
					<td align=\"center\" class=\"dleft\"><a href=\"index.php?url=" .
                    LinkToGT($path) .
                    "\" target=\"_new\" class=\"info-tooltip\">$path_decoded</a></td>
					<td align=\"center\"><a href=\"" .
                    $admin_file .
                    ".php?op=delreferrer&rid=$rid&csrf_token=" .
                    _PN_CSRF_TOKEN .
                    "\" class=\"table-icon icon-2 info-tooltip\" title=\"" .
                    _DELETE .
                    "\" onclick=\"return confirm('" .
                    _DELETE_THIS_SURE .
                    "');\"></a> <input type=\"checkbox\" class=\"styled\" name=\"rid[]\" value=\"$rid\" /></td>
				</tr>";
            }
        }
        $contents .=
		...
		<div class=\"text-center\"><input type=\"submit\" value='" .
            _DELETE_SELECTED .
            "' onclick=\"return confirm('" .
            _DELETE_THIS_SURE .
            "');\"></div><br /><p align=\"center\"><a href=\"" .
            $admin_file .
            ".php?op=delreferrer&all=1&csrf_token=" .
            _PN_CSRF_TOKEN .
            "\" onclick=\"return confirm('" .
            _DELETE_ALL_THIS_SURE .
            "');\"><button type=\"button\">" .
            _DELETE_ALL .
            "</button></a></p><br />
