...
if (check_admin_permission($filename)) {
    ...
    function update_comments_replay_time($main_parents)
    {
        ...
        $main_parents = is_array($main_parents)
        ...
        $result = $db->query(
            "SELECT c.cid, c.`date`, (SELECT `date` FROM " .
                COMMENTS_TABLE .
                " WHERE main_parent = c.cid AND status = '1' ORDER BY `date` DESC LIMIT 1) as last_replay_time FROM " .
                COMMENTS_TABLE .
                " AS c WHERE c.cid IN ($main_parents)"
        );
    ...
    function comments(
        ...
        $pagetitle =
            _COMMENTS_ADMIN .
            ($status == 1 ? " " . _APPROVED : " " . _PENDING) .
            ($reported == 1 ? " " . _REPORTED : "") .
            ($module_name != '' ? " - " . _MODULE . " $module_name" : "") .
            ($post_title != ''
                ? " - " . sprintf(_POST_COMMENTS_VIEW, $post_title)
                : "");

        $hooks->add_filter("set_page_title", function () use ($pagetitle) {
            return ["comments" => $pagetitle];
        });
        $contents = '';
        $contents .= GraphicAdmin();
        $nuke_configs['comments'] =
            $nuke_configs['comments'] != ''
                ? phpnuke_unserialize(stripslashes($nuke_configs['comments']))
                : [];

        $all_modules_comments = [];
        $all_modules_comments = $hooks->apply_filters(
            'modules_have_comments',
            $all_modules_comments
        );
        foreach (
            $all_modules_comments
            as $modules_comment_key => $modules_comment_value
        ) {
            $sel = $module_name == $modules_comment_key ? "selected" : "";
            $all_modules_comments_link[] =
                "<option value=\"" .
                $admin_file .
                ".php?op=comments&module_name=$modules_comment_key&status=$status" .
                ($reported != 0 ? "&reported=$reported" : "") .
                ($post_id != 0
                    ? "&post_id=$post_id&post_title=$post_title"
                    : "") .
                "\" $sel>$modules_comment_value</option>";
        }
        $contents .=
            ...
            "</a> | <a href=\"" .
            $admin_file .
            ".php?op=comments&status=0\">" .
            _PENDING_COMMENTS .
            "</a> | <a href=\"" .
            $admin_file .
            ".php?op=comments&reported=1\">" .
            _REPORTED_COMMENTS .
            "</a> | <a href=\"" .
            $admin_file .
            ".php?op=settings\">" .
            _SETTINGS .
            "</a> ]<br /><br />" .
            _VIEW_COMMENTS_IN_MODULE .
            " <select onchange=\"top.location.href=this.options[this.selectedIndex].value\" class=\"styledselect-select\"><option value=\"" .
            $admin_file .
            ".php?op=comments\">" .
            _ALL .
            "</options>\n" .
        ...
        $contents .=
            ...
            _COMMENT .
            "	</th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:180px;\">" .
            _POST .
            "</th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:70px;\">" .
            _IN_REPLY .
            "</th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:80px;\">" .
            _MODULE .
            "</th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:180px;\">" .
            _DATE .
            "</th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:100px;\">" .
            _POSTER .
            "</th>
				<th class=\"table-header-repeat no-padding\" style=\"width:160px\"><input data-label=\"" .
            _OPERATION .
            "\" type=\"checkbox\" class=\"styled select-all\" data-element=\"#nuke-comments\"></th>
			</tr>
			</thead>
			<tbody>";
        ...
        $link_to =
            ...
            ($post_id != 0 ? "&post_id=$post_id&post_title=$post_title" : "");
        ...
        if (!isset($post_id) or $post_id == 0) {
            $select2 =
                ", (SELECT COUNT(c3.cid) FROM " .
                COMMENTS_TABLE .
                " AS c3 WHERE " .
                str_replace("c.", "c3.", $where) .
                " AND c3.post_id = c.post_id) as total_post_comments";
        }

        $result = $db->query(
            "
				SELECT c.*, 
				(SELECT COUNT(c2.cid) FROM " .
                COMMENTS_TABLE .
                " AS c2 WHERE " .
                str_replace("c.", "c2.", $where) .
                ") as total_rows, 
				pc.name as reply_to, 
				(SELECT COUNT(c4.cid) FROM " .
                COMMENTS_TABLE .
                " AS c4 WHERE " .
                str_replace("c.", "c4.", $where) .
                " AND c4.cid " .
                ($nuke_configs['comments']['order_by'] == 1 ? ">" : "<") .
                "= IF(c.main_parent =0, c.cid,c.main_parent) AND c4.pid ='0') as position
				$select2 
				FROM " .
                COMMENTS_TABLE .
                " AS c 
        ...
        if ($db->count() > 0) {
            foreach ($result as $row) {
                ...
                $cid = intval($row['cid']);
                $pid = intval($row['pid']);
                $total_post_comments = isset($row['total_post_comments'])
                    ? " <span style=\"background:#ccc;border-radius:5px;padding:2px 5px;display:inline-block;\">" .
                        intval($row['total_post_comments']) .
                        "</span>"
                    : 0;
                ...
                $username_link =
                    $username == $ip
                        ? $ip_info
                        : LinkToGT(
                            sprintf($users_system->profile_url, '', $username)
                        );
                ...
                $current_comment_page =
                    $nuke_configs['comments']['item_per_page'] > 0
                        ? ceil(
                            $position /
                                $nuke_configs['comments']['item_per_page']
                        )
                        : 0;
                ...
                $reply_to =
                    $row['reply_to'] != ""
                        ? "<a href=\"$post_link#comment-$pid\" target=\"_blank\">" .
                            stripslashes($row['reply_to']) .
                            "</a>"
                        : "";
                ...
                $module_link_to =
                    "" .
                    $admin_file .
                    ".php?op=comments&module_name=$module" .
                    (isset($status) ? "&status=$status" : "") .
                    (isset($reported) ? "&reported=$reported" : "");
                $url_arr = $url != '' ? explode("/", $url) : [];
                $clean_url =
                    is_array($url_arr) && !empty($url_arr) ? $url_arr[2] : '';
                ...
                $contents .=
                    "<tr>
						...
						<td><a href=\"$post_link\" target=\"_blank\">$post_title </a>" .
                    (isset($row['total_post_comments']) &&
                    intval($row['total_post_comments']) != 0
                        ? " <a href=\"$module_link_to&post_id=$post_id&post_title=$post_title\">$total_post_comments<a/>"
                        : "") .
                    "</td>
						...
						<td align=\"center\">
							<a href=\"#\" title=\"" .
                    _EDIT .
                    "\" class=\"table-icon icon-1 info-tooltip editindialog\" data-op=\"comments_edit\" data-cid=\"$cid\"></a>
							<a href=\"" .
                    $admin_file .
                    ".php?op=comments_delete&cid=$cid&status=$status&csrf_token=" .
                    _PN_CSRF_TOKEN .
                    "\" title=\"" .
                    _DELETE .
                    ...
                    "');\"></a>
							<a href=\"" .
                    $admin_file .
                    ".php?op=comments_status&cid=$cid&new_status=$new_status&status=$status&csrf_token=" .
                    _PN_CSRF_TOKEN .
                    "\" title=\"$new_status_desc\" class=\"table-icon icon-$new_status_icon info-tooltip\"></a>
							<a href=\"#\" title=\"" .
                    _REPLY .
                    "\" class=\"table-icon icon-10 info-tooltip replyindialog\" data-cid=\"$cid\" data-op=\"comments_reply\"></a>
		...
		<div class=\"text-center\">
			<select name=\"mode\">
				<option value=\"delete\" onclick=\"return confirm('" .
            _DELETE_THIS_SURE .
            "');\">" .
            _DELETE .
            "</option>
    ...
    function comments_edit(
        ...
        if (
            isset($update) &&
            $update != "" &&
            isset($comment_fields) &&
            is_array($comment_fields) &&
            !empty($comment_fields)
        ) {
        ...
        if ($nuke_configs['comments']['editor'] == 0) {
            ...
            $content .=
                "<textarea class=\"form-textarea\" name='comment_fields[comment]' cols='70' rows='15'>" .
                $comment .
                "</textarea>";
        }
        $content .=
            "</td>
				...
				<tr>
					...
					<input type=\"submit\" value=\"" .
            _SEND .
            "\" name=\"update\" class=\"form-submit\">
					</td>
				</tr>
			</table><input type=\"hidden\" name=\"op\" value=\"comments_edit\"><input type=\"hidden\" name=\"cid\" value=\"$cid\"><input type=\"hidden\" name=\"csrf_token\" value=\"" .
            _PN_CSRF_TOKEN .
            "\" /> </form>";
        if ($show_header == 1) {
            $contents = '';
    ...
    function comments_delete(
        ...
        if (!empty($deleted_cids)) {
            ...
            $module_table_data = ['', ''];
            $module_table_data = $hooks->apply_filters(
            ...
            if (
                ...
                isset($module_table_data) &&
                !empty($module_table_data)
            ) {
                $rows = $result->results();
                foreach ($rows as $row) {
                    $module = $row['module'];
            ...
            $db->query(
                "DELETE FROM " . COMMENTS_TABLE . " WHERE " . $where . ""
            );
    ...
    function comments_status(
        ...
        if (!empty($status_cids)) {
            ...
            $db->query(
                "UPDATE " .
                    COMMENTS_TABLE .
                    " SET status = '$new_status' WHERE $where"
            );
    ...
    function comments_reply($cid, $reply_save, $reply)
    {
        ...
        if (
            ...
            $commenter_username = isset($userinfo['username'])
                ? $userinfo['username']
                : $aid;
            $commenter_name = isset($userinfo['name'])
                ? $userinfo['name']
                : $nuke_authors_cacheData[$aid]['realname'];
            $commenter_email = isset($userinfo['user_email'])
                ? $userinfo['user_email']
                : $nuke_authors_cacheData[$aid]['email'];
            $commenter_url = isset($userinfo['user_website'])
                ? $userinfo['user_website']
                : $nuke_authors_cacheData[$aid]['url'];
            ...
            $db->table(COMMENTS_TABLE)->insert([
                'pid' => $cid,
                'main_parent' =>
                    $row['main_parent'] == 0 ? $cid : $row['main_parent'],
                'module' => $row['module'],
                'post_id' => $row['post_id'],
        ...
        $content =
			...
			<table align=\"center\" border=\"0\" width=\"100%\" class=\"id-form product-table no-border\">
				<tr>
					<td align=\"" .
            _TEXTALIGN1 .
            "\"><div class=\"form-textarea\" style=\"width:100%;\">$comment</div></td>
				</tr>
				<tr>
					<td>";
        if ($nuke_configs['comments']['editor'] == 0) {
            ...
            $content .=
                "<textarea class=\"form-textarea\" name='reply' cols='70' rows='15'></textarea>";
        }
        $content .=
            "</td>
				</tr>
				<tr>
					...
					<input type=\"submit\" value=\"" .
            _SEND .
            "\" name=\"reply_save\" class=\"form-submit\">
					</td>
				</tr>
			</table>
    ...
    $comment_fields = isset($comment_fields) ? $comment_fields : [];
    $comment_username = isset($comment_username)
        ? filter($comment_username, "nohtml")
        : '';
