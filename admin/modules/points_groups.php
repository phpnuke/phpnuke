...
if (check_admin_permission($filename)) {
    ...
    function points_groups()
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["points_groups" => _POINTS_GROUP_ADMIN];
        });
		...
		<form action=\"" .
            ...
            _POINT_VALUE .
            ":</th>
					<td><input class=\"inp-form\" type=\"text\" name=\"points\" size=\"10\" maxlength=\"20\" value=\"0\">&nbsp;<i>(" .
            _ONLYNUMVAL .
            ")</i></td>
				</tr>
			</table>
		...
		<table border=\"0\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" class=\"product-table\">
            ...
            _POINTS .
            "</th>
				<th class=\"table-header-repeat\" style=\"width:180px\">" .
            _OPERATION .
            "</th>
			</tr>";
        ...
        if ($db->count() > 0) {
            foreach ($result as $row) {
                ...
                $contents .=
                    ...
                    $row['points'] .
                    "\" size=\"5\" class=\"inp-form\" style=\"text-align:center;\" name=\"points\">&nbsp;</td>
						<td align=\"center\" class=\"id-form\"><font class=\"content\">&nbsp;
						<input type=\"hidden\" name=\"id\" value=\"$id\">
						<input type=\"hidden\" name=\"op\" value=\"points_update\">
						<input type=\"submit\" value=\"" .
                    _UPDATE .
                    "\" class=\"form-submit\">&nbsp;</font>
						" .
                    ($row['type'] == 0
                        ? "<a href=\"" .
                            $admin_file .
                            ".php?op=group_edit&id=$id\" class=\"table-icon icon-1 info-tooltip\" title=\"" .
                            _EDIT .
                            "\"></a><a href=\"" .
                            $admin_file .
                            ".php?op=group_del&id=$id&csrf_token=" .
                            _PN_CSRF_TOKEN .
                            "\" class=\"table-icon icon-2 info-tooltip\" title=\"" .
                            _DELETE .
                            "\" onclick=\"return confirm('" .
                            _POINT_GROUP_DELETE_SURE .
                            "');\"></a>"
                        : "") .
                    "
						</td>
    ...
    function group_add($title, $description, $points)
    {
        ...
        if (!is_numeric($points) || @stristr($points, "-")) {
            ...
            $query = $db->query(
                "SHOW TABLE STATUS WHERE name='" . POINTS_GROUPS_TABLE . "'"
            );
            $row = $query->results()[0];
            $next_inc_value = !isset($row["AUTO_INCREMENT"])
                ? $row["Auto_increment"]
                : $row["AUTO_INCREMENT"];

            $title = filter($title, 'nohtml');
            $points = intval($points);
            $db->table(POINTS_GROUPS_TABLE)->insert([
                "id" => $next_inc_value,
                "title" => $title,
    ...
    function group_edit($id)
    {
        ...
        $hooks->add_filter("set_page_title", function () use ($title) {
            return ["group_edit" => _POINTS_GROUP_ADMIN . " - " . $title];
        });
        ...
        $contents .=
            "<div class=\"text-center\"><font class=\"option\"><b>" .
            sprintf(_EDIT_POINT_GROUP, $title) .
            "</b></font></div><br><br>
		<form action=\"" .
            $admin_file .
            ".php\" method=\"post\">
            ...
            _POINT_VALUE .
            ":</th>
					<td><input class=\"inp-form\" type=\"text\" name=\"points\" size=\"10\" maxlength=\"20\" value=\"$points\">&nbsp;<i>(" .
            _ONLYNUMVAL .
            ")</i></td>
				</tr>
			</table>
