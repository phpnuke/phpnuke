...
if (check_admin_permission($filename)) {
    ...
    $default_folder = is_God()
        ? 'files'
        : (isset($upload_allowed_info[$aid]['path']) &&
        $upload_allowed_info[$aid]['path'] != '' &&
        is_dir($upload_allowed_info[$aid]['path']) &&
        file_exists($upload_allowed_info[$aid]['path'])
            ? $upload_allowed_info[$aid]['path']
            : (is_dir("files/uploads/$aid") && file_exists("files/uploads/$aid")
                ? "files/uploads/$aid"
                : "files/uploads/"));
    ...
    function common()
    {
		...
		[ <a href=\"" .
            $admin_file .
            ".php?op=media_browser\">" .
            _FILES_MANAGER .
            "</a> | <a href=\"" .
            $admin_file .
            ".php?op=media_upload&upload_dir=files/uploads&csrf_token=" .
            _PN_CSRF_TOKEN .
            "\">" .
            _UPLOAD_FILE .
            "</a> ]
    ...
    function media_browser($ckeditor = '')
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["media_browser" => _MULTIMEDIA];
        });
        $contents = '';
        if ($ckeditor != "") {
		...
		<table width=\"100%\" class=\"product-table no-hover\">
			<tr>
				<th class=\"table-header-repeat line-left\" style=\"width:80%\">" .
            _FILES_LIST .
            "</th>
				<th class=\"table-header-repeat line-left\" style=\"width:20%\">" .
            _SELECT_PATH .
            "</th>
			</tr>
			<tr>
        ...
        if ($ckeditor == "") {
            $contents .=
                ...
                _PICTURE .
                "</th>
								<th class=\"table-header-repeat line-left\" style=\"\">" .
                _TITLE .
                "</th>
								<th class=\"table-header-repeat line-left\" style=\"width:110px\">" .
                _FILESIZE .
                "</th>
								<th class=\"table-header-repeat line-left\" style=\"width:180px\">" .
                _CREATIONDATE .
                "</th>
								<th class=\"table-header-repeat no-padding\" style=\"width:110px\"><input data-label=\"" .
                _OPERATION .
                "\" type=\"checkbox\" class=\"styled select-all\" data-element=\"#medias\"></th>
							</tr>
							</thead>";
        }
        $contents .=
            ...
            $admin_file .
            ".php?op=media_upload&upload_dir=$default_folder&ckeditor=$ckeditor" .
            ($ckeditor != '' ? "&CKEditorFuncNum=$CKEditorFuncNum" : "") .
            "&csrf_token=" .
            _PN_CSRF_TOKEN .
            "\" id=\"uploadto\">" .
            _UPLOAD_IN .
            "<br /><span id=\"upload_path\" dir=ltr>$default_folder</span></a><div id=\"media_menu\"></div></td>
			</tr>
		</table>
		<script>
    ...
    function media_upload($upload_dir, $ckeditor)
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["media_browser" => _MULTIMEDIA . " - " . _UPLOAD_FILE];
        });
        ...
        if (isset($_FILES['file_upload'])) {
            ...
            if (!is_God() && !check_default_folder($upload_dir)) {
                die(
                    '{"status":"error","message":"' .
                        sprintf(_NOTALLOWED_UPLOADPATH, $upload_dir) .
                        '"}'
                );
                exit();
            }
            ...
            if ($_FILES['file_upload']['error'] == 0) {
                ...
                $metadatas = read_media_metadata(
                    $_FILES['file_upload']['tmp_name']
                );

                if (
                    !in_array($metadatas['mime_type'], [
                        "video/avi",
                        "image/gif",
                        "image/jpeg",
                        "image/pjpeg",
                    ...
                    die(
                        '{"status":"error","message":"' .
                            sprintf(_NOTALLOWED_EXT, $extension) .
                            '"}'
                    );
                    exit();
                }
                ...
                $filename = validateLatin($_FILES['file_upload']['name'])
                    ? $_FILES['file_upload']['name']
                    : rawurlencode($_FILES['file_upload']['name']);
                $filename = substr($filename, 0, strrpos($filename, "."));

                $filename = sanitize($filename, ["_"]);
                $file_name = $filename . "." . $extension;

                while (
                    file_exists($upload_dir . $filename . $i . "." . $extension)
                ) {
                    $i = intval($i) + 1;
		...
		<form id=\"upload\" method=\"post\" action=\"" .
            $admin_file .
            ".php?op=media_upload&upload_dir=$upload_dir\" enctype=\"multipart/form-data\" accept-charset=\"utf-8\">
			<div id=\"drop\">
				<h1 dir=\"rtl\">" .
            _DROG_FILES_HERE .
            "<br />" .
            sprintf(_UPLOAD_DIR_WILL_BE, $upload_dir) .
            "</h1>
				<br />
				" .
            _GOBACK .
    ...
    function get_media_metadata($file_name)
    {
        ...
        $file_mime_type = preg_match('!^image/!', $metadatas['mime_type'])
            ? "image"
            : (preg_match('!^audio/!', $metadatas['mime_type'])
                ? "audio"
                : (preg_match('!^video/!', $metadatas['mime_type'])
                    ? "video"
                    : "application"));
        $contents .= "
		...
		<div class=\"media-preview-metadata\">
            ...
            "</td>
					<td><b>" .
            (isset($metadatas['fileformat']) && $metadatas['fileformat'] != ''
                ? $metadatas['fileformat']
                : $extension) .
            "</b></td>
				</tr>";
        ...
        if (!empty($metadatas['audio']['bitrate'])) {
            $contents .=
                ...
                "</td>
					<td><b>" .
                round($metadatas['audio']['bitrate'] / 1000) .
                "kb/s " .
                (!empty($metadatas['audio']['bitrate_mode'])
                    ? " " . strtoupper($metadatas['audio']['bitrate_mode'])
                    : "") .
                "</b></td>
				</tr>";
        }
        ...
        $contents .=
            ...
            "</td>
					<td><b>" .
            nuketimes(filectime($file_name)) .
            " " .
            _HOUR .
            " " .
            date("H:i", filectime($file_name)) .
            "</b></td>
				</tr>
			</table>
		</div>
        ...
        if ($file_mime_type == "image") {
            $contents .=
                "<img src=\"" .
                LinkToGT("index.php?timthumb=true&src=$file_name&w=390") .
                "\" width=\"390\" style=\"border:1px solid #ccc;padding:2px;\" />";
        } elseif ($file_mime_type == "audio" && $type['type'] != '') {
            $contents .=
                "
			<audio controls=\"controls\" type=\"" .
                $type['type'] .
                "\">
            ...
            $contents .=
                "<video width=\"390\" poster=\"" .
                LinkToGT(
                    "index.php?timthumb=true&data_image=true&src=$file_name"
                ) .
                "\" height=\"250\" controls=\"controls\" preload=\"none\">
				<source type=\"" .
                $type['type'] .
                "\" src=\"$file_name\" title=\"$file_name_clean\" />
			</video>";
        }
        if (
            $type['type'] == '' &&
            ($file_mime_type == "video" || $file_mime_type == "audio")
        ) {
        ...
        if (count($files) > 2) {
            ...
            foreach ($files as $file) {
                if (
                    ...
                    $list .=
                        '<li class="folder collapsed"><a href="#" rel="' .
                        htmlentities($folder . $file) .
                        '">' .
                        htmlentities($file) .
                        '</a></li>';
                }
    ...
    function media_get_files($directory, $page, $ckeditor)
    {
        ...
        preg_match_all(
            '#id="actions-box-slider">(.*)</div>#isU',
            $pagination,
            $matchs
        );
        $action_pagenum = str_replace(
            "media_browser",
            "action-pagenum media_browser",
            $matchs[0][0]
        );
        ...
        $responsedata = [
            'files' => $files_info,
            'pagination' => jquery_codes_load($plugin, true) . $pagination,
            'adminname' => is_God() ? '' : $aid,
        ];
        if (empty($files_info)) {
            $responsedata = null;
        }
