...
if (check_admin_permission($filename)) {
    class Walker_nav_menus_admin extends Walker
    {
        ...
        public function start_el(
            ...
            $class_names = $class_names
                ? ' class="dd-item ' . filter($class_names, "nohtml") . '"'
                : '';
    ...
    function nav_menus()
    {
        ...
        $hooks->add_filter("set_page_title", function () {
            return ["nav_menus" => _NAVS_ADMIN];
        });

        $contents .=
            "<div align=\"center\" style=\"margin:20px 0;\">[ <a href=\"" .
            $admin_file .
            ".php?op=nav_menus_admin\">" .
            _ADD_NEW_NAV .
            "</a> ]</div>";
        ...
        $contents .=
            ...
            _STATUS .
            "</th>
				<th class=\"table-header-repeat line-left\" style=\"text-align:center;width:110px;\">" .
            _OPERATION .
            "</th>
			</tr>
			</thead>
                ...
                $contents .=
                    "<tr>
						...
						<td align=\"center\">
							<a href=\"" .
                    $admin_file .
                    ".php?op=nav_menus_admin&mode=edit&nav_id=$nav_id\" title=\"" .
                    _EDIT .
                    "\" class=\"table-icon icon-1 info-tooltip\"></a>
							<a href=\"" .
                    $admin_file .
                    ".php?op=nav_menus_admin&mode=delete&nav_id=$nav_id&csrf_token=" .
                    _PN_CSRF_TOKEN .
                    "\" title=\"" .
                    _DELETE .
    ...
    function nav_menus_admin(
        $nav_id = 0,
        array $nav_fields,
        array $nav_menu_fields,
        $mode = 'add_nav'
    ) {
        ...
        $theme_nav_menus = isset($theme_setup['theme_nav_menus'])
            ? $theme_setup['theme_nav_menus']
            : [];

        // add menu to a nav
        if ($mode == "add_nav_menu" && !empty($nav_menu_fields)) {
            $result = $db
                ->table(NAV_MENUS_DATA_TABLE)
                ->order_by(["nid" => "DESC"])
                ->limit(0, 1)
                ->select()
                ->first();
            ...
            if ($nav_menu_fields['type'] == 'custom') {
                $insert = $db->table(NAV_MENUS_DATA_TABLE)->insert([
                            ...
                            filter(
                                implode(
                                    ...
                                    array_reverse(
                                        get_parent_names(
                                            $cat_id,
                                            $nuke_categories_cacheData[
                                                $nav_menu_fields['module']
                                            ],
                                            "parent_id",
                                            "catname_url"
                                        )
                ...
                $db->table(NAV_MENUS_DATA_TABLE)
                    ->setSchema("auto_increment = $new_increment_nid")
                    ->alter();
        ...
        if ($mode == "save_nav" && !empty($nav_fields)) {
            ...
            if (isset($nav_fields['final_items'])) {
                $nav_fields['final_items'] = objectToArray(
                    json_decode($nav_fields['final_items'])
                );
                $db->sql_query(
                    "UPDATE " .
                        NAV_MENUS_TABLE .
                        " SET nav_title = '" .
                        $nav_fields['general_title'] .
                        "', lang_nav_title = '" .
                        addslashes(
                            phpnuke_serialize($nav_fields['lang_titles'])
                        ) .
                        "', nav_location = '" .
                        $nav_fields['nav_location'] .
                        "', status = '" .
                        $nav_fields['nav_status'] .
                        "', date = '" .
                        _NOWTIME .
                        "' WHERE nav_id = '$nav_id'"
                );

                $unnested_nav_fields = array_flatten(
                    $nav_fields['final_items'],
                    0,
                    'nid',
                    'pid',
                    'children',
                ...
                if (!empty($unnested_nav_fields)) {
                    ...
                    foreach ($unnested_nav_fields as $nid => $nav_data) {
                        ...
                        foreach ($nav_data as $nav_data_key => $nav_data_val) {
                            if (
                                ...
                                $unnested_nav_fields[$nid]['attributes'][
                                    $nav_data_key
                                ] = $nav_data_val;
                                unset(
                                    $unnested_nav_fields[$nid][$nav_data_key]
                                );
                            }
                    ...
                    foreach (
                        $update_fields
                        as $update_field_key => $update_field
                    ) {
                        $update_query_field =
                            ($c == 0 ? "" : ", \n") .
                            (is_array($update_field)
                                ? $update_field_key
                                : $update_field) .
                            " = (CASE ";
                        $update_query_value = '';
                        foreach ($unnested_nav_fields as $nid => $nav_data) {
                            $update_query_value .=
                                "\nWHEN nid = '$nid' THEN '" .
                                (isset($nav_data[$update_field])
                                    ? (is_array($nav_data[$update_field])
                                        ? phpnuke_serialize(
                                            $nav_data[$update_field]
                                        )
                                        : $nav_data[$update_field])
                                    : "") .
                                "' ";
                    ...
                    if (!empty($must_removed_items)) {
                        ...
                        $db->sql_query(
                            "DELETE FROM " .
                                ...
                                " WHERE nid IN ($must_removed_items)"
                        );
                        $response = json_encode([
                            "status" => "success",
                            "message" => _OP_OK,
                    ...
                    if ($update_query_items != '') {
                        $update_query .=
                            $update_query_items .
                            "\n WHERE nid IN (" .
                            implode(",", array_keys($unnested_nav_fields)) .
                            ")";
                ...
                cache_system("nuke_nav_menus");
                add_log(
                    sprintf(_EDIT_NAV_LOG, $nav_fields['general_title']),
                    1
                );
            } else {
                $response = json_encode([
        ...
        if ($mode == "delete") {
            ...
            $result1 = $db->sql_query(
                "DELETE FROM " . NAV_MENUS_TABLE . " WHERE nav_id = '$nav_id'"
            );
            cache_system("nuke_nav_menus");
        ...
        if ($nav_id == 0) {
            $nav_id = $db->table(NAV_MENUS_TABLE)->insert([
                "date" => _NOWTIME,
                "status" => 2,
            ]);
        ...
        $result = $db->query(
            "SELECT n.*, nv.nav_title, nv.nav_location, nv.lang_nav_title, nv.status as nav_status FROM " .
                NAV_MENUS_DATA_TABLE .
                " AS n LEFT JOIN " .
                NAV_MENUS_TABLE .
                " AS nv ON n.nav_id = nv.nav_id WHERE n.nav_id = ? ORDER BY n.weight ASC, n.nid ASC",
            [$nav_id]
        );

        if (intval($result->count()) == 0) {
            $result = $db->query(
                "SELECT *, status as nav_status FROM " .
                    NAV_MENUS_TABLE .
                    " WHERE nav_id = ?",
                [$nav_id]
            );
        ...
        if (!empty($nav_menus)) {
            ...
            $args = [$nav_menus, 'nid', 'pid', $max_depth, $args];
            $menus_output .= call_user_func_array([$walker, "walk"], $args);
            //convert array menus to string
        }
        ...
        $args = (object) [
            ...
            'hide_content' => true,
            'before' =>
                '<input type="checkbox" class="styled" data-label="%2$s" id="checkbox-%1$d" value="%1$d" /> ',
            'after' => '',
        ];
        ...
        foreach (
            $nuke_categories_cacheData
            as $module => $nuke_categories_cacheData_val
        ) {
            foreach (
                $nuke_categories_cacheData_val
                as $key => &$nuke_categories_cacheData_value
            ) {
                $nuke_categories_cacheData_value[
                    'cattext'
                ] = category_lang_text(
                    $nuke_categories_cacheData_value['cattext']
                );
                ...
                " $module
										...
										<div class=\"beefup__body\">
											<ul class=\"categories_select checktree\" data-module=\"$module\">
												" .
                call_user_func_array([$walker, "walk"], $argsnew) .
                "
											</ul>
											<input data-label=\"" .
                _CHECKALL .
                "\" type=\"checkbox\" class=\"styled select-all\" data-element=\"#nav_cats_select-$module\">
											<div class=\"single-field\">
												<button class=\"add_menu\" data-action=\"categories\">" .
                _ADD .
                "</button>
											</div>
										</div>
        ...
        $itemoptionshtml =
            '<table class="product-table no-border id-form editoptions"><tr><th style=\"width:150px;\">' .
            _TITLE .
            '</th><td><input type="text" class="inp-form dd-itemOption" size="25" id="itemOption-title" /></td></tr><tr><th>' .
            _ADDRESS .
            '</th><td><input type="text" class="inp-form-ltr dd-itemOption" placeholder="http://" size="25" id="itemOption-url" /></td></tr><tr><th>' .
            _TARGET .
            '</th><td><input type="text" class="inp-form-ltr dd-itemOption" size="25" placeholder="' .
            _FOREXAMPLE .
            ' _target" id="itemOption-target" /></td></tr><tr><th> ' .
            _XHTML_FRIENDS_NETWORK .
            '</th><td><input type="text" class="inp-form dd-itemOption" size="25" placeholder="' .
            _FOREXAMPLE .
            ' friends" id="itemOption-xfn" /></td></tr><tr><th>' .
            _CUSTOMM_CLASSES .
            '</th><td><input type="text" class="inp-form-ltr dd-itemOption" size="25" id="itemOption-classes" /></td></tr><tr><th>' .
            _CUSTOM_STYLES .
            '</th><td><input type="text" class="inp-form-ltr dd-itemOption" size="25" id="itemOption-styles" /></td></tr><tr><th>' .
            _STATUS .
            '</th><td><select class="styledselect-select-dialog dd-itemOption" id="itemOption-status"><option value="1">' .
            _ACTIVE .
            '</option><option value="0">' .
            _INACTIVE .
            '</option></select></td></tr></table><button type="button" data-action="save">' .
            _SAVE .
            '</button>';
        ...
        $pagetitle =
            $nav_id == 0 ? _ADD_NEW_NAV : sprintf(_EDIT_NAV, $nav_title);
        $hooks->add_filter("set_page_title", function () use ($pagetitle) {
            return ["nav_menus_admin" => $pagetitle];
        });
        ...
        if (_DIRECTION == 'rtl') {
            $contents .=
                "<link rel=\"stylesheet\" href=\"admin/template/css/jquery.nestable-rtl.css\" type=\"text/css\" media=\"all\" />";
        }
        $contents .=
            "
		<style>
			...
			.categories_select {background:#eee;padding:10px;list-style:none;}
			.sub-menu {list-style:none;margin-" .
            _TEXTALIGN1 .
            ":5px;}
		</style>
		<table width=\"100%\" class=\"product-table no-border\">
            ...
            "</th>
				<td align=\"" .
            _TEXTALIGN1 .
            "\"><input type=\"text\" id=\"nav_title\" value=\"$nav_title\" class=\"inp-form\" /></td>
			</tr>";
                ...
                if ($languageslist != "") {
                    ...
                    $contents .=
                        ...
                        "</th>
							<td align=\"" .
                        _TEXTALIGN1 .
                        "\"><input type=\"text\" class=\"nav_titles inp-form\" data-language=\"$languageslist\" value=\"" .
                        (isset($lang_nav_title[$languageslist])
                            ? $lang_nav_title[$languageslist]
                            : "") .
                        "\" /></td>
						</tr>";
                }
        ...
        $contents .=
            ...
            "</th>
				<td align=\"" .
            _TEXTALIGN1 .
            "\"><select id=\"nav_location\" class=\"styledselect-select\">";
        ...
        $contents .=
            "<input type=\"radio\" value=\"1\" name=\"nav_status\" class=\"styled nav_status\" data-label=\"" .
            _ACTIVE .
            "\" $check1 />
				<input type=\"radio\" value=\"0\" name=\"nav_status\" class=\"styled nav_status\" data-label=\"" .
            _INACTIVE .
            "\" $check2 />
				</td>
			</tr>
		...
		<table width=\"100%\" class=\"id-form\">
            ...
            _CLOSE_ALL .
            "</button>
									<button type=\"button\" data-action=\"save-nav\">" .
            _SAVECHANGES .
            "</button>
								</menu>
							</th>
							...
							<tr>
								<td>
									<div class=\"dd\" id=\"nestable\">
										<ol class=\"dd-list\">
											" .
            ($nav_id != 0 && !empty($nav_menus) && $menus_output != ''
                ? $menus_output
                : "") .
            "
										</ol>
    ...
    $nav_fields = isset($nav_fields) ? $nav_fields : [];
    $nav_menu_fields = isset($nav_menu_fields) ? $nav_menu_fields : [];
