    ...
    function build_form($request_data, $echo = true)
    {
        ...
        foreach ($this->inputs as $filed_key => $val):
            ...
            switch ($val['type']) {
                ...
                case 'select':
                    ...
                    foreach ($val['options'] as $opt) {
                        ...
                        $opt_value = $opt[1];
                        $opt_text = $opt[2];
                        $opt_attrs = isset($opt['attrs']) ? $opt['attrs'] : [];
                        $opt_insert = '';
                        if (
                    ...
                    if (count($val['options']) > 0):
                        ...
                        foreach ($val['options'] as $opt) {
                            ...
                            if (!empty($opt_attrs)) {
                                foreach (
                                    $opt_attrs
                                    as $opt_attr_key => $opt_attr_val
                                ) {
                                    $end .=
                                        ' ' .
                                        $opt_attr_key .
                                        '="' .
                                        (defined($opt_attr_val)
                                            ? constant($opt_attr_val)
                                            : $opt_attr_val) .
                                        '"';
                                }
                            }
            ...
            if ($val['type'] === 'range' || $val['type'] === 'number') {
                $min_max_range .= !empty($val['min'])
                    ? ' min="' . $val['min'] . '"'
                    : '';
                $min_max_range .= !empty($val['max'])
                    ? ' max="' . $val['max'] . '"'
                    : '';
                $min_max_range .= !empty($val['step'])
                    ? ' step="' . $val['step'] . '"'
                    : '';
            }
            ...
            if ($val['type'] != 'hidden' && $val['type'] != 'html'):
                ...
                if (!empty($val['wrap_tag'])) {
                    ...
                    $wrap_before .=
                        count($val['wrap_class']) > 0
                            ? $this->_output_classes($val['wrap_class'])
                            : '';
                    $wrap_before .= !empty($val['wrap_style'])
                        ? ' style="' . $val['wrap_style'] . '"'
                        : '';
                    $wrap_before .= !empty($val['wrap_id'])
                        ? ' id="' . $val['wrap_id'] . '"'
                        : '';
                    $wrap_before .= '>';
                }
        ...
        if (!$this->has_submit && $this->form['add_submit']) {
            $output .=
                '<div class="form_field_wrap"><input type="submit" value="Submit" name="submit"></div>';
        }
