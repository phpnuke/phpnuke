...
class Hooks
{
    ...
    public function remove_filter(
        ...
        $function_to_remove = $this->_filter_build_unique_id(
            $function_to_remove
        );
    ...
    public function has_shortcode(string $content, string $tag): bool
    {
        ...
        if ($this->shortcode_exists($tag)) {
            ...
            foreach ($matches as $shortcode) {
                ...
                if (
                    !empty($shortcode[5]) &&
                    $this->has_shortcode($shortcode[5], $tag)
                ) {
                    return true;
                }
            }
    ...
    private function _do_shortcode_tag(array $m)
    {
        ...
        // self-closing tag
        return $m[1] .
            call_user_func(self::$shortcode_tags[$tag], $attr, null, $tag) .
            $m[6];
    }
