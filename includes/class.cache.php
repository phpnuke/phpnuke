...
class Cache {
	...
	public function retrieve($key, $timestamp = false)
	{
		if($this->isCached($key))
		{
			...
			if (!isset($this->cachedData[$key][$type])) return null; 
	...
	public function flush_caches()
	{
		...
		foreach($cache_files as $cache_file)
		{
			...
			if (!unlink($cachePath.$cache_file))
				throw new Exception("the cache file $cache_file can not be deleted.");
		}
	}
	...
	private function _loadCache()
	{
		...
		{
			$file = phpnuke_get_url_contents($this->getCacheDir(), true, false, true);
