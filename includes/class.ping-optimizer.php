...
class PingOptimizer
{
    ...
    var $future_pings = []; // List of future posts to be pinged
    ...
    function __construct()
    {
        ...
        $this->options =
            $nuke_configs['ping_options'] != ''
                ? phpnuke_unserialize($nuke_configs['ping_options'])
                : ['limit_ping' => 0, 'limit_number' => 1, 'limit_time' => 15];
        if ($cache->isCached("future_pings")) {
            $future_pings = $cache->retrieve("future_pings");
            $this->future_pings =
                $future_pings != ''
                    ? phpnuke_unserialize($cache->retrieve("future_pings"))
                    : [];
        } else {
            $this->future_pings = ['update_time' => _NOWTIME, 'posts' => []];
        ...
        $last_pings_update = isset($this->future_pings['update_time'])
            ? $this->future_pings['update_time']
            : _NOWTIME;
    ...
    function phpnuke_PingServices($post_id_title)
    {
        ...
        if (strpos($post_id_title, '~#') !== false) {
            ...
            $this->_post_url = !empty($this->_data)
                ? $this->_data['post_url']
                : '';
            $this->_poster = !empty($this->_data) ? $this->_data['poster'] : '';
            $this->_poster_ip = !empty($this->_data)
                ? $this->_data['poster_ip']
                : '';
        }
    ...
    function phpnuke_SendXmlrpc($server = '', $path = '')
    {
        ...
        $client->timeout = 3;
        $client->useragent .=
            ' -- PhpNuke-MTEdition/' . $nuke_configs['Version_Num'];
        ...
        $feedlink = LinkToGT(
            "index.php?modname=Feed&mode=rss" .
                (isset($this->_data['module_name']) &&
                $this->_data['module_name'] != ''
                    ? strtolower($this->_data['module_name'])
                    : "")
        );
    ...
    function phpnuke_Ping()
    {
        ...
        if ($this->ping_sites != "") {
            if ($this->active_pings == 1) {
                if (
                    $this->_data['status'] == 'publish' ||
                    $this->_data['status'] == 'future'
                ) {
                    $post_id_title =
                        intval($this->_ID) .
                        '~#' .
                        filter($this->_data['title'], "nohtml");
                    // if post_date is greater than current time/date then its a future post (don't ping it)

                    if ($this->_data['time'] > _NOWTIME) {
                        $this->future_pings['posts'][$this->_Module][
                            $this->_ID
                        ] = $this->_data;
                        $cache->store('future_pings', $this->future_pings);
                    } elseif (
                        ...
                        if (
                            isset($this->options['limit_ping']) &&
                            $this->options['limit_ping'] == 1
                        ) {
                            $this->future_pings['posts'][$this->_Module][
                                $this->_ID
                            ] = $this->_data;
    ...
    function phpnuke_FuturePing()
    {
        ...
        if (!empty($this->future_pings['posts'])) {
            $i = 0;
            foreach (
                $this->future_pings['posts']
                as $module_name => $pingdata
            ) {
                foreach ($pingdata as $id => $_data) {
                    ...
                    if (
                        ...
                        if ($i <= $this->options['limit_number']) {
                            unset(
                                $this->future_pings['posts'][$module_name][$id]
                            );
    ...
    function phpnuke_FuturePingDelete($module_folder, $id)
    {
        ...
        if (
            !empty($this->future_pings) &&
            isset($this->future_pings['posts'][$module_folder][$id])
        ) {
