...
class Ex_ipversionmismatch extends Exception
{
    public function __construct($message = null, $code = 0, $previous = null)
    {
        ...
        $this->message =
            __CLASS__ . " :[{$this->file}:{$this->line}] {$this->message}\n";
    }
}
...
class IPObj
{
    ...
    public function __construct($Ip)
    {
        ...
        elseif (filter_var($Ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
            ...
            throw new Ex_invalidipaddress(
                "Requires valid IPv4 or IPv6 address ({$Ip})"
            );
        }
        $this->toIntArray();
    }
    ...
    public function _Cmp($IPobj)
    {
        ...
        if (
            ...
            throw new Ex_ipversionmismatch(
                "Both IPs must be IPv4 or IPv6 ({$this}),({$IPobj})"
            );
        }
    }
    ...
    private function ipv4tov6()
    {
        ...
        if (!isset($this->IPv4Value)) {
            throw new Ex_invalidipv4address(
                "({$Ip}) Requires valid IPv4 address"
            );
        }
    ...
    private function expand()
    {
        ...
        if (strpos($Ip, '::') !== false) {
            $Ip = str_replace(
                '::',
                str_repeat(':0', 8 - substr_count($Ip, ':')) . ':',
                $Ip
            );
        }
        if (strpos($Ip, ':') === 0) {
...
class IpBlockList
{
    ...
    public function __construct(
        $iplist_array = [],
        $whitelistfile = '_whitelist.dat',
        $blacklistfile = '_blacklist.dat'
    ) {
        if (is_array($iplist_array) && !empty($iplist_array)) {
            $this->whitelistfile =
                isset($iplist_array['whitelistfile']) &&
                !empty($iplist_array['whitelistfile'])
                    ? $iplist_array['whitelistfile']
                    : "";
            $this->blacklistfile =
                isset($iplist_array['blacklistfile']) &&
                !empty($iplist_array['blacklistfile'])
                    ? $iplist_array['blacklistfile']
                    : "";
