...
class sql_inject
{
    ...
    function __construct(
        ...
        $this->urlRedirect =
            trim($urlRedirect) != '' && file_exists($urlRedirect)
                ? $urlRedirect
                : '';
        $this->bdestroy_session = $bdestroy_session;
        $this->rq = '';
        $this->banning_expire =
            $nuke_config['mtsn_block_ip_expire'] != 0
                ? _NOWTIME + $nuke_configs['mtsn_block_ip_expire']
                : 0;
    }
    ...
    function test($sRQ)
    {
        ...
        if (!is_admin()) {
            if ($nuke_configs['mtsn_string_filter'] == "1") {
                if ($this->_in_post('exec')) {
                    return $this->detect2('exec');
                }
                if ($this->_in_post('/*')) {
                    return $this->detect2('/*');
                }
                ...
                if (@stristr($sRQ, '/*') || stristr($sRQ, '/*')) {
                    return $this->detect();
                }
                if (@stristr($sRQ, '*') || stristr($sRQ, '*')) {
                    return $this->detect();
                }
                if (@stristr($sRQ, '.txt?') || stristr($sRQ, '.txt?')) {
                    return $this->detect();
                }
                if (@stristr($sRQ, '.gif?') || stristr($sRQ, '.gif?')) {
                    return $this->detect();
                }
                if (@stristr($sRQ, '..') || stristr($sRQ, '..')) {
                    return $this->detect();
            ...
            if ($nuke_configs['mtsn_html_filter'] == "1") {
                ...
                if (@stristr($sRQ, '<script>') || stristr($sRQ, '<script>')) {
                    return $this->detect();
                }
            }
            /////////////////////////// End Html Filter///////////////////////
            ...
            if ($nuke_configs['mtsn_injection_filter'] == "1") {
                if ($this->_in_post('union')) {
                    return $this->detect2('union');
                }
                if (@stristr($sRQ, "exec") || @stristr($sRQ, "cmd")) {
                    return $this->detect();
                }
                if (@stristr($sRQ, 'concat') || stristr($sRQ, 'concat')) {
                    return $this->detect();
                }
                if (
                    @stristr($sRQ, 'INSERT INTO') ||
                    stristr($sRQ, 'INSERT INTO')
                ) {
                    return $this->detect();
                }
                if (@stristr($sRQ, 'UNION') || stristr($sRQ, 'UNION')) {
                    return $this->detect();
                }
            }
        }
        ...
        if (
            @preg_match(
                "/[a-z0-9]+ +between +[a-z0-9]+ +and +[a-z0-9]+/",
                $sConditions,
                $matches
            )
        ) {
            ...
            if (
                strcasecmp($Evaluate, $Temp[0]) > 0 &&
                strcasecmp($Evaluate, $Temp[1]) < 0 &&
                $this->_in_post($matches[0])
            ) {
                return $this->detect();
            }
        }
    ...
    function test_badw($match)
    {
        if (
            isset($match[2]) &&
            (str_replace(" ", "", $match[2]) == 'true--' or
                str_replace(" ", "", $match[2]) == 'true')
        ) {
            return "sql injection";
        } elseif (
            isset($match[6]) &&
            isset($match[9]) &&
            str_replace(" ", "", $match[6]) == str_replace(" ", "", $match[9])
        ) {
            if ($match[6] != "" && $match[9] != "") {
                return "sql injection";
            }
    ...
    function protect_sql_injection($param)
    {
        foreach ($param as $param_name => $param_val) {
            if (@is_array($param_val)) {
                ...
                $badw = preg_replace_callback(
                    "#((\strue\s?([\-]*))|((or\s|or\t|OR\s|OR\t|\sOR\s|\tOR\t|\sor\s|\tor\t|\s|\t)?([a-zA-Z0-9]*)(\s|\t)?=(\s|\t)?([a-zA-Z0-9]*)))#i",
                    [$this, 'test_badw'],
                    str_replace("'", "", $param_val)
                );
    ...
    function detect()
    {
        ...
        $rq = mres($this->rq);
        $this->servAd = getenv("SERVER_NAME");
        $result = $db->table(MTSN_TABLE)->insert([
            'server' => $this->servAd,
            'ip' => $ipp,
            'time' => $time,
            'method' => $rq,
        ...
        if ($this->ipbanx == "1") {
            ...
            if ($ippx != "127.0.0.1") {
                ...
                $result = $db->table(MTSN_IPBAN_TABLE)->insert([
                    'blocker' => 'mtsn system',
                    'ipaddress' => $ippx,
                    'reason' => $rq,
                    'time' => $time,
