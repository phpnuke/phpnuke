...
class csrfProtector
{
	...
	public static function init($length = null, $action = null)
	{
		...
		if (!file_exists(INCLUDE_PATH."/csrfp/libs/config.php")) {
		...
		self::$config = include(INCLUDE_PATH."/csrfp/libs/config.php");
	...
	{
		$configLastModified = filemtime(INCLUDE_PATH."/csrfp/libs/config.php");
		if (file_exists(self::$config['jsPath'])) {
			$jsFileLastModified = filemtime(self::$config['jsPath']);
	...
	{
		if (!file_exists(INCLUDE_PATH."/csrfp/libs/csrf/csrfpJsFileBase.php")) {
			throw new baseJSFileNotFoundExceptio("base js file needed to create js file not found at " .__DIR__);
			return;;
		}

		$jsFile = file_get_contents(INCLUDE_PATH."/csrfp/libs/csrf/csrfpJsFileBase.php");
		$arrayStr = '';
		...
		file_put_contents(self::$config['jsPath'], $jsFile);
	}
	 ...
	 */
	public static function authorisePost($only_get=false)
	{
		global $pn_Sessions, $nuke_configs;
		...
		if($must_validate)
		{
			...
			$csrfp_session = $pn_Sessions->get(self::$config['CSRFP_TOKEN'], false);
			
			if ($csrfp_session == '')
				self::failedValidationAction();
				
			$csrfp_session = phpnuke_unserialize($csrfp_session);

			foreach($csrfp_session as $token => $token_data)
			{
