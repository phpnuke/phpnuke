...
if(! defined('ALLOW_ALL_EXTERNAL_SITES') ) 	define ('ALLOW_ALL_EXTERNAL_SITES', $ALLOW_ALL_EXTERNAL_SITES);				// Less secure. 
...
if(! defined('DEFAULT_A') )					define ('DEFAULT_A', 't');								// Default align value. Allows overrid in timthumb-config.php
...
class timthumb {
		...
		if(file_exists($this->cachefile)){
			...
			if($this->isURL){
				...
				if(filesize($this->cachefile) < 1){
					...
					if(_NOWTIME - @filemtime($this->cachefile) > WAIT_BETWEEN_FETCH_ERRORS){
	...
	protected function cleanCache(){
		...
		if(@filemtime($lastCleanFile) < (_NOWTIME - FILE_CACHE_TIME_BETWEEN_CLEANS) ){ //Cache was last cleaned more than 1 day ago
