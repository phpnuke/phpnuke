...
if (!defined('ALLOW_ALL_EXTERNAL_SITES')) {
    define('ALLOW_ALL_EXTERNAL_SITES', $ALLOW_ALL_EXTERNAL_SITES);
} // Less secure.
...
if (!defined('DEFAULT_A')) {
    define('DEFAULT_A', 't');
} // Default align value. Allows overrid in timthumb-config.php
...
class timthumb
{
    ...
    {
        global $ALLOWED_SITES, $visitor_ip;
        $this->startTime = microtime(true);
        date_default_timezone_set('UTC');
        $this->remote_addr = $visitor_ip;
        $this->debug(
            1,
            "Starting new request from " .
                $this->getIP() .
                " to " .
                $_SERVER['REQUEST_URI']
        );
        ...
        if (
            ...
            header('Expires: ' . gmdate('D, d M Y H:i:s', _NOWTIME));
            echo $imgData;
            return false;
            exit(0);
        }
        ...
        if ($this->isURL) {
            ...
            if ($this->param('webshot')) {
                if (WEBSHOT_ENABLED) {
                    $this->debug(
                        3,
                        "webshot param is set, so we're going to take a webshot."
                    );
                    $this->serveWebshot();
                } else {
                    $this->error(
                        "You added the webshot parameter but webshots are disabled on this server. You need to set WEBSHOT_ENABLED == true to enable webshots."
                    );
                }
            } else {
                $this->debug(
        ...
        if (file_exists($this->cachefile)) {
            ...
            if ($this->isURL) {
                ...
                if (filesize($this->cachefile) < 1) {
                    ...
                    if (
                        _NOWTIME - @filemtime($this->cachefile) >
    ...
    protected function serveErrors()
    {
        ...
        $html .= '</ul>';
        echo '<h1>A TimThumb error has occured</h1>The following error(s) occured:<br />' .
            $html .
            '<br />';
    ...
    protected function cleanCache()
    {
        ...
        $lastCleanFile =
            $this->cacheDirectory . '/timthumb_cacheLastCleanTime.touch';
        ...
        if (
            @filemtime($lastCleanFile) <
            _NOWTIME - FILE_CACHE_TIME_BETWEEN_CLEANS
        ) {
            ...
            if ($files) {
                $timeAgo = _NOWTIME - FILE_CACHE_MAX_FILE_AGE;
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        $align = $this->cropTop ? 't' : $this->param('a', DEFAULT_A);
        ...
        if (
            preg_match('/^image\/png$/i', $mimeType) &&
            !PNG_IS_TRANSPARENT &&
            $canvas_trans
        ) {
        ...
        if (
            (IMAGETYPE_PNG == $origType || IMAGETYPE_GIF == $origType) &&
    ...
    protected function getLocalImagePath($src)
    {
        ...
        if (strstr($_SERVER['SCRIPT_FILENAME'], ':')) {
            $sub_directories = explode(
                '\\',
                str_replace($this->docRoot, '', $_SERVER['SCRIPT_FILENAME'])
            );
        } else {
            $sub_directories = explode(
                '/',
    ...
    protected function serveWebshot()
    {
        ...
        $instr =
            "Please follow the instructions at http://code.google.com/p/timthumb/ to set your server up for taking website screenshots.";
    ...
    protected function sendImageHeaders($mimeType, $dataSize)
    {
        ...
        $gmdate_expires =
            gmdate('D, d M Y H:i:s', strtotime('now +10 days')) . ' GMT';
    ...
    protected function debug($level, $msg)
    {
        if (DEBUG_ON && $level <= DEBUG_LEVEL) {
            ...
            error_log(
                "TimThumb Debug line " . __LINE__ . " [$execTime : $tick]: $msg"
            );
        }
    }
    ...
    protected function setMemoryLimit()
    {
        ...
        if ($inibytes < $ourbytes) {
            ...
            $this->debug(
                3,
                "Not adjusting memory size because the current setting is " .
                    $inimem .
                    " and our size of " .
                    MEMORY_LIMIT .
                    " is smaller."
