...
class Walker_nav_categories extends Walker
{
    ...
    public function start_el(
        ...
        $class_names = $class_names
            ? ' class="' . filter($class_names, "nohtml") . '"'
            : '';
        ...
        $item_output =
            $before .
            (isset($args->hide_content) && $args->hide_content
                ? ''
                : $cattext) .
            $args->after;
...
class Walker_nav_menus extends Walker
{
    public function start_lvl(&$output, $depth = 0, $args = [])
    {
        ...
        $output .=
            ...
            $args->list_type .
            " class=\"" .
            (isset($args->list_class) && $args->list_class != ''
                ? $args->list_class
                : "") .
            "\">\n";
    }
    ...
    public function start_el(
        ...
        $classes = empty($element->attributes->classes)
            ? ''
            : $element->attributes->classes;
        $styles = empty($element->attributes->styles)
            ? ''
            : $element->attributes->styles;
        $xfn = empty($element->attributes->xfn)
            ? ''
            : $element->attributes->xfn;
        $target = empty($element->attributes->target)
            ? ''
        ...
        $output .=
            $indent . '<li' . $class_names . $styles_codes . $idrel . '>';
