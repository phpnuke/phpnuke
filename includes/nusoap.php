                ...
                if (
                    ...
                    if (is_array($val) && count($val) > 0) {
                        ...
                        if (count($array_types) > 1) {
                            ...
                            isset($tt) &&
                            isset($this->typemap[$this->XMLSchemaVersion][$tt])
                        ) {
                            if ($tt == 'integer') {
                                $tt = 'int';
                            }
...
function timestamp_to_iso8601($timestamp, $utc = true)
{
    ...
    if ($pos !== false) {
        if (strlen($datestr) == $pos + 5) {
            $datestr =
                substr($datestr, 0, $pos + 3) . ':' . substr($datestr, -2);
        }
    }
    if ($utc) {
        $pattern =
...
class nusoap_xmlschema extends nusoap_base
{
        ...
        switch ($name) {
            ...
            case 'attribute': // complexType attribute
                ...
                if (
                    ...
                    $this->complexTypes[$this->currentComplexType]['phpType'] =
                        'array';
            ...
            case 'complexType':
                ...
                if (isset($attrs['name'])) {
                    ...
                    $this->xdebug(
                        'processing named complexType ' . $attrs['name']
                    );
            ...
            case 'element':
                ...
                if (isset($attrs['type'])) {
                    ...
                    if (
                        ...
                        $this->xdebug(
                            'arrayType for unusual array is ' . $attrs['type']
                        );
                        $this->complexTypes[$this->currentComplexType][
                            'arrayType'
                        ] = $attrs['type'];
                    ...
                    $this->xdebug(
                        "processing element as ref to " . $attrs['ref']
                    );
                    $this->currentElement = "ref to " . $attrs['ref'];
                    $ename = $this->getLocalPart($attrs['ref']);
                } else {
                    $type = $this->CreateTypeName(
                        $this->currentComplexType . '_' . $attrs['name']
                    );
                    $this->xdebug(
                ...
                if (isset($ename) && $this->currentComplexType) {
                    ...
                    $this->complexTypes[$this->currentComplexType]['elements'][
                        $ename
                    ] = $attrs;
                } elseif (!isset($attrs['ref'])) {
            ...
            case 'restriction': // simpleType, simpleContent or complexContent value restriction
                ...
                if ($this->currentSimpleType) {
                    $this->simpleTypes[$this->currentSimpleType]['type'] =
                        $attrs['base'];
                } elseif ($this->currentComplexType) {
                    $this->complexTypes[$this->currentComplexType][
                        'restrictionBase'
            ...
            case 'simpleType':
                ...
                if (isset($attrs['name'])) {
                    $this->xdebug(
                        "processing simpleType for name " . $attrs['name']
                    );
                    ...
                    $name = $this->CreateTypeName(
                        $this->currentComplexType . '_' . $this->currentElement
                    );
                    $this->xdebug(
                    ...
                    $this->simpleTypes[$this->currentSimpleType]['phpType'] =
                        'scalar';
                }
                break;
    ...
    function schemaEndElement($parser, $name)
    {
        ...
        if ($name == 'complexType') {
            ...
            $this->xdebug(
                $this->varDump($this->complexTypes[$this->currentComplexType])
            );
        ...
        if ($name == 'simpleType') {
            ...
            $this->xdebug(
                $this->varDump($this->simpleTypes[$this->currentSimpleType])
            );
    ...
    function serializeSchema()
    {
        ...
        if (isset($this->attributes) && count($this->attributes) > 0) {
            foreach ($this->attributes as $attr => $aParts) {
                $xml .=
                    " <$schemaPrefix:attribute name=\"$attr\" type=\"" .
        ...
        foreach (
            array_diff($this->usedNamespaces, $this->enclosingNamespaces)
            as $nsp => $ns
        ) {
            $el .= " xmlns:$nsp=\"$ns\"";
        }
            ...
            if ($typeDef['phpType'] == 'struct') {
                ...
                foreach ($typeDef['elements'] as $child => $childDef) {
					...
					<tr><td align='right'>$childDef[name] (type: " .
                        $this->getLocalPart($childDef['type']) .
                        "):</td>
    ...
    function addElement($attrs)
    {
        if (!$this->getPrefix($attrs['type'])) {
            $attrs['type'] =
                $this->schemaTargetNamespace . ':' . $attrs['type'];
        }
        $this->elements[$attrs['name']] = $attrs;
        $this->elements[$attrs['name']]['typeClass'] = 'element';
...
class soap_transport_http extends nusoap_base
{
    ...
    function io_method()
    {
        ...
        if (
            ($this->scheme == 'http' || $this->scheme == 'ssl') &&
            $this->authtype != 'ntlm' &&
    ...
    function connect($connection_timeout = 0, $response_timeout = 30)
    {
        ...
        if ($this->io_method() == 'socket') {
            ...
            if (!$this->fp) {
                ...
                if ($this->errno) {
                    $msg .=
                        ', Error (' . $this->errno . '): ' . $this->error_str;
                } else {
                    $msg .=
    ...
    {
        // build cookie string
        $cookie_str = $this->getCookiesForRequest(
            $cookies,
    ...
    function getResponse()
    {
        ...
        if ($this->io_method() == 'socket') {
            ...
            if (
                isset($this->incoming_headers['transfer-encoding']) &&
                strtolower($this->incoming_headers['transfer-encoding']) ==
                    'chunked'
            ) {
            ...
            $this->debug(
                'received a total of ' .
                    strlen($this->incoming_payload) .
                    ' bytes of data from server'
            );
            // close filepointer
            if (
                (isset($this->incoming_headers['connection']) &&
                    strtolower($this->incoming_headers['connection']) ==
                        'close') ||
                !$this->persistentConnection ||
                feof($this->fp)
            ) {
        ...
        if (
            isset($this->incoming_headers['location']) &&
        ...
        // see if we need to resend the request with http digest authentication
        if (
            isset($this->incoming_headers['www-authenticate']) &&
            $http_status == 401
        ) {
            $this->debug(
            ...
            if (
                strstr($this->incoming_headers['www-authenticate'], "Digest ")
            ) {
                $this->debug('Server wants digest authentication');
                // remove "Digest " from our elements
                $digestString = str_replace(
                    'Digest ',
                    '',
                    $this->incoming_headers['www-authenticate']
                );
                ...
                foreach ($digestElements as $val) {
                    ...
                    $digestRequest[$tempElement[0]] = str_replace(
                        "\"",
                        '',
                        $tempElement[1]
                    );
        ...
        if (
            isset($this->incoming_headers['content-encoding']) &&
            $this->incoming_headers['content-encoding'] != ''
        ) {
            if (
                strtolower($this->incoming_headers['content-encoding']) ==
                    'deflate' ||
                strtolower($this->incoming_headers['content-encoding']) ==
                    'gzip'
            ) {
                ...
                if (function_exists('gzinflate')) {
                    ...
                    $datalen = strlen($data);
                    if (
                        $this->incoming_headers['content-encoding'] == 'deflate'
                    ) {
...
class nusoap_server extends nusoap_base
{
        ...
        if (isset($debug)) {
            ...
            foreach ($qs as $v) {
                if (substr($v, 0, 6) == 'debug=') {
                    $this->debug(
                        "In nusoap_server, set debug_flag=" .
                            substr($v, 6) .
    ...
    function service($data)
    {
        ...
        $this->debug(
            "In service, request method=$rm query string=$qs strlen(\$data)=" .
                strlen($data)
        );
        if ($rm == 'POST') {
    ...
    function parse_http_headers()
    {
        ...
        if (function_exists('getallheaders')) {
            ...
            if (
                ...
                strpos($this->headers['content-type'], '=')
            ) {
                $enc = str_replace(
                    '"',
                    '',
                    substr(strstr($this->headers["content-type"], '='), 1)
                );
    ...
    function parse_request($data = '')
    {
        ...
        if (
            ...
            $this->debug(
                'got content encoding: ' . $this->headers['content-encoding']
            );
            if (
    ...
    function invoke_method()
    {
        ...
        if ($this->wsdl) {
            if (
                ...
                $this->debug(
                    'in invoke_method, found WSDL operation=' .
                        $this->methodname
                );
                $this->appendDebug('opData=' . $this->varDump($this->opData));
            } elseif (
                $this->opData = $this->wsdl->getOperationDataForSoapAction(
                    $this->SOAPAction
                )
            ) {
                // Note: hopefully this case will only be used for doc/lit, since rpc services should have wrapper element
                $this->debug(
                    'in invoke_method, found WSDL soapAction=' .
                        $this->SOAPAction .
                        ' for operation=' .
                        $this->opData['name']
                );
                ...
                $this->debug(
                    'in invoke_method, no WSDL for operation=' .
                        $this->methodname
                );
                $this->fault(
                    'SOAP-ENV:Client',
        ...
        if (
            strlen($delim) > 0 &&
            substr_count($this->methodname, $delim) == 1
        ) {
            $try_class = substr(
                $this->methodname,
                0,
                strpos($this->methodname, $delim)
            );
            if (class_exists($try_class)) {
                ...
                $method = substr(
                    $this->methodname,
                    strpos($this->methodname, $delim) + strlen($delim)
                );
                $this->debug(
        ...
        $this->debug(
            "in invoke_method, called method $this->methodname, received data of type " .
    ...
    {
        $this->debug(
            'Entering serialize_return methodname: ' .
                $this->methodname .
                ' methodURI: ' .
                $this->methodURI
        );
        ...
        if (
            ...
            $this->debug(
                'got a(n) ' . gettype($this->methodreturn) . ' from method'
            );
            ...
            if ($this->wsdl) {
                ...
                $return_val = $this->wsdl->serializeRPCParameters(
                    $this->methodname,
                    'output',
                    $opParams
                );
        ...
        if ($this->wsdl) {
            $this->debug(
                'have WSDL for serialization: style is ' .
                    $this->opData['style']
            );
            if ($this->opData['style'] == 'rpc') {
                $this->debug(
                    'style is rpc for serialization: use is ' .
    ...
    function configureWSDL(
        ...
        ][0]->schemaTargetNamespace = $schemaTargetNamespace;
        $this->wsdl->schemas[$schemaTargetNamespace][0]->imports[
            'http://schemas.xmlsoap.org/soap/encoding/'
        ][0] = ['location' => '', 'loaded' => true];
        $this->wsdl->schemas[$schemaTargetNamespace][0]->imports[
...
class wsdl extends nusoap_base
{
    ...
    function fetchWSDL($wsdl)
    {
                ...
                foreach ($list as $xs) {
                    ...
                    foreach ($xs->imports as $ns2 => $list2) {
                        for ($ii = 0; $ii < count($list2); $ii++) {
                            if (!$list2[$ii]['loaded']) {
                                $this->schemas[$ns][$ns2]->imports[$ns2][$ii][
                                    'loaded'
                                ] = true;
        ...
        foreach ($this->bindings as $binding => $bindingData) {
            if (
                isset($bindingData['operations']) &&
                is_array($bindingData['operations'])
            ) {
                foreach ($bindingData['operations'] as $operation => $data) {
                    $this->debug('post-parse data gathering for ' . $operation);
                    $this->bindings[$binding]['operations'][$operation][
                        'input'
                    ] = isset(
                        $this->bindings[$binding]['operations'][$operation][
                            'input'
                        ...
                        ? array_merge(
                            ...
                            $this->portTypes[$bindingData['portType']][
                                $operation
                            ]['input']
                        )
                        : $this->portTypes[$bindingData['portType']][
                    ...
                    $this->bindings[$binding]['operations'][$operation][
                        'transport'
                    ] = isset($bindingData['transport'])
                        ? $bindingData['transport']
                        : '';
                    $this->bindings[$binding]['operations'][$operation][
                        'documentation'
                    ] = isset(
                        $this->portTypes[$bindingData['portType']][$operation][
                            'documentation'
                        ]
                    )
                        ? $this->portTypes[$bindingData['portType']][
    ...
    function parseWSDL($wsdl = '')
    {
        ...
        if (
            ...
            if (
                ...
                $path = isset($wsdl_props['host'])
                    ? $wsdl_props['host'] . ':' . $wsdl_props['path']
                    : $wsdl_props['path'];
            } else {
                $path = $wsdl;
            }
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            if (preg_match('/:/', $name)) {
                ...
                // get ns
                $namespace = isset($this->namespaces[$prefix])
                    ...
                    if ($name == 'part') {
                        if (isset($attrs['type'])) {
                            $this->debug(
                                "msg " .
                                    $this->currentMessage .
                                    ": found part (with type) $attrs[name]: " .
                                    implode(',', $attrs)
                            );
                        ...
                        if (isset($attrs['element'])) {
                            $this->debug(
                                "msg " .
                                    $this->currentMessage .
                                    ": found part (with element) $attrs[name]: " .
                                    implode(',', $attrs)
                            );
                            $this->messages[$this->currentMessage][
                                $attrs['name']
                        ...
                        case 'operation':
                            ...
                            if (isset($attrs['parameterOrder'])) {
                                $this->portTypes[$this->currentPortType][
                                    $attrs['name']
                                ]['parameterOrder'] = $attrs['parameterOrder'];
                            }
                            break;
                        case 'documentation':
                    ...
                    switch ($name) {
                        ...
                        case 'operation':
                            if (isset($attrs['soapAction'])) {
                                $this->bindings[$this->currentBinding][
                                    'operations'
                                ][$this->currentOperation]['soapAction'] =
                                    $attrs['soapAction'];
                            }
                            if (isset($attrs['style'])) {
                                $this->bindings[$this->currentBinding][
                                    'operations'
                                ][$this->currentOperation]['style'] =
                                    $attrs['style'];
                            }
                            if (isset($attrs['name'])) {
                                ...
                                $this->bindings[$this->currentBinding][
                                    'operations'
                                ][$this->currentOperation]['binding'] =
                                    $this->currentBinding;
                                $this->bindings[$this->currentBinding][
                                    'operations'
                                ][$this->currentOperation]['endpoint'] = isset(
                                    $this->bindings[$this->currentBinding][
                                        'endpoint'
                                    ]
                                )
                ...
                case "binding":
                    if (isset($attrs['name'])) {
                        ...
                        if (strpos($attrs['name'], ':')) {
                            $this->currentBinding = $this->getLocalPart(
                                $attrs['name']
                            );
                        } else {
                        ...
                        $this->bindings[$this->currentBinding][
                            'portType'
                        ] = $this->getLocalPart($attrs['type']);
                        $this->debug(
                            "current binding: $this->currentBinding of portType: " .
                                $attrs['type']
                        );
                    }
                    break;
    ...
    function end_element($parser, $name)
    {
        ...
        if (
            ...
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
            $this->schemas[$this->currentSchema->schemaTargetNamespace][] =
                $this->currentSchema;
    ...
    {
        $pos = isset($this->depth_array[$this->depth])
            ? $this->depth_array[$this->depth]
            : 0;
    ...
    function getOperations($portName = '', $bindingType = 'soap')
    {
        ...
        foreach ($this->ports as $port => $portData) {
            $this->debug(
                "getOperations checking port $port bindingType " .
                    ...
                    foreach ($msgParts as $partName => $partType) {
                        ...
                        if (strpos($partType, ':')) {
                            $typePrefix = $this->getPrefixFromNamespace(
                                $this->getPrefix($partType)
                            );
                        } elseif (
                            isset(
                                $this->typemap[$this->namespaces['xsd']][
                                    $partType
                                ]
                            )
                        ) {
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    $binding_xml .=
                        ...
                        '"/>';
                    if (
                        isset($opParts['input']['encodingStyle']) &&
                    ...
                    $binding_xml .=
                        ...
                        $opParts['input']['use'] .
                        '" namespace="' .
                        $opParts['input']['namespace'] .
                        '"' .
                        $enc_style .
                        '/></input>';
                    if (
                        isset($opParts['output']['encodingStyle']) &&
    ...
    function serializeRPCParameters(
        ...
        if (!($opData = $this->getOperationData($operation, $bindingType))) {
            $this->debug(
                'Unable to retrieve WSDL data for operation: ' .
                    $operation .
                    ' bindingType: ' .
                    $bindingType
            );
        ...
        if (
            $direction == 'input' &&
            isset($opData['output']['encodingStyle']) &&
        ...
        if (
            ...
            if (is_array($parameters)) {
                $parametersArrayType = $this->isArraySimpleOrStruct(
                    $parameters
                );
                ...
                if (
                    ...
                    if (
                        $direction == 'output' &&
                        $parametersArrayType == 'arraySimple' &&
                        $parameter_count == 1
                    ) {
                    ...
                    if (
                        ($parametersArrayType == 'arrayStruct' ||
                            $parameter_count == 0) &&
                ...
                foreach ($parts as $name => $type) {
                    ...
                    if (
                        isset($opData[$direction]['encodingStyle']) &&
    ...
    function serializeParameters($operation, $direction, $parameters)
    {
        ...
        if (
            ...
            $this->debug("use=$use");
            $this->debug(
                'got ' . count($opData[$direction]['parts']) . ' part(s)'
            );
            if (is_array($parameters)) {
                $parametersArrayType = $this->isArraySimpleOrStruct(
                    $parameters
    ...
    function serializeType(
        ...
        if (strpos($type, ':')) {
            ...
            if (
                ...
                if (
                    ($uqType == 'long' || $uqType == 'unsignedLong') &&
        ...
        $phpType = $typeDef['phpType'];
        $this->debug(
            "in serializeType: uqType: $uqType, ns: $ns, phptype: $phpType, arrayType: " .
                (isset($typeDef['arrayType']) ? $typeDef['arrayType'] : '')
        );
        // if php type == struct, map value to the <all> element names
        if ($phpType == 'struct') {
            if (
                isset($typeDef['typeClass']) &&
            ...
            if (is_null($value)) {
                if ($use == 'literal') {
                    ...
                    $xml =
                        "<$elementName$elementNS xsi:nil=\"true\" xsi:type=\"" .
                        $this->getPrefixFromNamespace($ns) .
                        ":$uqType\"/>";
                }
                $this->debug("in serializeType: returning: $xml");
            ...
            if (is_array($value)) {
                $elementAttrs = $this->serializeComplexTypeAttributes(
                    $typeDef,
                    $value,
                    $ns,
                    $uqType
                ...
                if ($use == 'literal') {
                    if ($forceType) {
                        $xml =
                            "<$elementName$elementNS$elementAttrs xsi:type=\"" .
                            $this->getPrefixFromNamespace($ns) .
                            ":$uqType\">";
                    } else {
                ...
                if (
                    ...
                    // complexContent
                    $xml .= $this->serializeComplexTypeElements(
                        $typeDef,
                        $value,
                        $ns,
                        $uqType,
            ...
            if (is_array($value) && sizeof($value) >= 1) {
                ...
                foreach ($value as $k => $v) {
                    ...
                    if (
                        !in_array(
                            $typeDef['arrayType'],
                            $this->typemap['http://www.w3.org/2001/XMLSchema']
                        )
                    ) {
                        $contents .= $this->serializeType(
                            'item',
                            $typeDef['arrayType'],
                            $v,
                            $use
    ...
    function serializeComplexTypeAttributes($typeDef, $value, $ns, $uqType)
    {
        ...
        if (isset($typeDef['extensionBase'])) {
            ...
            if ($typeDefx = $this->getTypeDef($uqTypex, $nsx)) {
                ...
                $xml .= $this->serializeComplexTypeAttributes(
                    $typeDefx,
                    $value,
                    $nsx,
                    $uqTypex
        ...
        if (isset($typeDef['attrs']) && is_array($typeDef['attrs'])) {
            ...
            foreach ($typeDef['attrs'] as $aName => $attrs) {
                ...
                if ($xname) {
                    $xml .=
                        " $aName=\"" .
                        $this->expandEntities($xvalue[$xname]) .
                        "\"";
                }
            }
        } else {
        ...
        if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
            ...
            foreach ($typeDef['elements'] as $eName => $attrs) {
                ...
                if (
                    ...
                    if (
                        isset($attrs['minOccurs']) &&
                        $attrs['minOccurs'] != '0'
                    ) {
                        $this->debug(
                            "apparent error: no value provided for element $eName with minOccurs=" .
                                $attrs['minOccurs']
                        );
                    }
                    // do nothing
    ...
    function addComplexType(
        ...
        $restrictionBase = strpos($restrictionBase, ':')
            ? $this->expandQname($restrictionBase)
            : $restrictionBase;
        $arrayType = strpos($arrayType, ':')
            ? $this->expandQname($arrayType)
            : $arrayType;
        $typens = isset($this->namespaces['types'])
    ...
    function addSimpleType(
        ...
        $typens = isset($this->namespaces['types'])
            ? $this->namespaces['types']
            : $this->namespaces['tns'];
        $this->schemas[$typens][0]->addSimpleType(
            $name,
            $restrictionBase,
            $typeClass,
            $phpType,
    ...
    function addOperation(
        ...
        if ($in) {
            foreach ($in as $pName => $pType) {
                if (strpos($pType, ':')) {
                    $pType =
                        $this->getNamespaceFromPrefix(
                            $this->getPrefix($pType)
                        ) .
                        ":" .
                        $this->getLocalPart($pType);
                }
...
class nusoap_parser extends nusoap_base
{
        ...
        if (!empty($xml)) {
            ...
            if ($pos_xml !== false) {
                $xml_decl = substr(
                    $xml,
                    $pos_xml,
                    strpos($xml, '?>', $pos_xml + 2) - $pos_xml + 1
                );
                if (
                    preg_match(
                        "/encoding=[\"']([^\"']*)[\"']/",
                    ...
                    if (strtoupper($xml_encoding) != $encoding) {
                        $err =
                            "Charset from HTTP Content-Type '" .
                            $encoding .
            ...
            if (!xml_parse($this->parser, $xml, true)) {
                ...
                $this->debug(
                    'parsed successfully, found root struct: ' .
                        $this->root_struct .
                        ' of name ' .
    ...
    function start_element($parser, $name, $attrs)
    {
        ...
        if ($this->status == 'header') {
            ...
            $this->document .=
                "<" . (isset($prefix) ? $prefix . ':' : '') . "$name$attstr>";
        }
    }
    /**
     * end-element handler
    ...
    function end_element($parser, $name)
    {
        ...
        if (isset($this->body_position) && $pos > $this->body_position) {
            ...
            if (isset($this->message[$pos]['attrs']['href'])) {
                ...
                if (
                    isset($this->message[$pos]['nil']) &&
                    $this->message[$pos]['nil']
                ) {
                    $this->message[$pos]['xattrs']['!'] = null;
                } elseif (
                    isset($this->message[$pos]['cdata']) &&
                    ...
                    if (isset($this->message[$pos]['type'])) {
                        $this->message[$pos]['xattrs'][
                            ...
                            isset($this->message[$pos]['type_namespace'])
                                ? $this->message[$pos]['type_namespace']
                                : ''
                        );
                    } else {
                        ...
                        if (
                            isset($this->message[$parent]['type']) &&
    ...
    function buildVal($pos)
    {
        ...
        if ($this->message[$pos]['children'] != '') {
            ...
            // md array
            if (
                isset($this->message[$pos]['arrayCols']) &&
                ...
                $this->debug(
                    'in buildVal, adding array ' . $this->message[$pos]['name']
                );
                foreach ($children as $child_pos) {
                ...
                $this->message[$pos]['type'] == 'Map' &&
                $this->message[$pos]['type_namespace'] ==
                    'http://xml.apache.org/xml-soap'
            ) {
                $this->debug(
                    'in buildVal, Java Map ' . $this->message[$pos]['name']
                );
                foreach ($children as $child_pos) {
                ...
                $this->debug(
                    'in buildVal, adding Java Vector or generic compound type ' .
                        $this->message[$pos]['name']
                );
                if (
                    $this->message[$pos]['type'] == 'Vector' &&
                    $this->message[$pos]['type_namespace'] ==
                        'http://xml.apache.org/xml-soap'
                ) {
                    $notstruct = 1;
...
class nusoap_client extends nusoap_base
{
    ...
    function call(
        ...
        $this->debug(
            'SOAP message length=' .
                strlen($soapmsg) .
                ' contents (max 1000 bytes)=' .
                substr($soapmsg, 0, 1000)
        );
        // send
        $return = $this->send(
            $this->getHTTPBody($soapmsg),
            $soapAction,
            $this->timeout,
            $this->response_timeout
        );
        if ($errstr = $this->getError()) {
            ...
            if (is_array($return) && isset($return['faultcode'])) {
                ...
                $this->setError(
                    $return['faultcode'] . ': ' . $return['faultstring']
                );
                $this->fault = true;
    ...
    function checkWSDL()
    {
        ...
        if ($errstr = $this->wsdl->getError()) {
            ...
            $this->operations = $this->wsdl->getOperations(
                $this->portName,
                'soap'
            )
        ) {
    ...
    function parseResponse($headers, $data)
    {
        ...
        if (!strstr($headers['content-type'], 'text/xml')) {
            $this->setError(
                'Response not of type text/xml: ' . $headers['content-type']
            );
            return false;
        }
        if (strpos($headers['content-type'], '=')) {
        ...
        foreach ($this->operations as $operation => $opData) {
            if ($operation != '') {
                ...
                if (sizeof($opData['input']['parts']) > 0) {
                    ...
                    $paramArrayStr = substr(
                        $paramArrayStr,
                        0,
                        strlen($paramArrayStr) - 2
                    );
                    $paramCommentStr = substr(
                        $paramCommentStr,
                        0,
                        strlen($paramCommentStr) - 2
                    );
                } else {
                    $paramStr = '';
                    $paramArrayStr = '';
                ...
                $evalStr .=
                    "// $paramCommentStr
	function " .
                    str_replace('.', '__', $operation) .
                    "($paramStr) {
		\$params = array($paramArrayStr);
		return \$this->call('$operation', \$params, '" .
                    $opData['namespace'] .
                    "', '" .
                    (isset($opData['soapAction'])
                        ? $opData['soapAction']
                        : '') .
                    "');
	}
    ...
    function checkCookies()
    {
        ...
        $this->debug(
            'checkCookie: check ' . sizeof($this->cookies) . ' cookies'
        );
        ...
        $this->debug(
            'checkCookie: ' . sizeof($this->cookies) . ' cookies left in array'
        );
        return true;
    }
    /**
    ...
    function UpdateCookies($cookies)
    {
        ...
        foreach ($cookies as $newCookie) {
            ...
            for ($i = 0; $i < count($this->cookies); $i++) {
                ...
                $found = true;
                $this->debug(
                    'Update cookie ' . $newName . '=' . $newCookie['value']
                );
                break;
            }
            if (!$found) {
                $this->debug(
                    'Add cookie ' . $newName . '=' . $newCookie['value']
                );
