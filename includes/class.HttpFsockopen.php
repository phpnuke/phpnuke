...
class HttpFsockopen
{
    ...
    public function __construct($url, $use_autoload = true)
    {
        ...
        if (
            !empty($url_array["scheme"]) &&
            preg_match("#^https|ssl$#i", $url_array["scheme"])
        ) {
            $this->ssl = true;
        } else {
            $this->ssl = false;
    ...
    public static function load($class)
    {
        $file =
            dirname(__FILE__) .
            DIRECTORY_SEPARATOR .
            preg_replace("#[_]+#", DIRECTORY_SEPARATOR, $class) .
            ".php";
        if (file_exists($file)) {
            include_once $file;
        }
...
class HttpFsockopen_Header
{
    ...
    {
        if (!$this->cookie) {
            $this->cookie = new HttpFsockopen_Cookie($this->get('Set-Cookie'));
        }
        return $this->cookie;
    }
}
