...
class Database implements \IteratorAggregate, \ArrayAccess
{
    ...
    protected function config($path = '')
    {
        ...
        if (is_array($path) && count($path)) {
            ...
            $configValue = isset(
                $this->_config['connections'][$this->_config['default']][$path]
            )
                ? $this->_config['connections'][$this->_config['default']][
                    $path
                ]
                : null;
            if ($path) {
    ...
    private function sql_params_check(&$sql, &$params)
    {
        if (is_array($params) && !empty($params)) {
            ...
            foreach ($params as $param_key => $param_val) {
                $new_param_key =
                    !is_numeric($param_key) && substr($param_key, 0, 1) == ':'
                        ? $param_key
                        : $i;
            ...
            $params =
                sizeof($newparams) == sizeof($params) ? $newparams : $params;
            ...
            if (!empty($matchs1[1])) {
                foreach ($params as $param_key => $param_val) {
                    if (
                        !in_array(str_replace(":", "", $param_key), $matchs1[1])
                    ) {
    ...
    public function query($sql, $params = [])
    {
        ...
        try {
            ...
            if (count($e->errorInfo) == 3) {
                $this->_errors[] = array_merge($e->errorInfo, [$sql]);
            }
        }
    ...
    public function createOrUpdate($values, $conditionColumn = [])
    {
        ...
        // if no condition so search by ID
            ...
            $value = isset($value[$this->_ColumnsId])
                ? $value[$this->_ColumnsId]
                : null;
        }

        // check if any records exists by condition
        $exists = $this->where($column, $value)
            ->first()
            ->results();
        // if exist so update the record's
        if (count($exists)) {
            return $this->where($column, $value)->update($values);
        }
        // insert new record
    ...
    public function update($values = [], $_lastWhere = '', $_lastParams = '')
    {
        ...
        // generate sql statement
        $sql =
            "UPDATE {$this->_table} SET {$set} " .
            ($this->_query != '' ? $this->_query : $_lastWhere);

        // check if query is not have an error
        $this->_params =
            isset($this->_params) && !empty($this->_params)
                ? $this->_params
                : $_lastParams;
        if (
            !$this->query($sql, array_merge($values, $this->_params))->error()
        ) {
            return true;
        }
    ...
    public function save()
    {
        ...
        return $this->update(
            (array) $this->getNewValues(),
            self::$_instance->_lastWhere,
            self::$_instance->_lastParams
        );
    }
    ...
    {
        return $result = $this->where($this->_idColumn, $id)->first();
    }
    ...
    public function in($field, $values = [])
    {
        if (count($values)) {
            ...
            $where =
                " $this->_where $field IN (" .
                implode(",", $params_value) .
                ")";
            $this->_lastWhere .= $where;
            $this->_query .= $where;
            $this->_where = "AND";
    ...
    public function paginate($recordsCount = 0)
    {
        ...
        $startFrom = isset($_GET[config("pagination.link_query_key")])
            ? ($_GET[config("pagination.link_query_key")] - 1) * $recordsCount
            : 0;

        // get pages count rounded up to the next highest integer
        $this->_colsCount = ceil(
            count($this->select()->results()) / $recordsCount
        );
    ...
    public function link()
    {
        ...
        // current page
        $currentPage = isset($_GET[config("pagination.link_query_key")])
        ...
        if ($this->_colsCount > 0) {
            // check if columns count is not 0 or less
    ...
    public function count()
    {
        if (isset($this->_count)) {
            ...
            $results = (array) $this->results();
            return isset($results[0]) || count($results)
                ? count($this->_results)
                : 0;
        }
    }
    ...
    public function join($table, $conditions = [], $join = '')
    {
        if (!empty($conditions)) {
            $this->_query .=
                strtoupper($join) . " JOIN {$table} ON "; // convert $join to upper case (left -> LEFT)
    ...
    public function schema($structures = [])
    {
        if (count($structures)) {
            ...
            foreach ($structures as $column => $options) {
                ...
                $type == 'increments'
                    ? ($type =
                        "INT UNSIGNED PRIMARY KEY AUTO_INCREMENT NOT NULL")
                    : null;
...
class Collection extends Database
{
    ...
    {
        return new static(
            array_merge($this->results(), $this->toArray($items))
        );
    }
