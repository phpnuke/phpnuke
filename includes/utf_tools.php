...
if (extension_loaded('mbstring')) {
    ...
    if (version_compare(PHP_VERSION, '5.2.0', '>=')) {
        ...
        function utf8_strrpos($str, $needle, $offset = null)
        {
            ...
            if (is_null($offset)) {
                ...
                if (!is_int($offset)) {
                    trigger_error(
                        'utf8_strrpos expects parameter 3 to be long',
                        E_USER_ERROR
                    );
                    return false;
                }
    ...
    function utf8_substr($str, $offset, $length = null)
    {
        ...
        if (
            $length === 0 ||
            ($offset < 0 && $length < 0 && $length < $offset)
        ) {
            return '';
        }
...
function utf8_recode($string, $encoding)
{
            ...
            case '15':
                if (!function_exists('iso_8859_' . $array[1])) {
                    if (
                        !file_exists(
                            $phpbb_root_path .
                                'includes/utf/data/recode_basic.' .
                                $phpEx
                        )
                    ) {
                        trigger_error(
                            'Basic reencoder file is missing',
                            E_USER_ERROR
                        );
                    }
    ...
    if (
        ...
        if (!function_exists('sjis')) {
            if (
                !file_exists(
                    $phpbb_root_path . 'includes/utf/data/recode_cjk.' . $phpEx
                )
            ) {
...
function utf8_ord($chr)
{
    switch (strlen($chr)) {
        ...
        case 3:
            return ((ord($chr[0]) & 0x0f) << 12) |
                ((ord($chr[1]) & 0x3f) << 6) |
                (ord($chr[2]) & 0x3f);
            break;

        case 4:
            return ((ord($chr[0]) & 0x07) << 18) |
                ((ord($chr[1]) & 0x3f) << 12) |
                ((ord($chr[2]) & 0x3f) << 6) |
                (ord($chr[3]) & 0x3f);
            break;
...
function utf8_case_fold($text, $option = 'full')
{
    ...
    if (!isset($uniarray['c'])) {
        $uniarray['c'] = include $phpbb_root_path .
            'includes/utf/data/case_fold_c.' .
            $phpEx;
    }

    // only set full if we need to
    if ($option === 'full' && !isset($uniarray['f'])) {
        $uniarray['f'] = include $phpbb_root_path .
            'includes/utf/data/case_fold_f.' .
            $phpEx;
    }

    // only set simple if we need to
    if ($option !== 'full' && !isset($uniarray['s'])) {
        $uniarray['s'] = include $phpbb_root_path .
            'includes/utf/data/case_fold_s.' .
            $phpEx;
    }
...
function utf8_clean_string($text)
{
    ...
    static $homographs = [];
    if (empty($homographs)) {
        $homographs = include $phpbb_root_path .
            'includes/utf/data/confusables.' .
            $phpEx;
    }
...
function utf8_str_replace($search, $replace, $subject)
{
    ...
    for ($i = 0; $i < $length; $i++) {
        ...
        $offset = 0;
        while (
            ($start = utf8_strpos($subject, $search[$i], $offset)) !== false
        ) {
            $subject =
                utf8_substr($subject, 0, $start) .
                $replace[$i] .
                utf8_substr($subject, $start + $search_length);
        ...
        if ($value < 128) {
            ...
            if (count($values) == $num_octets) {
                if ($num_octets == 3) {
                    ...
                    $unicode .=
                        '%' . dechex($values[0]) . '%' . dechex($values[1]);
                    $unicode_length += 6;
                }
