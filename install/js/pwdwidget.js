...
function PasswordWidget(divid,pwdname)
{
	...
	this.txtShow = 'نمایش';
	this.txtMask = 'مخفی';
	this.txtGenerate = 'ساخت';
	this.txtWeak='ضعیف';
	this.txtMedium='متوسط';
	this.txtGood='قوی';
...
function _MakePWDWidget()
{
	...
	code += "<input type='password' class='form-control' name='"+pwdname+"' id='"+this.pwdfieldid+"' placeholder='کلمه عبور' data-validate='validateUpassLabel'>";

	this.pwdtxtfield=pwdname+"_text";

	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	code += "<input type='text' class='form-control' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' style='display: none;' placeholder='کلمه عبور' data-validate='validateUpassLabel'>";
